...
// 加密**七个策略榜全部共用**的轴集，**16 根**（2026-07-30 站长「能否对齐轴数？以最多的
...
const TABS_CONFIG = {
    ...
    // ⚠️ 行 payload 走后端唯一的 `_strategy_row` ⇒ **16 轴的 cryptoStrategySorts**（七个榜
    ...
    // 一个筛刚翻多满一周，实测交集很小）。
    // 行 payload 走后端唯一的 `_strategy_row` ⇒ **16 轴的 cryptoStrategySorts**（七个榜共用；
    ...
    // 的规矩（*MonthlyStrategy / weeklyStrategy 先例），继续有效。
    {
        ...
        tabs: [
            { key: "dailyChange", name: "日线", full: "涨跌幅", tf: "日线",
              desc: "最新已收盘日 K 的涨跌幅（当根收盘 ÷ 当根开盘，币安日 K 每天 00:00 UTC 收盘，所以看的是昨天那一整根）。没有任何筛选条件——全部 加密 USDT 永续合约都在里面，谁涨谁跌一眼看全，是先看清全市场在发生什么、再去策略榜里筛的入口。默认按涨幅从高到低，点排序条可以切成日成交额（涨得多是不是也有人接）、日线RSI（是不是已经超买）、日CVD强弱与日订单流（这波是买盘推的还是卖盘砸的）。上市当天、还没有一根已收盘日 K 的新合约不入榜；上市不足约 23 天的合约涨跌幅照常显示，但 RSI / CVD强弱 / 订单流 会显示「—」（暖机根数不够），排序时自动沉底。" },
            { key: "weeklyChange", name: "周线", full: "涨跌幅", tf: "周线",
              desc: "最新已收盘周 K 的涨跌幅（周 K 每周一 00:00 UTC 收盘，所以整周之内这张榜的数值是不变的，下周一才换一批）。没有任何筛选条件，全部 加密 USDT 永续合约。它比日线那张钝得多，正好用来分辨「这几天的涨只是反弹」还是「整周都在往上走」。请注意排序条上的 RSI、CVD强弱、订单流全部是周线口径而不是日线值，成交额也是那一根周 K 的成交额，不是 7 天滚动也不是日均。只要有 1 根已收盘周 K 就入榜，所以刚上市一两周的新合约也在；但它们的周线 RSI 要 16 根周 K 才算得出来，不够的显示「—」并在排序时沉底。" },
            { key: "monthlyChange", name: "月线", full: "涨跌幅", tf: "月线",
              desc: "最新已收盘月 K 的涨跌幅（月 K 每月 1 号 00:00 UTC 收盘，所以整个月之内这张榜是不动的——那是正确行为，不是数据卡住了）。没有任何筛选条件，全部 加密 USDT 永续合约。这是站内周期最长的一张行情榜，看的是「这个月谁真的走出来了」，短线噪音基本被抹平。请注意排序条上的 RSI、CVD强弱、订单流全部是月线口径，成交额是那一根月 K 的成交额。只要有 1 根已收盘月 K 就入榜（上市当月的新合约还没有，暂不入榜）；月线 RSI 需要 16 根月 K、约一年半才算得出来，所以有四成左右的合约这几根轴会显示「—」并在排序时沉底——这本身也是一种信息：显示「—」的就是还没走完一轮周期的品种。" },
        ],
    },
    {
        ...
        // ① ⊆ ⑦）。**七个榜轴集已于 2026-07-30 对齐成同一套 16 轴。**⑤⑥⑦ 都蕴含
        // 「周线 SAR 多头」但彼此无包含关系（刚翻多满一周 / 结构在张开 / 月线也多头）。
        label: "加密策略", asset: "加密", tf: "月线",
        tabs: [
            { key: "monthlyWeeklyDaily", name: "月线SAR × 周线SAR × 日线SAR",
              desc: "月线、周线、日线三个周期的 SAR 全部多头——最大级别方向、中级别趋势、短期节奏同时向上，纯趋势共振的一档，不看 EMA 也不看资金。上市不足 3 个月、还算不出月线 SAR 的新合约不入榜。范围是全部 加密 USDT 永续合约，表格显示的是日线数值。" },
            ...
            // 成员资格判断。（轴集现为七个榜共用的 cryptoStrategySorts 16 轴，2026-07-30 对齐。）
            ...
            { key: "dailyEmaExpansion", name: "日线9/21扩张＋量/CVD/RSI递增", tf: "日线",
              desc: "四个条件同时成立，全部看最新已收盘的那根日线：① EMA9 在 EMA21 上方，且两者的间距比前一天更大（均线正在张开，不是单纯的多头排列）；② 成交量比前一天大（口径与图表上的成交量柱一致，看的是币的数量不是 USDT 金额）；③ CVD 在 0 轴上方且比前一天更高；④ RSI 比前一天更高。结构、量能、资金、动能四路同时抬头的那一天，找的是启动当日而不是已经走了一段的行情——所以成员天天换，连着两天一样反而少见。上市不足 23 天的新合约数据不够，不入榜。范围是全部 加密 USDT 永续合约。" },
...
function renderMarketOverview() {
    ...
    items.push(`<div class="mkt__item" title="全市场 加密 USDT 永续合约 24h 总成交额">
