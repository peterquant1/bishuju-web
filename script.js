...
// 加密**六个策略榜全部共用**的轴集，**16 根**（2026-07-30 站长「能否对齐轴数？以最多的
...
//
// ⚠️ 轴标签带「日/周/月」前缀是**必要的**：这六个榜的筛选条件横跨月/周/日三个周期，而
...
const TABS_CONFIG = {
    ...
    // 当时的 13 轴周线族，判据存档见后端 build_rankings docstring 末尾）===
    // **轴集只有一族**：六个榜全部共用 **16 轴的 cryptoStrategySorts**，行全部走后端唯一的
    // `_strategy_row`（2026-07-30 站长「能否对齐轴数？以最多的为准」；对齐前是 ①②③④ 用
    // 11 轴、⑤⑥ 用 13 轴的 cryptoWeeklyExpansionSorts 两族）。
    // 六个榜行 payload 逐字同构，**差别只在筛选条件**。首轴统一 = 日成交额降序。
    ...
    // ⚠️ 行 payload 走后端唯一的 `_strategy_row` ⇒ **16 轴的 cryptoStrategySorts**（六个榜
    ...
    // 一个筛刚翻多满一周，实测交集很小）。
    // 行 payload 走后端唯一的 `_strategy_row` ⇒ **16 轴的 cryptoStrategySorts**（六个榜共用；
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        // 榜（⑥ 周线9/21扩张＋SAR多头）。**六个榜轴集已于 2026-07-30 对齐成同一套 16 轴。**⑤⑥ 都蕴含
        ...
        tabs: [
            ...
            // 成员资格判断。（轴集现为六个榜共用的 cryptoStrategySorts 16 轴，2026-07-30 对齐。）
