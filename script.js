...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //   · 纯 K 线形态、零指标（**五个，09-12 那轮没变**）：`monthlyFourBull` / `monthlyFiveBear` + `weeklySixBull` /
        //     `weeklyFourBull` / `weeklyFiveBear`（09-11 06:4x 一度九个；08:1x 移除月线两连阳 / 三连阴后首阳与日线两连阳后六个；
        //     08:5x `dailyFourBull` 改成两连阳＋价涨量增 ⇒ 看成交量、移出这一类 ⇒ 五个，同时新增的周线版同理不计入 ——
        //     **那两张 09-12 整个移除，本类因此没有变化**；`monthlyVolRising` 只数成交量，从头到尾不计入）
        //
        // 【结构性关系 —— **2026-09-12 起**（包含仍四对、一个没动；互斥 八对 → **四对**：
        //   移除的周线那张带走三对、`monthlyVolRising` 撤回阳K 带走一对）】
        ...
        //     → **09-12 移除周线那张（带走三对）＋ `monthlyVolRising` 撤回阳K（带走月线那对）⇒ 回到四对**），全是判据保证：
        ...
        //     ✏️ **09-12 作废的四对**：`weeklyTwoBullExactCloseVolRising` ∩ `weeklySixBull` / ∩ `weeklyFourBull` / ∩ `weeklyFiveBear`
        //       （随榜移除）；`monthlyVolRising` ∩ `monthlyFiveBear`（随阳K 撤回 —— 撤回当天两者交集实测 2 个）。
        ...
        //   · **周线榜八个**（09-11 08:5x 一度九个，**09-12 移除 `weeklyTwoBullExactCloseVolRising` 后八个**）/ **月线榜五个**
        //     （`monthlySarFirstBar` / `monthlyFourBull` / `monthlyFiveBear` / `monthlyVolRising` / `monthlyEma921Persist`；09-10 一度六个，
        //     09-11 08:1x 移除三个后三个，11:0x、17:5x 各新增一个；09-12 那轮没动月线榜数）/ **日线榜两个**（`dailyEmaSarFirstTwo` /
        //     `dailySarFirstBar`；09-11 06:4x 一度四个，**09-12 移除 `dailyFourBull` 后两个**）。
        //     ⇒ 加密策略榜共 **15** 个（09-11 17:5x 一度 17）。
        //
        ...
        tabs: [
            ...
            { key: "weeklyFiveBear", name: "周线五连阴", tf: "周线",
              desc: "最新已收盘的那根周线，正好是连续第 5 根阴线——这一周收阴，往前数第 2 到第 5 周也都收阴，而第 6 周不是阴线。同样是恰好 5 根不是至少 5 根：已经连跌 6 周、8 周的标的不在这张榜上。本榜是「月线五连阴」的周线版，方向与站内大多数榜相反：它一个多头条件都没有，连续五周下跌就是连续五周下跌。所以它有三种截然不同的读法，取决于你自己的判断：可以当成超跌名单，找跌了一个多月、可能酝酿反弹的标的；也可以当成空头趋势确认，按周计的下跌结构完好；还可以当成回避名单，直接排除掉这些标的。本站不替你选其中哪一种——榜只负责把「按周计已经连跌五周」这件事挑出来。卡在第 5 根的用意是：跌一两周是波动，连跌五周说明这是趋势而不是噪声。这同样是一次性的事件清单——同一个标的只在第 5 根那一周出现，下一周它要么成了第 6 根、要么已经收阳，所以相邻两周的名单不会有重叠；它和「周线四连阳」「周线六连阳」永远不会出现同一个标的，因为那两张都要求这一周收阳，而同一根周线不可能既收阳又收阴。命中数时多时少：回放过去 60 周，多数周份是个位数，全市场一起大跌的周份能到一百四十多个，大约每五六周有一周一个都没有。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱——想找超跌反弹候选的，可以看日线那几根轴有没有先转向。上市不足 6 周的新合约看不出「第 6 周不是阴线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // 🔴 2026-09-11 08:1x 站长「移除：月线两连阳，月线三连阴后首阳，月线两连阳＋成交量递增，日线两连阳」（纯移除、无替代）：
