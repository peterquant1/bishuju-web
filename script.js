...
function axesSub(item, sf, volLabel, extra) {
    ...
    // 周线EMA间距：**加密十个策略榜 + 股票系三个策略榜的行都带**（2026-07-30 轴数对齐后
...
// 涨跌幅。**四个资产同批加**，挂在全部十三个策略榜上（加密十个 + 股票系三个）。
...
// 不停盘）；A股/美股/ETF ＝ 相对上一根收盘、含集合竞价跳空。三个涨跌幅榜历来就这么分。
...
const AXIS_D_CHGPCT = { key: "changePercent", label: "日涨跌幅", format: v => fmtGapVal(v.changePercent),
                        hint: "最新已收盘那根日K的涨跌幅。加密＝K线实体(收−开)/开；A股/美股＝相对上一根收盘、含跳空" };
...
// 股票系（A股/美股/ETF）三个策略榜的 **16 轴**：日线十根（日线RSI·日成交额·日涨跌幅·
// 日CVD强弱·日量比·日EMA间距·日ADX·日+DI·日MACD强弱·日波动幅度）+ 周线五根（周线RSI·周线EMA间距·
// 周ADX·周+DI·周MACD强弱）+ 月线RSI —— 严格按"日线 → 周线 → 月线"的周期递进排、同周期
// 的挤在一起。**首轴仍是 日线RSI**（＝默认排序，必须与后端 `value` 取的量一致，别把新轴
// 插到最前）。
// ⚠️⚠️ **比加密的 17 轴少一根「日订单流」，而且永远补不上**：tushare（A股）与 Massive
// （美股/ETF）的日线都**没有 taker 逐笔归边字段** —— 数据源硬边界，不是遗漏。2026-07-30
// 站长「能否对齐轴数？以最多的为准」那轮卡的就是这一根，**别再想办法凑齐**：硬凑要么
// 得造一根语义不同的轴、要么发一根永远全 null 的幽灵轴，两条都比"少一根"更糟。
// 轴数履历：11（指令⑥ 砍 DI 后）→ +周MACD → +日MACD → 2026-07-28 +日波动幅度 ⇒ 14
// → 2026-07-30 轴数对齐 +周线EMA间距 ⇒ 15 → **2026-08-12 +日涨跌幅 ⇒ 16**。
// ⚠️ 「日涨跌幅」插在「日成交额」之后（两根同属"行情量"，技术指标排在它们后面），
// **没动首轴** —— 首轴＝默认排序，必须与后端 `value` 取的量（日线RSI）一致。
const singleStrategySorts = [AXIS_D_RSI, AXIS_D_VOL, AXIS_D_CHGPCT, AXIS_D_CVD, AXIS_D_VOLRATIO, AXIS_D_EMAGAP,
                             ...dmiSorts, AXIS_D_MACD, AXIS_D_ATR,
                             AXIS_WRSI, AXIS_W_EMAGAP, ...weeklyDmiSorts, AXIS_W_MACD, AXIS_MRSI];
// 加密**十个策略榜全部共用**的轴集，**17 根**（2026-07-30 站长「能否对齐轴数？以最多的
// 为准」＋ 2026-08-12「所有升降序也新增一个涨幅升降序」之后的现状）：日线 11 根（日成交额·
// 日涨跌幅·日线RSI·日CVD强弱·日订单流·日量比·日EMA间距·日ADX·日+DI·日MACD强弱·
// 日波动幅度）+ 周线 5 根（周线RSI·周线EMA间距·周ADX·周+DI·周MACD强弱）
...
//
// ⚠️ 轴标签带「日/周/月」前缀是**必要的**：这十个榜的筛选条件横跨月/周/日三个周期，而
 ...
 *  月线榜装月线 RSI）。这与另十个策略榜相反（那些榜通用 key 一律是日线值、周线值另带
...
const TABS_CONFIG = {
    ...
    // ⚠️ 四线族曾经有过两个榜（`dailyFourEmaAligned` 只看排列、2026-07-28 移除；
    // `dailyFourEmaPersist`「扩张存续」、**2026-08-09 站长移除，纯移除无替代**），
    // 现在只剩下面的 ⑦ `dailyFourEmaExpansion`（最新一根正在扩张）。判据存档在后端。
    // ⚠️ 「排列」≠「扩张」：站内「扩张」一词专指**间距在变大**，两个词别混用——这条用词
    // 规矩仍然管着 ⑦ 的 name。
    // ⚠️ `dailySarSecondBar`「日线SAR多头第二根」**2026-08-11 站长移除**（判据存档在后端
    // build_rankings docstring 末尾；复活 = 后端重建列表 + 这里与 TAB_GROUPS 各加回一条）。
    // ⇒ 编号整体前移：原 ④~⑨ 现为 ③~⑧。
    // ③ **最新已收盘周线是 SAR 多头段的首根 K 线（1 个条件）** —— 2026-07-30 建榜时是
    // "第二根"，**2026-08-11 站长改判据成"第一根"**：倒1 多头 且 **倒2 空头**（＝这一根
    // 刚刚由空翻多），全在周线。"事件"型 SAR 榜之一（**现共两个**：本榜与 ⑧ `dailyEmaSarFlip`）。
    // ⚠️⚠️ **key 里的 "SecondBar" 已名不副实**（槽位规矩：逻辑迭代、key 固定，改 key 要
    // 连累 KV/paidMeta/前端/用户 localStorage 四处）。**读到这个 key 别去查"第二根"。**
    // ⚠️⚠️ **与 ③ 不再同构了**（建榜时"只差周期"那句话作废）：③ 仍是"第二根"、本榜已是
    // "首根"，且周期也不同 ⇒ 既不互斥也无包含，各问各的。别再读成"同一件事的两个周期"。
    // ⚠️⚠️ 与 ⑧ `dailyEmaSarFlip` 都叫"首根"但两处不同：① 周期不同（周线 vs 日线）⇒ 无包含
    // 关系；② **本榜是纯 SAR 首根、完全不看 EMA**，⑧ 另要日线 9/21 扩张。别拿本榜当"周线版
    // 的 ⑧"，也别给本榜偷加 EMA 门槛。
    // ⚠️ 本榜条件蕴含「周线 SAR 多头」——自检点（改判据后仍成立）。
    // ⚠️ 行 payload 走后端唯一的 `_strategy_row` ⇒ **17 轴的 cryptoStrategySorts**（十个榜
    // 共用）。周线块那 5 根轴仍是本榜的主要读法（按周ADX / 周MACD强弱 分「大级别也在向上」
    // vs「空头里的反弹」），只是不再靠"轴集不同"来体现。
    ...
    // 一个筛刚翻多的那一周〔2026-08-11 ④ 改判据后是首根、不再是"满一周"〕）。
    // 行 payload 走后端唯一的 `_strategy_row` ⇒ **17 轴的 cryptoStrategySorts**（十个榜共用；
    ...
    // ⚠️ **"事件"型**：只挑"最新那一根正在扩张"的，扩张一停就不在了 ⇒ 成员天天换、
    // 命中数天然很小，看到只有几个标的是正常的。
    // ⚠️ 它上线当天（2026-08-09 早）曾与「日线9/21/55/200扩张存续」（状态型）并列、且是那张
    // 的严格子集，90 天回放实测保留率中位 34.6%、两榜完全相同 0/90 天 ⇒ 当时判定并列是对的；
    // **几小时后站长直接移除了那张**，所以现在四线族只剩本榜，那层包含关系已无对家。
    ...
    // ⚠️ 它与「日线SAR多头第二根」那对结构性互斥**已随该榜 2026-08-11 被移除而消失**
    //   （站内现已没有任何一对结构性互斥的 live 榜）。那条互斥仍写在后端 compute 层的
    //   `sarSecondBar` 注释里，但已降为"live 榜 vs 保留组"。desc 里对应那段也已删。
    ...
    //   同 ③④ 的先例）；也别因为有 EMA 就加 emaGap 轴 —— 它早就在轴集里。
    // 💡 行里 `emaGap` **恒为正**（9/21 扩张蕴含 EMA9>EMA21）⇒ 现在 ②⑥⑦⑧⑨ 五个榜都是。
    dailyEmaSarFlip: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },

    // ⑨ **日线 EMA9/21/55 三线扩张 且 SAR 多头 且 收阴K（3 个条件，全在日线）** ——
    //   2026-08-11 站长「最新已收盘日线是阴K，且三线扩张，且SAR是多头」（没点资产 ⇒ 加密）。
    // ⚠️⚠️ **站内唯一一个要求阴K 的榜，也是唯一"找回调"的榜**：另外八个问的都是"哪些在
    //   启动/在延续"、天然偏阳线一侧；本榜要的是多头结构里收阴的那一天（回踩/二次上车）。
    // ⚠️ 与 ⑥ `dailyTripleEmaCvd` **两个方向都不包含**：两者都要三线扩张，但本榜再加
    //   "阴K + SAR多头"、⑥ 再加 "CVD 递增"。**90 天回放里两榜交集累计 0 人次** —— 因为
    //   日CVD强弱的买卖拆分方向完全由 close>open 决定（见上方 AXIS_D_CVD 那段），阴K 那天
    //   极难 CVD 递增。⚠️ **但那不是判据保证的互斥**（CVD 走 EMA(14) 平滑，一根小阴线可能
    //   压不过前 13 根）⇒ 别把"两榜不相交"写成结构性结论。
    // ⚠️ **命中天然很小、空榜正常**：90 天回放 min 0 / 中位 4 / max 15 / 2 天为 0。
    //   **相邻两日成员 Jaccard 中位 0.000**（25/39 对完全不重合）⇒ 强"事件"型：阴K 是单日
    //   事件，池子（三线扩张）虽稳，榜上的人几乎天天全换。看到跟昨天完全不是一批人别当故障。
    // ⚠️ **别因为判据里有阴K/SAR 就加轴**：两者都是布尔值、排不了序（同「阴K / SAR多头
    //   不设轴」那条在案先例）；也别因为有 EMA55 就加 ema55 轴（后端没发该字段 ⇒ 幽灵轴）。
    // 💡 行里 `emaGap` **恒为正**（三线扩张蕴含 9/21 扩张），同 ⑥⑦ 那个免费自检点。
    dailyTripleEmaSarBearish: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },

    // ⑩ **周线 EMA9/21 扩张（1 个条件）** —— 2026-08-12 站长「最新已收盘周线是EMA9/21扩张」
    //   （没点资产 ⇒ 加密，在案十余次的先例）。
    // ⚠️⚠️ **④ `weeklyEmaSarBull` ⊊ 本榜 —— 站内第二对结构性包含关系**（④ ＝ 本榜 ∩ 周线
    //   SAR 多头）。此前只有「① ⊊ ⑤」一对，**从 2026-08-12 起是两对**，别再照旧注释写
    //   "站内唯一一对"。差集＝"结构已张开、SAR 还没站到多头一侧"的更早期形态。
    // ⚠️ **不适用「严格子集 ⇒ 默认取代」**（那条只针对保留率九成级的双胞胎）：本榜是全集、
    //   ④ 再叠一道趋势确认，量级与语义都分得开，且站长指令就是"新增"。两个名字只差后半截
    //   是刻意的，让包含关系在 rail 上直接可见（同 ①/⑤ 那对的先例）。
    // ⚠️⚠️ **与 2026-07-23 那个已移除的 `weeklyEmaExpansion` 判据不同 ⇒ 开了新 key**：
    //   那个是 (9/21 ∪ 9/26) **并集**，本榜是**严格 9/21**。「同判据 ⇒ 复用旧 key」只适用于
    //   判据逐字相同的情形。name 因此写「9/21扩张」不写「两线扩张」（后者是并集专称）。
    // ⚠️ 与 ③ `weeklySarSecondBar`（纯 SAR 首根、完全不看 EMA）**无包含关系**：一个问"结构
    //   有没有在张开"（状态型）、一个问"这一周是不是刚翻多"（事件型）。
    // ⚠️ **周线走缓存 ⇒ 同一周内成员完全不变**，每周一 00:00 UTC 才换一批（同 ③）。
    //   周内反复打开看到完全一样的标的是正确行为，不是数据卡住了。
    // 💡 行里 `weeklyEmaGap` **恒为正**（扩张的定义含"EMA9 在 EMA21 上方"）——这是全站
    //   第二个 weeklyEmaGap 恒正的榜（另一个是 ④），免费自检点。
...
const TAB_GROUPS = [
    ...
    {
        // 加密策略榜：**十个**（数量以下面 tabs 数组的条目为准，别在注释里维护一个会发霉
        ...
        // 的 5 个条件（月线SAR + 周线SAR + 周线扩张 + 日线扩张 + CVD），与这十个榜都不
        // 一样。硬套会让页面上的规则说明与后端实际筛选严重不符。
        // 组 tf 取第一个 tab 的周期（月线），其余 tab 各自覆盖。**排法：同周期的放一起 +
        // 新榜追加在末尾**（① 月×周×日 → ② 日线9/21扩张＋三递增 → ③ 周线SAR首根 →
        // ④ 周线扩张＋SAR → ⑤ 月×周 SAR → ⑥ 日线三线＋CVD → ⑦ 日线四线扩张 →
        // ⑧ 日线9/21扩张＋SAR首根；追加在末尾是为了不让整套编号位移）。
        ...
        // ⚠️ 现存两个"事件"型 SAR 榜（③ 周线首根 / ⑧ 日线首根＋EMA，**两者周期不同、且 ③
        // 完全不看 EMA ⇒ 无包含关系**）+ 一个"趋势＋结构双确认"榜（④ 周线9/21扩张＋SAR多头）
        // + 两个纯 SAR 共振"状态"型榜（① 三级 / ⑤ 两级，**① ⊆ ⑤**，"更严 vs 更宽"、
        // 2026-08-01 起知情保留）。
        // ⚠️⚠️ **站内现有两对结构性包含关系**（2026-08-12 起，别再照旧注释写"仅存一对"）：
        // **① ⊆ ⑤** 与 **④ ⊆ ⑩**（后者＝「周线9/21扩张＋SAR多头」⊊「周线9/21扩张」）。
        // **各榜轴集已于 2026-07-30 对齐成同一套**（2026-08-12 加「日涨跌幅」后为 17 轴）。
        // ③④⑤ 都蕴含「周线 SAR 多头」但彼此无包含关系（刚翻多那一周 / 结构在张开 /
        // 月线也多头）；⑩ 只看 EMA、不蕴含 SAR 多头。
        ...
        tabs: [
            ...
            // 成员资格判断。（轴集现为十个榜共用的 cryptoStrategySorts 17 轴。）
            ...
            // ⚠️ 四线族曾经的两个榜都已移除，判据存档见后端 build_rankings：
            ...
            // ⇒ 四线族现在只剩下面的「日线9/21/55/200扩张」一张（事件型）。
            // ⚠️ **name 用「排列」不用「扩张」**：站内「扩张」专指间距在变大，两个词混用
            // 会把判据说反 —— 这条用词规矩现在管着那张仅存的四线榜。
            // ⚠️ 「日线SAR多头第二根」`dailySarSecondBar` **2026-08-11 站长移除**（纯移除、
            // 无替代）。判据存档在后端 build_rankings docstring 末尾；compute 层旗标
            // `sarSecondBar` 照常产出为保留组 ⇒ 复活 = 后端重建列表 + 这里与 TABS_CONFIG
            // 各加回一条，零抓取/缓存改动。
            // tf 周线：两根 K 都在周线。⚠️ 站长逐字（两次）：2026-07-30 建榜「最新已收盘周线
            // 是SAR多头的第二根K线。」→ **2026-08-11「周线SAR多头第二根，这个TAB的逻辑改为：
            // 周线SAR多头第一根。」**（本槽位第二版判据：倒1 多头 且 倒2 空头 ＝ 刚刚翻多）
            // ⚠️⚠️ **key 仍是 `weeklySarSecondBar`、"SecondBar" 已名不副实**（槽位规矩：逻辑
            // 迭代、key 固定）。**读到这个 key 别去查"第二根"。**
            // ⚠️⚠️ **与上面那张日线版不再同构**（建榜时"只差周期"那句话作废）：那张仍是"第二
            // 根"、本榜已是"首根"，周期也不同 ⇒ 既不互斥也无包含。
            // ⚠️⚠️ 与 ⑨「日线9/21扩张＋SAR多头首根」都叫"首根"：周期不同 ⇒ 无包含关系；且
            // **本榜是纯 SAR 首根、完全不看 EMA**，⑨ 另要日线 9/21 扩张。名字里不含 EMA 的
            // 那个就是纯的。
            // ⚠️ 本榜条件蕴含「周线 SAR 多头」：SAR 多头是"状态"、能连着很多周为真，本榜是
            // "事件"——只挑其中刚翻多的那一周。轴集与其余各榜相同（17 轴，行走 `_strategy_row`）。
            { key: "weeklySarSecondBar", name: "周线SAR多头首根", tf: "周线",
              desc: "最新已收盘的那根周线，正好是这一轮 Parabolic SAR 多头趋势的首根——也就是说 SAR 圆点刚刚在这一周由价格上方翻到下方，而前一周还是空头。它抓的是周线级别趋势反转的那一刻：日线翻多是几天的事、容易反复，周线翻多是几周的事，一旦成立分量重得多，所以「周线刚刚翻多」往往是一波大级别行情起步的最早信号，比等它走完一两周再确认要提前得多，代价是还没有经过任何时间检验、假信号的比例也更高。这是一次性的事件清单不是持续清单——同一个标的在一轮行情里只在翻多的那一周出现，下一周它就成了第二根、不再在榜上，所以命中数天然不多、而且每周换一批人。要留意本榜只看 SAR、完全不看均线：那些「SAR 已经翻多、但均线结构还没张开」的标的这里有，需要结构也确认的话看「周线9/21扩张＋SAR多头」那张。还有一点和日线的榜不同：周线数据每周一才更新一次，所以同一周之内反复打开本榜，看到的标的完全一样是正常的，不是数据卡住了。表格里的日线八轴和周线五轴都不参与筛选，用来在这批标的里再分强弱。上市不足 3 周的新合约看不出「翻多的前一周是空头」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            ...
            // ⚠️ **本榜上线当天（2026-08-09 早）曾 ⊊「日线9/21/55/200扩张存续」**（严格子集，
            // 两个名字只差最后两个字是刻意的，让包含关系在 rail 上可见）。当时判定"两张都留"
            // 的依据是 90 天回放：保留率中位 34.6%、完全相同 0/90 天、相邻两日 Jaccard 本榜
            // 40% vs 那张 89% ⇒ 一个事件、一个状态，分得开。
            // **⚠️ 但站长当天稍后直接移除了那张** ⇒ 那层包含关系已无 live 对家，desc 里那段
            // 对比也已改写成不依赖对家的说法。此处保留只为存档当时的判断依据。
            // ⚠️ **name 把周期数写全**（9/21/55/200），遵守加密"不用「N 线扩张」说法"的规矩。
            { key: "dailyFourEmaExpansion", name: "日线9/21/55/200扩张", tf: "日线",
              desc: "只有一个条件，看最新已收盘的那根日线：EMA9 > EMA21 > EMA55 > EMA200 四条均线依次排开，而且 9 与 21、21 与 55、55 与 200 这三档间距全部比前一天更大——四个周期的均线同时在张开，是短、中、长三个层级一起加速的那一天。它是本站门槛最严的一档趋势启动信号：既要四条均线的先后次序全对，又要三档间距同一天一起扩大，任何一档在收窄就出局。要留意这是一张一次性的事件清单：同一个标的只在真正扩张的那几天出现，扩张一停就不在了，所以命中数天然很小、而且几乎每天换一批人，看到只有几个标的是正常的。它问的是「今天是不是正在扩张」而不是「这一轮扩张有没有一直没破」，所以它抓的是加速发生的那一刻，不适合用来跟踪一段趋势是否还完好——扩张过之后进入平推或缓慢收窄、但四条均线次序尚未被打破的标的，在这张榜上是看不到的。上市不足 201 天、还算不出 EMA200 的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            ...
            // tf 日线。⚠️ 2026-08-11（当日第 4 条指令）站长逐字：「新增一个TAB，逻辑是：
            // 最新已收盘日线是阴K，且三线扩张，且SAR是多头。」（没点资产 ⇒ 加密）
            // ⚠️ 判据 ①② ＝ compute 层 `triple_ema_bearish_data`（三线扩张 ∩ 阴K，那份产出
            // 自 2026-07-21 移除旧 tab 后闲置近三周、本次转正），③ 是 build 层显式的
            // `and s in daily_sar_bull_data`。**不是那个旧 tab 的复活**（旧的没有 SAR 那条）。
            // ⚠️ **name 把 EMA 周期数写全**（加密不用「N 线扩张」的说法，那是股票系的命名）；
            // 三个条件同周期 ⇒ 一律用 ＋ 不用 ×。「阴K」用站长的原词。
            // ⚠️ desc 要如实写清三件事：这是站内唯一找回调的榜、成员几乎天天全换、空榜正常。
            { key: "dailyTripleEmaSarBearish", name: "日线9/21/55扩张＋SAR多头＋阴K", tf: "日线",
              desc: "三个条件都看最新已收盘的那根日线：① 它是一根阴 K，收盘价低于开盘价；② EMA9 在 EMA21 上方、EMA21 又在 EMA55 上方，而且 9 与 21、21 与 55 这两档间距都比前一天更大——三条均线依次排开并且还在张开；③ Parabolic SAR 处在多头一侧，圆点仍在价格下方。它是本站唯一一张专门找回调的榜：其余各榜问的都是「哪些标的正在启动或还在延续」，命中的自然多是当天上涨的；这张反过来，要的是趋势结构完好、方向也没被破坏，但当天收了一根阴线的标的，也就是强势中的回踩。第三个条件不是可有可无的摆设——一根够深的阴线正好可能击穿 SAR 圆点让它翻空，那种「结构还在、方向已经先坏了」的标的会被它挡在外面，留下的才是回调而不是转折。要留意这是一张一次性的事件清单：收阴是单日事件，同一个标的今天在榜上、明天收阳就不在了，所以成员几乎每天全换，看到跟昨天完全不是同一批标的是正常的；遇到空榜同样正常。上市不足 56 天、还算不出 EMA55 的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            // tf 周线：唯一的条件在周线。⚠️ 2026-08-12 站长逐字：「新增一个TAB，逻辑是：
            // 最新已收盘周线是EMA9/21扩张。」（没点资产 ⇒ 加密，在案十余次的先例）
            // ⚠️⚠️ **⑤「周线9/21扩张＋SAR多头」⊊ 本榜**（那张 ＝ 本榜 ∩ 周线 SAR 多头）——
            // 站内第二对结构性包含关系（第一对是 ①⊊④）。两个名字只差后半截是刻意的，
            // 让这层关系在 rail 上直接可见。**不适用「严格子集⇒默认取代」**（那条只针对
            // 保留率九成级的双胞胎）：本榜是全集、那张再叠一道趋势确认，量级与语义分得开。
            // ⚠️ name 写「9/21扩张」不写「两线扩张」：后者是 (9/21 ∪ 9/26) 并集专称，
            // 本榜严格 9/21（与 2026-07-23 那个已移除的并集榜判据不同，故 key 也是新的）。
            { key: "weeklyEma921Expansion", name: "周线9/21扩张", tf: "周线",
              desc: "只有一个条件，看最新已收盘的那根周线：EMA9 在 EMA21 上方，而且两条线的间距比上一周更大。两个部分缺一不可——间距的数值变大不等于扩张，如果 EMA9 还在 EMA21 下方，那只是跌势在收窄而不是涨势在张开。它是本站条件最少的榜之一，问的就是一件事：周线级别的均线结构有没有正在加速张开。因为不附加任何方向或资金确认，命中会比同组其他周线榜宽，里面既有已经走出一段的强势标的，也有刚刚由弱转强、结构才张开一两周的。它与「周线9/21扩张＋SAR多头」是同一件事的宽窄两档：那张在本榜的基础上再要求 Parabolic SAR 也站在多头一侧，所以那张的标的一定也在本榜里，本榜多出来的部分正是「结构已经张开、但 SAR 还没翻多」的更早期一批——想要更早介入看本榜，想要方向也确认过再动手看那张。要留意周线数据每周一才更新一次，所以同一周之内反复打开本榜，看到的标的完全一样是正常的，不是数据卡住了。表格里的日线各轴和周线各轴都不参与筛选，用来在这批标的里再分强弱。需要至少 22 根已收盘周 K 才算得出 EMA9/21 扩张，所以上市不足约半年的新合约不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
        ],
    },
    ...
    // （singleStrategySorts 16 轴，A股 的三个「递增」是布尔值不设轴），故 TABS_CONFIG
