...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "monthlySarBearish", name: "月线突破SAR空头第二根低点＋至少二连阳", tf: "月线",
              desc: "两个条件都看最新已收盘的那根月线：一是它的收盘价站上了最近一轮月线 SAR 空头段里第 2 根 K 线的最低价，二是它至少是连续第 2 根阳线——这个月和上个月都收阳，连涨三个月、十个月的也都算，不设上限。先拆开说第一条：在月线上找到最近一轮 SAR 处在空头（圆点在 K 线上方）的那一段，取这一段第 2 根月 K 的最低价当参照线，再看最新已收盘那根月线的收盘价有没有高过这条线。第二条把「价格只是刚好蹭到线上方」和「按月计的上涨已经连起来」分开：站上了参照线、但这个月或上个月收了阴的，不在本榜上。有四处要先说清楚。第一，「最近一轮」不要求那一段已经结束：如果最新这根月线的 SAR 还在空头侧，参照的就是正在进行的这一轮下跌——收盘价站上这轮下跌第 2 根的低点、又已经连涨两个月，是抢在 SAR 翻多之前的领先信号；如果 SAR 已经翻多，参照的就是刚刚结束的那一段。本榜完全不问当前 SAR 是多头还是空头。第二，有一个退化的情形：当那个参照根恰好就是最新已收盘的这根月线时，收了阳线的 K 线收盘价必然高于自己的最低价，这类标的只要连涨两个月就一定命中——这是刻意保留的，不是漏做。第三，参照价取的是那一段第 2 根的最低价，不是收盘价、也不是第 1 根；段里只有 1 根、或者可用历史里一次空头都没出现过的标的没有参照线，不入榜。第四，连阳这一条是至少不是恰好，所以它和「月线二连阳或四到八连阳」那张榜口径不同：正好连涨三个月、九个月以上的标的可以出现在本榜、却不在那张；反过来那张的成员也未必站上了参照线——两张之间没有包含关系。另一方面，本榜的每一个标的必然都在「月线成交量递增或阳K」上（最新这根收阳就满足那张的条件）。命中数：加上连阳条件那天（最新已收盘的是 2026 年 8 月）是 38 个，只看参照线时是 120 个。回放过去 48 个月，每月中位 12 个、最多 75 个，有 2 个月一个都没有；合计下来大约只剩只看参照线时的五分之一。成员的连阳根数以两个月最多、三个月次之，连涨七个月以上的很少。这仍是一张门槛清单，但换得比只看参照线时快得多：上个月的成员这个月还在的比例中位数约 46%（只看参照线时是 84%），已经结束的在榜段中位只有 1 个月、最长 6 个月——连阳一断就离开。也要如实说一句：它不是择时信号。回放里成员下个月收盘更高的比例是 42.6%（767 个样本），当月可判全体是 37.3%，高出五个百分点左右；按每个月和当月全市场的中位数比，成员下个月跑赢的比例约 54.5%，比只看参照线时（约 52%）略好——但样本少了五倍，撑不起一个结论。门槛：算 SAR 至少要 3 根已收盘月 K。另外月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，同一个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。范围是全部加密 USDT 永续合约。" },
            { key: "monthlyFourToSixBull", name: "月线二连阳或四到八连阳", tf: "月线",
              desc: "最新已收盘的那根月线，正好是连续第 2 根阳线，或者正好是连续第 4 到第 8 根阳线——按月计的连涨恰好走到第二个月，或者走在第四、五、六、七、八个月这五档之中的一档；恰好第 3 根的那一档刻意不要。有四处要先说清楚。第一，每一档都是恰好不是至少：只连涨 1 个月的不在榜上，正好连涨 3 个月的不在，已经连涨 9 个月、10 个月的也不在（它们的最新一根是第 9 根、第 10 根）。第二，因为中间空掉了第 3 根，同一个标的在榜上是断开的：连涨到第二个月时在榜上，第三个月若再收阳就离开，第四个月再收阳又回来，之后第 5、6、7、8 根都还在，第九个月必然离开（要么成了第 9 根、要么连阳已经断掉）——所以最多连着出现五个月（第 4 到第 8 根）；回放过去 48 个月，已经结束的在榜段最长只有 4 个月，上个月的成员这个月还在的比例中位数只有 3% 左右（成员大多是刚满两个月那一档，下个月不是连阳断了、就是走到第 3 根离开）。第三，命中里绝大多数是刚满两个月那一档：改判据当天（最新已收盘的是 2026 年 8 月）139 个里有 136 个是第 2 根，第 5 根 2 个，第 6 根 1 个，第 4、7、8 根一个都没有（被排除的第 3 根那一档有 12 个）；按 48 个月合计，第 2 根占八成，第 4 根约一成二，第 8 根那一档这 48 个月里一次都没出现过。如果你要找的是「已经涨了很久」的那一批，请用表格上方的排序轴再筛。第四，本榜只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按月计的上涨连续了几个月。本榜的每一个标的也必然都在「月线成交量递增或阳K」上——最新这根收阳就满足那张的条件；那张范围宽得多，放量收阴的、只涨了一个月的、正好连涨三个月的、已经连涨九个月以上的也都在。另外站内还有一张「周线四到六连阳」，那是周线上的第四到第六周，周期和区间都和本榜不同，别混起来看。命中数跟着大盘走、波动很大：回放过去 48 个月，每月中位 10 个左右，四分之一的月份不超过 3 个，全市场一起普涨的月份能到 139 个（正好就是改判据当天），48 个月里有 2 个月一个都没有。最后一句要紧的：把它当择时信号要很小心。回放里成员下个月收盘更高的比例约 39.1%（1147 个样本），全市场约 36.9%，只高出两个百分点。去掉第 3 根那一档的效果也要如实说：单看那一档，下个月收盘更高的比例是 45.3%（333 个样本），反而是各档里最高的——连涨三个月的标的多扎堆出现在全市场普涨的月份；换成每个月和当月全市场的中位数比，那一档下个月跑赢的只有 47.7%，其余各档合起来是 51.1%，去掉它只是让这张榜相对大盘略好一点，谈不上质变。它更适合当成「按月计的上涨已经连续起来」的筛选起点，再用表格里的各轴自己分强弱。另外月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，同一个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选。门槛方面：判「恰好」必须看得见连阳段之前那一根，所以第 2 根这档要至少 3 根已收盘月 K，第 4 根那档要 5 根、往上每加一档多要一根、第 8 根那档要 9 根，上市不足三个月的新合约不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
        ],
    },
