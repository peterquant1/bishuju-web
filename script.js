...
function axesSub(item, sf, volLabel, extra) {
    ...
    // 周线EMA间距：**加密八个策略榜 + 股票系三个策略榜的行都带**（2026-07-30 轴数对齐后
...
//   消失，而对齐的收益是八个榜口径一致）。要重新评估就得先在股票系数据上重测 ——
...
// 加密**八个策略榜全部共用**的轴集，**16 根**（2026-07-30 站长「能否对齐轴数？以最多的
...
//
// ⚠️ 轴标签带「日/周/月」前缀是**必要的**：这八个榜的筛选条件横跨月/周/日三个周期，而
 ...
 *  月线榜装月线 RSI）。这与另八个策略榜相反（那些榜通用 key 一律是日线值、周线值另带
...
const TABS_CONFIG = {
    ...
    // ⚠️ 行 payload 走后端唯一的 `_strategy_row` ⇒ **16 轴的 cryptoStrategySorts**（八个榜
    ...
    // 一个筛刚翻多满一周，实测交集很小）。
    // 行 payload 走后端唯一的 `_strategy_row` ⇒ **16 轴的 cryptoStrategySorts**（八个榜共用；
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            // 成员资格判断。（轴集现为八个榜共用的 cryptoStrategySorts 16 轴，2026-07-30 对齐。）
