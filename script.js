...
const TABS_CONFIG = {
    ...
    // 行 payload 同样走后端 `_wk_expansion_row` ⇒ **与 ④ 共用这 13 轴**（本榜是这个常量
    // 的第二个消费者，改它会同时动两个榜）。周线那 5 根在这里是描述值：按 周线EMA间距 /
...
const TAB_GROUPS = [
    ...
    // 的规矩（*MonthlyStrategy / weeklyStrategy 先例），继续有效。
    {
        ...
        // （月×周×日 → 日线两个 → 周线两个）——两个日线榜相邻是刻意的：判据只差一处
        // （9/21/55 三线扩张 vs 四线排列且启动过），挨着才看得出对照。周线那两个
        // （周线SAR多头 / 日线突破周线SAR空头起点）2026-07-28 起也相邻，两者都以周线 SAR
        // 为轴心、行构造与 13 轴完全相同，**但没有包含关系**——挨着放正是为了看出这一点：
        // 一个问"周线方向现在是不是向上"，另一个问"日线有没有先站上这轮下跌的起点"。
        // ⚠️ 曾经还有第三个周线榜（并集＋周CVD），2026-07-26 晚站长要求移除。
