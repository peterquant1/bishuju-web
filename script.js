...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        // ④ 日线9/21扩张＋SAR首根 → ⑤ 月线SAR首根 → ⑥ 周线9/21/55扩张；追加在末尾是
        // 为了不让整套编号位移，所以 ⑤ 虽是月线榜不挪去 ① 旁边、⑥ 虽是周线榜也不挪去
        // ② 旁边）。
        ...
        // `weeklyTripleEma` 成 ⑥（追加在末尾 ⇒ 既有编号未动）。
        ...
        // 张开，完全不看 SAR 也不看资金）。
        // ⚠️ **周线榜现为两个（②⑥），互不包含**：都蕴含周线 9/21 扩张，② 再加 SAR 门、
        // ⑥ 再加 EMA55 档 ⇒ 「站内没有任何一对结构性包含的 live 榜」继续成立。
        ...
        tabs: [
            ...
            // 站长移除**（判据存档见后端）⇒ 站内不再有要求阴K 的榜、也不再有"找回调"的榜。
