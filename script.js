...
const TAB_GROUPS = [
    ...
    // 的规矩（*MonthlyStrategy / weeklyStrategy 先例），继续有效。
    {
        ...
        tabs: [
            ...
            // **站内已经没有 live 的并集榜了** —— 但本榜是严格 9/21、`dailyEmaExpansion` 是
            // 严格 9/21/55，**绝不能因为"没有并集榜要区分了"就把它们简称回「N 线扩张」**：
            // 并集口径随时可能随一条指令回来（`ema926Expansion` 旗标一直在后端产出），那时
            // 这两个词的含义必须还是干净的。（「三线扩张」在站内历来只用于股票系那套命名。）
