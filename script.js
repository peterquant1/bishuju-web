...
const TABS_CONFIG = {
    ...
    // === 加密策略：**九个榜**（数量以下面的条目为准；完整增删履历见 CLAUDE.md 顶部的日期
    ...
    //   同 ③④ 的先例）；也别因为有 EMA 就加 emaGap 轴 —— 它早就在那 16 根里。
...
const TAB_GROUPS = [
    ...
    {
        // 加密策略榜：**九个**（数量以下面 tabs 数组的条目为准，别在注释里维护一个会发霉
        ...
        // + 两个纯 SAR 共振"状态"型榜（① 三级 / ⑤ 两级，**① ⊆ ⑤ 是站内仅存的一对严格包含
        // 关系**，"更严 vs 更宽"、2026-08-01 起知情保留）。
        // **八个榜轴集已于 2026-07-30 对齐成同一套 16 轴。**③④⑤ 都蕴含
        // 「周线 SAR 多头」但彼此无包含关系（刚翻多那一周 / 结构在张开 / 月线也多头）。
        ...
        tabs: [
            ...
            // "事件"——只挑其中刚翻多的那一周。轴集与其余各榜相同（16 轴，行走 `_strategy_row`）。
