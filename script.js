...
const TABS_CONFIG = {
    ...
    weeklyBreakFirstEmaBearExpansionFirstBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "dailyEma926SarDivergencePersist", name: "日线9/26金叉存续＋最近一轮空头首根SAR点高于前一轮", tf: "日线",
              desc: "两个条件都看最新已收盘的那根日线，而且都是「还在不在」的状态，不是「刚刚发生」的事件：一是 EMA9 在 EMA26 上方——9/26 金叉之后还没有死叉；二是最近一轮 Parabolic SAR 空头段首根日线上的 SAR 圆点，比再前一轮空头首根的圆点更高。拆开说第二条：SAR 由多翻空的那一根，圆点会直接跳到刚结束那一波上涨的最高价附近，所以「空头首根的圆点」基本就是那一波上涨的顶；拿最近一轮和再前一轮比，比的是最近一个波段高点有没有高过前一个波段高点。这个比较在那一轮空头开始的那天就定下来了，之后不管 SAR 翻回多头还是继续空头都不变，要等下一轮空头出现才重新比一次——这就是这里说的「存续」。两条合起来找的是：日线 EMA9 还在 EMA26 上方、最近一次见顶又比上一次高的标的，也就是日线级别的上升结构还没被破坏的那一批。有五处要先说清楚。第一，「最近一轮空头」包括正在进行的这一轮：最新这根日线的 SAR 还在空头侧（价格正在回调）时，比的就是这一轮回调开始那天的圆点，所以本榜不看当前 SAR 是多头还是空头——表格里「日SAR多头根数」显示「—」的就是正在回调的，上线这一天 282 个里有 83 个，回放里平均约占一半。第二，两个「空头首根」都必须是真正由多翻空的那一根——日线只取最近约 500 根，最早那段空头的开头可能不在窗口里，这种没有可靠参照的标的不入榜。第三，两处比较都是严格大于，一样高不算。第四，第一条只看最新这一根日线上两条均线的上下位置，不要求在可用历史里看得到那次金叉——新币上市后 EMA9 一直在 EMA26 上方的也算；EMA 的算法和 TradingView 自带的 EMA 一致。第五，本榜不看 EMA9/21 是否扩张，也不看成交量、RSI 和 CVD。和站内其他榜的关系：「日线9/21扩张＋SAR多头首根＋最近一轮空头首根SAR点高于前一轮」比的也是这一对圆点——那张只在 SAR 重新翻多的那一天比，本榜从这一轮空头开始的那天起就一直记着；那张的标的绝大多数也在本榜上，但不是必然：9/21 扩张推不出 EMA9 在 EMA26 上方，回放 360 天里那张累计 1196 个成员（按天计）中有 49 个当天不在本榜。「日线SAR空头首根＋SAR点高于前一轮空头首根＋月线突破SAR空头第二根低点」的前两条，就是本榜第二条在那一轮空头开始那天成立，所以那张的标的只要日线 EMA9 也在 EMA26 上方，就一定在本榜上；但那张不要求这一条、本榜不看月线，两张互不包含。这是一张状态清单：两条都不破就一直在——回放里前一天的成员第二天还在的比例中位约 94%，已经结束的在榜段中位 9 天、最长 106 天；离开本榜的，约四分之三是 EMA9 跌回 EMA26 下方，约五分之一是新一轮空头开始、首根圆点没能高过上一轮（最近一个高点没创新高），其余是两条同一天破。命中数：回放过去 360 天（2025 年 9 月 26 日到 2026 年 9 月 20 日），每天中位 60 个、最少 20 个、最多 325 个，没有一天是空的，中位约占全市场的 12%；只看第一条每天中位 97 个，只看第二条中位 191 个。上线这一天（最新已收盘的是 2026 年 9 月 20 日那根日线）是 282 个，占全市场 523 个的一半多，接近这 360 天里的最高（最多的几天在 2026 年 5 月中和 9 月上旬），是这几天普涨后的高位，别当常态。也要如实说一句：它不是择时信号。回放里成员次日收涨的比例 47.0%（31726 个样本），全市场 45.8%；5 天后收盘比入榜那天更高的比例 45.5%，全市场 42.8%，5 天后涨跌幅中位数 −1.01%（全市场 −1.44%）；只看第一条是 45.9% 和 44.3%，只看第二条是 46.4% 和 43.0%。合在一起算略好于大盘，但按天比，成员次日涨跌幅的中位数跑赢当天全市场的只有 46% 的天数，而且成员天天大多是同一批、样本高度重叠，这点差距撑不起结论。它更适合当成「日线上升结构还没坏」的背景名单，配合别的榜或表格里的各轴再筛。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。上市不足 26 天的新合约算不出 EMA26，不入榜。范围是全部加密 USDT 永续合约。" },
            { key: "weeklyBreakFirstEmaBearExpansionFirstBull", name: "周线突破首次9/21空头扩张后首根阳线收盘价", tf: "周线",
              desc: "只看最新已收盘的那根周线：它的收盘价，高过了这个币「第一次 EMA9/21 空头扩张」之后第一根阳线的收盘价；如果从那之后一根阳线都没有，就改拿第一根阴线的收盘价来比。拆开说：EMA9/21 空头扩张指 EMA9 在 EMA21 下方、而且两条均线的间距比上一周更大（跌势正在张开），和站内「9/21扩张」正好反过来，死叉的那一周本身就算；「第一次」指这个币有周线以来的第一次，对新币来说往往就是上市后的第一轮下跌。从那一周的下一周起往后找，遇到的第一根收阳的周 K（收盘高于开盘，也就是这轮下跌开始后的第一次反弹）就是参照根，拿它的收盘价当参照线，再看最新已收盘那根周线的收盘价有没有高过这条线。找的是：价格已经站回这个币第一轮下跌之后第一次反弹收盘价上方的标的。有五处要先说清楚。第一，是整段历史里的第一次、不是最近一次：参照线一旦定下就永久不变，之后再怎么跌、再怎么涨都不换，所以这是一条长期的线——上市早的老币，参照线多在 2020 到 2022 年（比如 BTC 是 2020 年 3 月那轮大跌之后的第一根阳线），新币多在上市后的头几个月；上线这一周的 99 个里，有 84 个的第一次空头扩张发生在 2025 或 2026 年。第二，「之后」从扩张那一周的下一周算起，扩张那一周本身收阳也不算；收盘价正好等于开盘价的十字星，既不算阳线也不算阴线。第三，改拿首根阴线来比的那种情况几乎不会入榜：一根阳线都没有，说明之后每周都收阴、收盘价一路走低，高不过第一根阴线的收盘价；而一旦出现第一根阳线，参照线就换成那根阳线——回放 104 周里没有一个标的是靠这一支入榜的，上线这一周也没有标的处在这种状态。第四，没有参照线的不入榜：EMA21 要 21 根周 K 才算得出、比间距还要再多 1 根，上市不足 22 周的新合约算不出（上线这一周有 20 个）；有周线以来一次空头扩张都没出现过的也没有参照线（上线这一周有 11 个，多是上市后周线一直偏强的）。第五，比较是严格大于，一样高不算；本榜只看周线 K 线和周线 EMA9/21，不看 SAR、成交量和 RSI；EMA 的算法和 TradingView 自带的 EMA 一致。和站内其他榜的关系：「周线突破SAR空头最新阳线收盘价＋SAR多头」也是拿一轮下跌里某根阳线的收盘价当参照线，但那张用的是最近一轮 SAR 空头段、每轮下跌都会换线，本榜用的是整段历史里第一次 EMA 空头扩张、永不换线，两张互不包含（上线这一周那张的 264 个里有 50 个也在本榜）。这是一张门槛清单：参照线不动，收盘价在线上方就一直在、跌回线下就离开——回放里上一周的成员这一周还在的比例中位数约 92%，但已经结束的在榜段中位只有 2 周（在参照线附近上下反复的标的会进进出出），最长 97 周。命中数：回放过去 104 周，每周中位约 60 个、最少 23 个、最多 133 个（2024 年 12 月初），没有哪一周是空的，中位约占全市场的 15%；上线这一周（最新已收盘的是 2026 年 9 月 14 日那一周）是 99 个，在这 104 周里偏高。也要如实说一句：它不是择时信号。回放里成员下一周收涨的比例是 41.8%（6401 个样本），全市场 42.2%，下一周涨跌幅中位数 −2.36%（全市场 −2.23%）；4 周后收盘比入榜那周更高的比例 35.1%，全市场 36.5%。按周比，成员涨跌幅的中位数跑赢当周全市场的周数约六成（下一周 60/103、4 周后 68/100），但幅度很小；合在一起算反而略差，是因为成员多的那些周，接下来全市场普遍下跌、成员也跟着跌。它更适合当成「已经站回第一轮下跌后反弹位」的背景名单，配合别的榜或表格里的各轴再筛。周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。范围是全部加密 USDT 永续合约。" },
