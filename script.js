...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "monthlySarBearTwoBull", name: "月线SAR空头＋突破空头首根收盘价", tf: "月线",
              desc: "两个条件都看最新已收盘的那根月线：一是这根月线的 Parabolic SAR 站在空头一侧（圆点在价格上方）；二是它的收盘价，高过了这一轮 SAR 空头段里第一根月线的收盘价。拆开说第二条：在月线上找到当前这一段 SAR 处在空头的区间，它的第一根——也就是由多翻空的那个月——的收盘价就是参照线，再看最新这根月线收在不在它上面。合起来找的是：月线级别还没转向、但价格已经把翻空那个月的收盘价重新收复回来的标的。有五处要先说清楚。第一，第一条已经保证「最近一轮空头」就是正在走的这一段，所以参照线一定取自当前这轮下跌的起点，不是历史上某一段。第二，比较是严格大于，一样高不算；两边都用原始价格、不做四舍五入。第三，参照线在这一段空头里不会变（要等 SAR 翻多、下次再翻空才会换一条），所以成员的进出只看收盘价：回放里这个月的成员下个月还在的比例中位 57%，掉出的约七成是价格跌回参照线下方、约三成是 SAR 翻多整批离开；已结束的在榜段中位只有 1 个月、最长 11 个月。第四，上市初期那种从第一根月 K 直接起算的空头段也算数——月线取 100 根，对现有合约就是全部历史，和 TradingView 上看到的一样。第五，本榜只看月线 SAR 的方向和这一条参照线：不看均线、RSI、成交量、CVD，也不看月 K 的阴阳和连阳根数；门槛是至少 3 根已收盘月 K，上市不足三个月的新合约不入榜。命中数：回放过去 48 个月，每月中位 18 个、最少 1 个、最多 128 个，没有一个月是空的；中位约占当月可判标的的 6%、占当月月线 SAR 在空头那批的 8%。上线这个月（最新已收盘的是 2026 年 8 月那根月线）是 44 个——这个月全市场 519 个可判标的里，469 个月线 SAR 在空头侧，其中收盘价站回翻空那根之上的只有 44 个。也要如实说一句：它不是买入信号，回放里反而略跑输大盘。成员下个月收盘更高的比例 35.5%（1122 个样本），当月可判全体 37.4%；下个月涨跌幅的中位数 −8.64%，全体 −7.24%；3 个月后收盘更高 30.6%（全体 31.2%），中位 −21.84%（全体 −17.38%）。按月比倒是有 54.8% 的月份成员跑赢当月中位——两个口径打架，说明这点差距不稳定，别当成结论。它更适合当成「月线还在下跌趋势里、但价格已经收复了翻空那一根」的观察名单。和站内其他榜的关系：和「月线SAR多头首根＋收盘价反包所有空头SAR」一定不会同时出现（那张要求月线 SAR 刚刚翻多，本榜要求还在空头）。「月线突破SAR空头第二根低点」比的是同一段空头里第 2 根的最低价、而且完全不看 SAR 方向，参照线通常比本榜这条低，所以本榜的标的回放 48 个月里（1166 人次）没有一个掉在那张之外——但这不是判据保证的包含：如果那一段空头第 2 根的最低价反而高过首根的收盘价，两者就会脱钩。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
        ],
    },
