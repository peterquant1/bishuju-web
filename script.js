...
const TABS_CONFIG = {
    ...
    // === 加密策略：七个榜（2026-07-25 晚站长先「移除加密所有TAB」清空 → 逐条指令加回五个 →
    ...
    // ⚠️⚠️ **七个榜里唯一的"事件"型 SAR 榜，也是全站唯一的"恰好第 N 根"型榜**：①④ 的
    // SAR 多头是**状态**（会连着很多天/周为真、成员跨期稳定），本榜是**一次性事件**——
    // 每个标的一辈子只在翻多后的那一天命中 ⇒ **成员每天几乎全换、命中数天然很小**。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        // `dailySarSecondBar` ⇒ 七个**；被清掉那些的显示名/desc/专属轴见 git 历史）。
        // ⚠️ **刻意不套 singleStrategyGroup 工厂**：那个工厂的 name/desc 写死的是股票系
        // 的 5 个条件（月线SAR + 周线SAR + 周线扩张 + 日线扩张 + CVD），与这七个榜都不
        // 一样。硬套会让页面上的规则说明与后端实际筛选严重不符。
        // 组 tf 取第一个 tab 的周期（月线），其余六个 tab 各自覆盖。**排法：同周期的放一起**
        // （月×周×日 → 日线两个 → 周线三个 → ⑦ 日线SAR第二根）——两个日线榜相邻是刻意的：
        // 判据只差一处（9/21/55 三线扩张 vs 四线排列且启动过），挨着才看得出对照。
        // ⚠️ **⑦ 追加在末尾、没有挪到前面那两个日线榜旁边**：新榜一律追加是房规（⑤⑥ 也是
        // 这么加的），插到中间会让 ①-⑦ 整套编号位移，而后端 docstring / CLAUDE.md / 本文件
        // 多处按编号互相引用。⑦ 与前面的日线榜也不是同一类东西（那两个看 EMA 结构、是状态；
        // ⑦ 看 SAR 翻多、是一次性事件），并排反而容易被当成同族。
