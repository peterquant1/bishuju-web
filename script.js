...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     · **`dailySarFirstBar`（日线 SAR 多头首根，第 9 条新增；不带 EMA 门、相邻两日交集恒空）**
        ...
        //   · **包含：六对**（九对 → 09-09 15:3x 三对 → 09-10 新增 `weeklyEmaSarFirstTwo`
        ...
        //       「恒为正」仍只有 `dailyEmaSarFirstTwo`，**别互抄**。两张日线榜两个方向都不包含，不计入包含 / 互斥对数。
        ...
        tabs: [
            ...
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前两根＋成交额递增", tf: "日线",
              desc: "三个条件都看最新已收盘的那根日线：① Parabolic SAR 站在多头一侧，而且这一根是这一轮多头的头两根之一——要么是刚刚由空翻多的第一根（前一天还在空头一侧），要么是紧接着站住的第二根；② EMA9 在 EMA21 上方，并且两条均线的间距比前一天更大，也就是均线结构同一天也在张开；③ 这一天的成交额比前一天大。只看 SAR 翻多，每天都会冒出一批标的，其中不少只是下跌途中的一次反抽；再要求均线正在张开，留下的是方向刚转过来、结构也已经跟上的那一小撮；第三条再把其中价格翻上来了、成交额却比前一天还小的筛掉。站内另有一张「日线SAR多头首根」，只看 SAR 刚翻多这一个条件、不看均线和成交额；本榜里翻多当天的那一档，必然也都在那张榜上。成交额按 USDT 计，就是表格里 日成交额 那一列的数，比的只是这一天和前一天，不是和更早的均量比，也不是按币的个数算的成交量。本榜把「翻多当天」和「次日站住」这两档放在了一起：想抢第一时间的看第一根，想让信号多被确认一天、滤掉翻多当天就掉头的假信号的看第二根——表格里的 日SAR多头根数 那一列就是分这两档用的，它在本榜只会是 1 或 2。要留意这是一张事件清单而不是状态清单：同一个标的最多连着两天出现在这里，第三天起就不在了，所以成员换得很快、命中数天然很小；加上成交额这一条之后更少，回放过去 90 天，每天命中的中位数在 6 个上下，其中有 3 天一个都没有——遇到空榜是正常的。另外表格里的 日EMA间距 一定是正数（判据要求均线正在张开），这是判据决定的、不是数据出错。范围是全部加密 USDT 永续合约。" },
            ...
            // ⚠️⚠️ 1 个条件、全在日线：倒1 SAR 多头 且 倒2 空头（后端新旗标 `sarFirstBar`）；**不带任何 EMA / 成交额条件**。tf 日线 ⇒ 日线榜两个。
            // ⚠️⚠️ 与「日线9/21扩张＋SAR多头前两根＋成交额递增」两个方向都不包含（那张的首根那批 ⊆ 本榜、第二根那批 ∩ 本榜 ＝ ∅）⇒ 不计入包含 / 互斥对数；
            //   那张的 desc 追加了一句指向本榜。自检点：日SAR多头根数恒为 1；日EMA间距可正可负（别照那张抄「恒为正」）。
            // ⚠️ name 照「月线SAR多头首根」建榜时的写法；1 个条件 ⇒ 无 ＋。别为 SAR 加轴；18 轴照旧。
            // 📊 360 天回放：中位 14 / p25 7 / p75 24 / max 100 / 均值 18.6 / 2 天为 0（90 / 180 天 min 2、0 天为 0）；上线当天 18（过 9/21 扩张门的 9）；
            //    相邻两日交集 359 对全空；次日仍多头 96.1%；这一轮多头最终持续中位 9 根（≤3 根 13.8%、≥10 根 45.6%）；起点前移 100 根重算判定差 0.26%
            { key: "dailySarFirstBar", name: "日线SAR多头首根", tf: "日线",
              desc: "只看最新已收盘的那根日线：Parabolic SAR 正好在这一根由空翻多——SAR 圆点从价格上方翻到了下方，而前一天还停在空头一侧。注意是恰好第一根：已经翻多两天、三天的标的不在这张榜上，表格里的 日SAR多头根数 在本榜只会是 1。本榜只看 SAR 这一个条件，完全不看均线、成交额或资金流，所以比「日线9/21扩张＋SAR多头前两根＋成交额递增」宽得多：那张榜里翻多当天的那一档都能在本榜上找到，而本榜里多出来的，是 SAR 翻多了、但均线还没在张开或者成交额没比前一天大的那些——其中不少只是下跌途中的一次反抽，需要自己分辨；也因此表格里的 日EMA间距 可以是负数（EMA9 还在 EMA21 下方），这是判据决定的、不是数据出错。代价要说清楚：日线 SAR 是很敏感的信号，回放过去一年，翻多之后这一轮多头维持的天数中位只有 9 天左右，大约七分之一在三天之内就重新翻回空头，能撑过十天的不到一半。这是一次性的事件清单不是持续清单——同一个标的只在翻多的那一天出现，第二天它要么成了第二根、要么已经翻回空头，都会离开本榜，所以相邻两天的名单不会有重叠（回放里翻多次日仍保持多头的超过九成五，多数标的第二天是因为成了第二根而离开）。命中数随行情起伏：回放过去一年，每天中位 14 个左右，全市场一起反弹的日子能到一百个，只有两天一个都没有。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。除了恒为 1 的 日SAR多头根数，表格里的其余各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
        ],
    },
