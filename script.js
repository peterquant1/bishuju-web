...
const TABS_CONFIG = {
    ...
    // **轴集只有一族**：全部共用 **18 轴的 cryptoStrategySorts**，行全部走后端唯一的
    // `_strategy_row`（2026-07-30 站长「能否对齐轴数？以最多的为准」；对齐前是两族：日线族
    // 11 轴、周线族 13 轴的 cryptoWeeklyExpansionSorts。18 = 2026-09-04 加「日SAR多头根数」后）。
    ...
    //   `weeklyEmaSarFirstTwo` 严格包含**（那个榜 ＝ 本榜 ∩ SAR前两根）。
    ...
    // 行 payload 走后端唯一的 `_strategy_row` ⇒ **18 轴的 cryptoStrategySorts**（全族共用；
    ...
    //   也别因为有 EMA 就加 emaGap 轴 —— 它早就在那 18 根里。
    ...
    // ⚠️ 别为"连阳/连阴"加轴（布尔值排不了序）；18 轴照旧、与本族其余各榜共用同一个 sorts 对象。
    monthlyFourBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    monthlyFiveBear: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    // ⭕ 2026-09-09 复活：周线 EMA9/21 扩张（**单条件** —— 站长「即可」＝只有这一个门）。
    // 18 轴照旧、与本族其余各榜共用同一个 sorts 对象；**别为「扩张」加 emaGap 轴**（它早就在那 18 根里）。
    ...
    // 18 轴照旧、与本族其余各榜共用同一个 sorts 对象；别为 SAR 加轴（布尔值排不了序），
    // 也别因为有 EMA 就加 emaGap 轴（「周线EMA间距」早就在那 18 根里）。
    ...
    // ⚠️ `weeklyFourBull` 复用 2026-06-17 建、07-08 移除（付费墙之前）的同名旧 key，判据逐字相同（后端 `fourBull`
    //   转正）；另两个是新 key、后端新增字段 ⇒ WEEKLY_CACHE_SCHEMA 19→20。
    // ⚠️ 别为"连阳/连阴"加轴（布尔值排不了序）；18 轴照旧、与本族其余各榜共用同一个 sorts 对象。
    weeklySixBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    weeklyFourBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    weeklyFiveBear: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },

    // ➕ 2026-09-10（当日第 6 条）站长「新增一个TAB，逻辑是：月线恰好2连阳。」（没点资产 ⇒ 加密）＝ `monthlyFourBull` 的「恰好 2 根」版，
    //   纯 K 线形态、零指标：倒1 阳、倒2 阳 **且倒3 非阳**（门槛 ≥3 根已收盘月 K）；后端新增字段 `twoBullExact` ⇒ MONTHLY_CACHE_SCHEMA 14→15。
    // ⚠️⚠️ 与 `monthlyFourBull`（倒3 阳 vs 非阳）、`monthlyFiveBear`（倒1 阳 vs 阴）**结构性互斥** ⇒ 站内互斥 live 对由四对变【六对】。
    //   （⚠️ 同日第 7 条新增 `monthlyBullAfterThreeBear` 后与它也互斥：倒2 阳 vs 阴；它的成员下个月再收阳就正好落进本榜。）
    //   （⚠️ 同日第 8 条新增 `monthlyTwoBullExactVolRising` ＝ 本榜 ∩ 月成交量递增 ⇒ 它 ⊊ 本榜；两张共用后端 `twoBullExact` 旗标。）
    // ⚠️ key 带 `Exact`：照已移除的 `weeklyTwoBullExact` 先例（周线端「≥2」`twoBull` 与「恰好 2」`twoBullExact` 只差一个后缀）。
    // ⚠️ 别为「连阳」加轴（布尔值排不了序）；18 轴照旧、与本族其余各榜共用同一个 sorts 对象。
    monthlyTwoBullExact: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },

    // ➕ 2026-09-10（当日第 7 条）站长「新增一个TAB，逻辑是：月线恰好3连阴之后出现阳线。」（先回复口径、站长确认后开工；没点资产 ⇒ 加密）。
    //   纯 K 线形态、零指标：倒1 阳、倒2..倒4 全阴、倒5 **非**阴（门槛 ≥5 根已收盘月 K）；后端新增字段 `bullAfterThreeBear` ⇒ MONTHLY_CACHE_SCHEMA 15→16。
    // ⚠️⚠️ 与 `monthlyTwoBullExact` / `monthlyFourBull`（倒2 阳 vs 阴）、`monthlyFiveBear`（倒1 阳 vs 阴）**结构性互斥** ⇒ 站内互斥 live 对由六对变【九对】。
    // ⚠️ 本榜成员下个月若再收阳 ⇒ 正好落进 `monthlyTwoBullExact`（接力，不是包含）。别为阴阳形态加轴；18 轴照旧。
    monthlyBullAfterThreeBear: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },

    // ➕ 2026-09-10（当日第 8 条）站长「新增一个TAB，逻辑是：月线恰好2连阳且成交量递增。」（没点资产 ⇒ 加密）＝ `monthlyTwoBullExact` ∩ 月成交量递增。
    //   ① 后端旗标 `twoBullExact`（与月线两连阳同一个）② 后端新增旗标 `volRising`（最新已收盘月 K 的 k[5] 成交量 > 上一根）⇒ MONTHLY_CACHE_SCHEMA 16→17。
    // ⚠️⚠️ **⊊ `monthlyTwoBullExact`**（包含由五对变【六对】）；互斥全部继承自那张（∩ 四连阳 / 五连阴 / 三连阴后首阳）⇒【十二对】。
    // ⚠️ 「成交量」是月 K 的 k[5]；表格里的成交额是日线 k[7] —— 两者不是一回事，别拿表格去核对「递增」。18 轴照旧。
    monthlyTwoBullExactVolRising: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    // ⚠️ 事件型：相邻两日成员交集恒空。别为 SAR 加轴（布尔值排不了序）；18 轴照旧、与本族共用同一个 sorts 对象。
    ...
    // ➕ 2026-09-11 站长「新增一个TAB，逻辑是日线4连阳。」（没点资产 ⇒ 加密）＝ `monthlyFourBull` /
    //   `weeklyFourBull` 的日线版，纯 K 线形态、零指标；恰好 4 连阳（同站内 N 连阳/连阴榜口径）。
    // ⚠️ key `dailyFourBull` 复用 2026-07-22 移除的同名旧 tab，但判据不同（旧＝9/21扩张∩4连阳，本＝纯 4 连阳）；
    //   上线前实测公开顶层/paidMeta/付费镜像三处均无残留，复用安全。别为"连阳"加轴（布尔值排不了序）；18 轴照旧、共用同一个 sorts。
    dailyFourBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    // ➕ 2026-09-11 站长「新增一个TAB，逻辑是日线2连阳。」（没点资产 ⇒ 加密）＝ `monthlyTwoBullExact` /
    //   已移除的 `weeklyTwoBullExact` 的日线版，纯 K 线形态、零指标；恰好 2 连阳（倒1阳、倒2阳、倒3非阳）。
    // ⚠️ key 带 `Exact` 照月/周线两连阳先例；全新 key、站内从无、三处均无残留。**与 `dailyFourBull` 结构性互斥**（倒3 非阳 vs 阳）。
    //   别为"连阳"加轴（布尔值排不了序）；18 轴照旧、共用同一个 sorts。
    dailyTwoBullExact: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //
        // 【现状分类】
        //   · 事件型（成员换得快）：`dailyEmaSarFirstTwo`（日线 SAR 翻多头两根＋9/21 扩张
        //     ＋成交额递增〔09-10 追加〕，最多连着两天）· **`weeklyEmaSarFirstTwo`（周线 SAR
        //     翻多头两根＋9/21 扩张，最多连着两周）**
        //     · `monthlySarFirstBar`（月线 SAR 翻多首根**＋收盘价反包所有空头SAR**〔09-10 12:1x 追加〕，相邻两月交集恒空）
        //     · `monthlyFourBull` / `monthlyFiveBear`（月线恰好 4 连阳 / 5 连阴）· **`monthlyTwoBullExact`（月线恰好 2 连阳，09-10 当日第 6 条新增）**
        //     · **`monthlyBullAfterThreeBear`（月线恰好 3 连阴后首阳，第 7 条新增；相邻两月交集恒空）**
        //     · **`monthlyTwoBullExactVolRising`（月线两连阳＋成交量递增，第 8 条新增；⊊ 两连阳、相邻两月交集恒空）**
        //     · **`dailySarFirstBar`（日线 SAR 多头前两根：09-10 第 9 条新增时是首根、09-11 改成前两根；不带 EMA 门、最多连着两天）**
        //     · **`weeklySixBull` / `weeklyFourBull` / `weeklyFiveBear`（周线恰好 6 连阳 / 4 连阳 / 5 连阴，
        //       09-10 12:1x 新增；相邻两周交集恒空）**
        //     · **`dailyFourBull` / `dailyTwoBullExact`（日线恰好 4 连阳 / 恰好 2 连阳，09-11 新增；相邻两日交集恒空、两者互斥）**
        ...
        //   · 纯 K 线形态、零指标：`monthlyFourBull` / `monthlyFiveBear` + **`weeklySixBull` / `weeklyFourBull` /
        //     `weeklyFiveBear`（09-10 12:1x 起共五个）** + **`monthlyTwoBullExact`（当日第 6 条起共六个）** + **`monthlyBullAfterThreeBear`（第 7 条起共七个）**
        //     + **`dailyFourBull` / `dailyTwoBullExact`（09-11 起共九个）**（`weeklyTwoBullExact` 2026-09-09 移除后一度只剩月线那两个）
        //
        // 【结构性关系 —— 2026-09-11 起】
        //   · **包含：五对**（九对 → 09-09 15:3x 三对 → 09-10 新增 `weeklyEmaSarFirstTwo`
        //     一次带来三对，回到六对 → 13:3x 移除 `weeklyTripleEma` 带走一对 ⇒ 五对 → 第 8 条新增 `monthlyTwoBullExactVolRising` 带来一对 ⇒ 六对
        ...
        //       `weeklyEmaSarFirstTwo`  ⊊ `weeklyEmaSarBull`        （＝ 后者 ∩ SAR前两根）
        //       `weeklyEmaSarFirstTwo`  ⊊ `weeklyEma921Expansion`   （＝ 条件① 本身）
        //       `monthlyTwoBullExactVolRising` ⊊ `monthlyTwoBullExact`（＝ 后者 ∩ 月成交量递增）
        //       `dailyEmaSarFirstTwo`  ⊊ `dailySarFirstBar`         （＝ 后者 ∩ 9/21扩张 ∩ 成交额递增；09-11 后者改成前两根后才成立）
        //     ✏️ **作废的两对**（09-11）：`weeklyEmaSarBull` ⊊ `weeklySarUptrend`、`weeklyEmaSarFirstTwo` ⊊ `weeklySarUptrend`
        ...
        //   · **互斥：十三对**（三对 → 一对 → 09-10 12:1x 新增三个周线连阳/连阴榜后四对 → 当日第 6 条新增 `monthlyTwoBullExact` 后六对 → 第 7 条新增 `monthlyBullAfterThreeBear` 后九对 → 第 8 条新增 `monthlyTwoBullExactVolRising` 后十二对（全部继承自两连阳）→ **09-11 新增 `dailyTwoBullExact` 与 `dailyFourBull` 互斥后十三对**），全是判据保证：
        //       `dailyTwoBullExact` ∩ `dailyFourBull`（倒3 一个要非阳一个要阳；⚠️ 09-11 加这两个日线榜那两轮漏改了本段，本次补上）
        //       `monthlyFourBull` ∩ `monthlyFiveBear`（倒1 一个要阳一个要阴）
        //       `monthlyTwoBullExact` ∩ `monthlyFourBull`（倒3 一个要非阳一个要阳）· `monthlyTwoBullExact` ∩ `monthlyFiveBear`（倒1 阳 vs 阴）
        //       `monthlyBullAfterThreeBear` ∩ `monthlyTwoBullExact` · ∩ `monthlyFourBull`（倒2 阴 vs 阳）· `monthlyBullAfterThreeBear` ∩ `monthlyFiveBear`（倒1 阳 vs 阴）
        //       `monthlyTwoBullExactVolRising` ∩ `monthlyFourBull` / `monthlyFiveBear` / `monthlyBullAfterThreeBear`（继承自两连阳）
        //       `weeklySixBull`   ∩ `weeklyFourBull` （倒5 一个要阳一个要非阳）
        //       `weeklySixBull`   ∩ `weeklyFiveBear` · `weeklyFourBull` ∩ `weeklyFiveBear`（倒1 阳 vs 阴）
        //   · **`weeklyEmaGap` 恒为正的周线榜三个**（09-10 13:3x 移除 `weeklyTripleEma` 前是四个）：
        //     `weeklyEmaSarBull` / `weeklyEma921Expansion` / **`weeklyEmaSarFirstTwo`**（都蕴含 9/21 扩张）；
        //     `weeklySarUptrend` **不适用**（它完全不看 EMA、可正可负）——**别互抄**。
        //   · **`emaGap` 恒为正的日线榜只剩一个**：`dailyEmaSarFirstTwo`（也是唯一的日线榜）。
        //     ⚠️ 09-10 新增的那个是**周线**榜，不影响这一条。
        //     ⚠️ **当日第 9 条新增 `dailySarFirstBar` 后它不再是唯一的日线榜**；那张不看 EMA、emaGap 可正可负 ——
        //       「恒为正」仍只有 `dailyEmaSarFirstTwo`，**别互抄**。两张日线榜第 9 条时两个方向都不包含，**09-11 那张改成前两根后
        //       `dailyEmaSarFirstTwo` ⊊ `dailySarFirstBar`**（已计入上面的包含五对）。
        //     ⚠️ 09-11 新增的 `dailyFourBull` / `dailyTwoBullExact` 也不看 EMA ⇒ 同样**不适用**「恒为正」。
        //   · **周线榜七个**（09-10 12:1x 由五个变八个，13:3x 移除 `weeklyTripleEma` 后七个）/ **月线榜六个**（当日第 6 条加 `monthlyTwoBullExact`、第 7 条加 `monthlyBullAfterThreeBear`、第 8 条加 `monthlyTwoBullExactVolRising`）/ **日线榜四个**（第 9 条加 `dailySarFirstBar`，09-11 加 `dailyFourBull` / `dailyTwoBullExact`）。
        //
        ...
        // **各榜轴集已于 2026-07-30 对齐成同一套**（2026-09-04 加「日SAR多头根数」后为 18 轴）。
        label: "加密策略", asset: "加密", tf: "月线",
        tabs: [
            // ⚠️⚠️ **本组第一条 ＝ 站长 2026-09-09 明确要求的「置顶」**（逐字：「…并将该TAB
            //   置顶。」）—— 它由 ④`dailyEmaSarFlip`「首根」＋ ⑪`dailyEmaSarSecond`「第二根」
            //   **合并**而成，两个旧 key 一并退役、加密策略榜 15 → 14、全站 22 → 21。
            //   **rail 顺序就由本数组的顺序决定**，要再调位置改这里即可（后端 build_rankings
            //   里那个 dict 的顺序按建榜时间排、与 rail 无关，别去那边找）。
            // tf 日线：两个条件都在日线。判据 ＝ `ema921_expansion_data` ∩
            //   （`daily_sar_flip_data` ∪ `daily_lookup` 的 `sarSecondBar` 旗标）——
            //   **两个判据源都留着**，别化简成 `sarBullBars in (1,2)`（理由见后端注释）。
            // ⚠️⚠️ 连带作废：④∩⑪ 那对结构性互斥**随本次合并消失** ⇒ 站内结构性互斥 live 对
            //   由四对变【三对】（⑦∩⑧ · ⑫∩⑧ · ⑬∩⑭，且这三对的榜号本轮整体位移了 ⇒ 去查 key
            //   别信号）；「首根与第二根互斥」降级成本榜**内部两个分支**之间 ⇒ 两分支不重复
            //   计数，命中 ＝ 首根 ＋ 第二根（上线当天 10 + 22 = 32）。
            // ⚠️ 与合并前 ④ 的**行为差别**：④ 相邻两日成员交集恒为空，**本榜不再恒空**
            //   （今天的首根明天成第二根、仍在榜上，最多连着两天）。别照旧注释读。
            // ⚠️ emaGap 恒正的日线榜由四个变【三个】（本榜 / dailyTripleEmaCvd /
            //   dailyEmaSarBearish）；每行 sarBullBars 恒 ∈ {1,2}（跨实现对账）。
            //   别为 SAR 加轴（布尔值排不了序、后端没发圆点价 ⇒ 幽灵轴）。
            // ➕ 2026-09-10 站长追加条件③「且成交额递增」⇒ 3 个条件、name 同步加「＋成交额递增」
            //   （key 不动）。成交额按 USDT 计（后端 `turnoverRising`，k[7]，就是「日成交额」那一列）。
            //   ⚠️ 追加后与 `weeklyEmaSarFirstTwo` 不再同判据；橱窗回放 0/90 → 3/90 天为空。
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前两根＋成交额递增", tf: "日线",
              desc: "三个条件都看最新已收盘的那根日线：① Parabolic SAR 站在多头一侧，而且这一根是这一轮多头的头两根之一——要么是刚刚由空翻多的第一根（前一天还在空头一侧），要么是紧接着站住的第二根；② EMA9 在 EMA21 上方，并且两条均线的间距比前一天更大，也就是均线结构同一天也在张开；③ 这一天的成交额比前一天大。只看 SAR 翻多，每天都会冒出一批标的，其中不少只是下跌途中的一次反抽；再要求均线正在张开，留下的是方向刚转过来、结构也已经跟上的那一小撮；第三条再把其中价格翻上来了、成交额却比前一天还小的筛掉。站内另有一张「日线SAR多头前两根」，只看 SAR 多头的头两根这一个条件、不看均线和成交额；本榜的每一个标的都必然在那张榜上，本榜就是它再叠上均线张开、成交额放大两道条件之后留下的那一小撮。成交额按 USDT 计，就是表格里 日成交额 那一列的数，比的只是这一天和前一天，不是和更早的均量比，也不是按币的个数算的成交量。本榜把「翻多当天」和「次日站住」这两档放在了一起：想抢第一时间的看第一根，想让信号多被确认一天、滤掉翻多当天就掉头的假信号的看第二根——表格里的 日SAR多头根数 那一列就是分这两档用的，它在本榜只会是 1 或 2。要留意这是一张事件清单而不是状态清单：同一个标的最多连着两天出现在这里，第三天起就不在了，所以成员换得很快、命中数天然很小；加上成交额这一条之后更少，回放过去 90 天，每天命中的中位数在 6 个上下，其中有 3 天一个都没有——遇到空榜是正常的。另外表格里的 日EMA间距 一定是正数（判据要求均线正在张开），这是判据决定的、不是数据出错。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "monthlyFourBull", name: "月线四连阳", tf: "月线",
              desc: "最新已收盘的那根月线，正好是连续第 4 根阳线——这个月收阳，往前数第 2、3、4 个月也都收阳，而第 5 个月不是阳线。注意是恰好 4 根，不是至少 4 根：已经连涨 5 个月、6 个月的标的不在这张榜上，它们的最新一根是第 5 根、第 6 根而不是第 4 根。本榜是全站少数几张纯看 K 线形态的榜之一，完全不看均线、SAR、资金流或任何指标——只问一件事：按月计的上涨，是不是刚好走到第四个月。月线是全站最大的周期，连续四个月收阳意味着这轮上涨已经跨过了「反弹」的量级、按月计的结构确实转了过来，而卡在第 4 根这个位置，找的是趋势已经立住、但还没走到人尽皆知的那一段。代价也要说清楚：月线级别的形态确认得慢，等到第四根阳线收出来，价格往往已经离底部很远了。这是一次性的事件清单不是持续清单——同一个标的只在第 4 根那个月出现，下个月它要么成了第 5 根、要么连阳已经断掉，两种情况都会让它离开本榜，所以相邻两个月的名单不会有重叠。命中数天然很少，遇到空榜是正常的：回放过去四年，多数月份只有个位数，全市场一起普涨的月份能到几十个，而大约每六个月就有一个月一个都没有——上线当天正好是这样的月份，全市场连阳的标的要么只走到第 3 根、要么已经超过 4 根，恰好卡在第 4 根的一个也没有。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 5 个月的新合约看不出「第 5 个月不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            { key: "monthlyFiveBear", name: "月线五连阴", tf: "月线",
              desc: "最新已收盘的那根月线，正好是连续第 5 根阴线——这个月收阴，往前数第 2 到第 5 个月也都收阴，而第 6 个月不是阴线。同样是恰好 5 根不是至少 5 根：已经连跌 6 个月、8 个月的标的不在这张榜上。本榜和站内大多数榜的方向是反的，值得单独说明：其他榜大多在找刚启动、还在延续的上涨（周线上与它同方向的是「周线五连阴」那张，本榜是它的月线版）；本榜一个多头条件都没有，连续五个月下跌就是连续五个月下跌。所以它有三种截然不同的读法，取决于你自己的判断：可以当成超跌名单，找跌了快半年、可能酝酿反转的标的；也可以当成空头趋势确认，按月计的下跌结构完好；还可以当成回避名单，直接排除掉这些标的。本站不替你选其中哪一种——榜只负责把「按月计已经连跌五个月」这件事挑出来。卡在第 5 根这个位置的用意是：跌一两个月是波动，跌满五个月说明这是趋势而不是噪声，而再往后拖的标的多半已经进入长期阴跌、和「刚跌透」是两回事。这同样是一次性的事件清单——同一个标的只在第 5 根那个月出现，下个月它要么成了第 6 根、要么已经收阳，所以相邻两个月的名单不会有重叠。它和「月线四连阳」那张榜永远不会出现同一个标的，因为同一根月线不可能既收阳又收阴。命中数比四连阳那张稳一些：回放过去四年每个月都有，多数月份是个位数到十几个，全市场一起大跌的月份能到近百个。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱——想找超跌反转候选的，可以看日线那几根轴有没有先转向。上市不足 6 个月的新合约看不出「第 6 个月不是阴线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "weeklyEma921Expansion", name: "周线9/21扩张", tf: "周线",
              desc: "只看最新已收盘的那根周线一件事：EMA9 在 EMA21 上方，而且两条均线之间的间距比上一周更大——短周期均线正在加速甩开中周期均线，结构在张开。要注意「扩张」是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。这是站内周线组里条件最少的一张，也是那一组里两张榜的母集：「周线9/21扩张＋SAR多头」「周线9/21扩张＋SAR多头前两根」这两张榜的成员必然全部出现在本榜里，它们都是在「结构在张开」这个池子上再叠一道门——前一张再要求周线 SAR 站在多头一侧，后一张更严，要求 SAR 恰好处在刚翻多的头两根之内。所以本榜适合当那两张的上游来用：先在这里看清楚哪些标的的周线结构正在张开，再决定要不要用更严的那两张往下收窄。它是一张状态清单不是事件清单：只要 EMA9 还在 EMA21 上方而且还在继续张开，同一个标的可以连着好几周都留在榜上，回放过去 60 周大约一半成员会跨周延续，每周命中数中位在 21 上下、最少 6 个、最多 67 个，还没有出现过空榜；上线当天的 67 个恰好就是这 60 周里最多的一周，所以别把它当成常态。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。需要至少 22 根已收盘周 K（约 5 个月）才算得出上一周的间距，所以上市不足半年的新合约不入榜。表格里的日线各轴不参与筛选，用来在这批标的里再看日线强弱；本榜的周线EMA间距 恒为正——这是判据决定的，按它降序能看出结构已经张开到什么程度，升序则是刚刚张开的那一批。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "weeklyEmaSarFirstTwo", name: "周线9/21扩张＋SAR多头前两根", tf: "周线",
              desc: "两个条件都在最新已收盘的那根周线上：一是 EMA9 在 EMA21 上方、而且两条均线的间距比上一周更大（结构正在张开），二是周线的 SAR 恰好处在这一轮多头的头两根之内——要么这根周线就是刚刚由空翻多的第一根，要么是紧接着的第二根。两个条件合起来找的是同一件事的最早期形态：均线结构已经开始张开，而按周计的趋势方向也刚刚站到多头这一侧，两者几乎同时发生。有两处容易读错，先说清楚。第一，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。第二，前两根是恰好不是至少：已经翻多三周、五周的标的不在这张榜上，它们早就过了这个窗口。日线那张「日线9/21扩张＋SAR多头前两根＋成交额递增」的前两个条件和本榜一样、只是放在日线上，另外还多要求当天成交额比前一天大（本榜不看成交额）。两张榜之间没有包含关系，同时出现在两边的标的很少见——日线刚翻多和周线刚翻多是两回事，真撞在一起时反而值得多看一眼。本榜是站内最严的一张周线榜：它的成员必然同时出现在「周线9/21扩张＋SAR多头」「周线9/21扩张」这两张榜里，因为那两张各自只要求本榜条件的一部分。所以反过来用也成立——先看那两张里的哪些标的，最近两周才刚刚把趋势和结构凑齐。另有一张「周线SAR多头＋RSI≥50」，它和本榜都要求周线 SAR 多头，但它还要求周线 RSI 不低于 50、本榜不看 RSI，所以本榜的成员不一定在那张榜上。这是一张事件清单不是状态清单：这周的第一根，下周会变成第二根、仍然留在榜上，第三周就必然掉出去，所以同一个标的最多连着出现两周。命中数天然很少，遇到空榜是正常的——两个条件本就要求结构和趋势在很窄的时间窗里同时成立。上线当天命中 7 个，全部来自「第一根」那一支；当天全市场有 20 个标的的周线 SAR 正好走到第二根，但它们的 EMA9/21 一个都没在张开，所以都没进来，这正是两个条件各自在起作用的样子。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。门槛方面：算上一周的间距需要至少 22 根已收盘周 K（约 5 个月），所以上市不足半年的新合约不入榜；判断第二根还要再多一根周 K，所以刚好满 22 周的标的只可能从第一根那一支进来。表格里的各轴都不参与筛选，用来在这批标的里再分强弱：本榜的周线EMA间距 恒为正，这是判据决定的，按它升序能挑出结构刚刚张开的那一批。要提醒一句，日SAR多头根数 那根轴量的是日线不是周线，在本榜上它可以是任意值、也可能显示「—」，别把它当成本榜的筛选条件来读。日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-10 12:1x UTC 站长同一条消息连发三条（没点资产 ⇒ 加密）：「新增一个TAB，逻辑是：周线6连阳。」
            //   「新增一个TAB，逻辑是：周线4连阳。」「新增一个TAB,逻辑是：周线5连阴。」
            //   ＝ `monthlyFourBull` / `monthlyFiveBear` 的**周线版**，纯 K 线形态、零指标；按站长给的顺序追加在末尾（⑩⑪⑫）。
            // ⚠️⚠️ **恰好不是至少**（站长没写"恰好"，按站内全部 N 连阳/连阴榜的既有口径）；门槛各比根数多一根。
            // ⚠️⚠️ 三榜两两结构性互斥 ⇒ 站内互斥 live 对由一对变【四对】；事件型、相邻两周交集恒空、周内恒定。
            // ⚠️ `weeklyFourBull` 复用 2026-06-17 建、07-08 移除（付费墙之前）的同名旧 key，判据逐字相同。
            // ⚠️ 显示名按 09-06 月线那两个的先例用汉字数字（站长写的是 6 / 4 / 5）；一个条件 ⇒ 名字里无 ＋。
            // 📊 60 周回放：六连阳 中位 0 / max 24 / **32 周为 0**；四连阳 中位 3 / max 87 / 6 周为 0；
            //   五连阴 中位 4 / max 141 / 11 周为 0 —— 六连阳空榜是常态（上线当天就是 0），desc 里写给用户了。
            { key: "weeklySixBull", name: "周线六连阳", tf: "周线",
              desc: "最新已收盘的那根周线，正好是连续第 6 根阳线——这一周收阳，往前数第 2 到第 6 周也都收阳，而第 7 周不是阳线。注意是恰好 6 根，不是至少 6 根：已经连涨 7 周、8 周的标的不在这张榜上，它们的最新一根是第 7 根、第 8 根而不是第 6 根。本榜和「周线四连阳」「周线五连阴」是一组，都只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按周计的上涨，是不是刚好走到第六周。连涨六周在周线上已经很少见，意味着这轮上涨持续了一个半月、中间没有一周收阴，趋势的连贯性很强；代价也要说清楚：等第六根阳线收出来，价格往往已经离这轮起点很远了。这是一次性的事件清单不是持续清单——同一个标的只在第 6 根那一周出现，下一周它要么成了第 7 根、要么连阳已经断掉，两种情况都会让它离开本榜，所以相邻两周的名单不会有重叠；它和「周线四连阳」也永远不会出现同一个标的——四连阳要求往前第 5 周不是阳线，而六连阳要求那一周也是阳线。命中数天然很少，空榜是常态：回放过去 60 周，超过一半的周份一个都没有，全市场一起普涨的周份能到二十几个。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 7 周的新合约看不出「第 7 周不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            { key: "weeklyFourBull", name: "周线四连阳", tf: "周线",
              desc: "最新已收盘的那根周线，正好是连续第 4 根阳线——这一周收阳，往前数第 2、3、4 周也都收阳，而第 5 周不是阳线。注意是恰好 4 根，不是至少 4 根：已经连涨 5 周、6 周的标的不在这张榜上，它们的最新一根是第 5 根、第 6 根而不是第 4 根。本榜是「月线四连阳」的周线版，同样只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按周计的上涨，是不是刚好走到第四周。连续四周收阳说明这轮上涨已经持续了一个月、中途没有一周收阴，卡在第 4 根这个位置，找的是连涨已经成形、但还没走得太远的那一段；想看走得更远的，可以对照「周线六连阳」那张。这是一次性的事件清单不是持续清单——同一个标的只在第 4 根那一周出现，下一周它要么成了第 5 根、要么连阳已经断掉，所以相邻两周的名单不会有重叠；它和「周线六连阳」「周线五连阴」也都不会出现同一个标的。命中数不多、遇到空榜也正常：回放过去 60 周，多数周份是个位数，全市场一起普涨的周份能到八十多个，大约每十周有一周一个都没有。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 5 周的新合约看不出「第 5 周不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            { key: "weeklyFiveBear", name: "周线五连阴", tf: "周线",
              desc: "最新已收盘的那根周线，正好是连续第 5 根阴线——这一周收阴，往前数第 2 到第 5 周也都收阴，而第 6 周不是阴线。同样是恰好 5 根不是至少 5 根：已经连跌 6 周、8 周的标的不在这张榜上。本榜是「月线五连阴」的周线版，方向与站内大多数榜相反：它一个多头条件都没有，连续五周下跌就是连续五周下跌。所以它有三种截然不同的读法，取决于你自己的判断：可以当成超跌名单，找跌了一个多月、可能酝酿反弹的标的；也可以当成空头趋势确认，按周计的下跌结构完好；还可以当成回避名单，直接排除掉这些标的。本站不替你选其中哪一种——榜只负责把「按周计已经连跌五周」这件事挑出来。卡在第 5 根的用意是：跌一两周是波动，连跌五周说明这是趋势而不是噪声。这同样是一次性的事件清单——同一个标的只在第 5 根那一周出现，下一周它要么成了第 6 根、要么已经收阳，所以相邻两周的名单不会有重叠；它和「周线四连阳」「周线六连阳」永远不会出现同一个标的，因为同一根周线不可能既收阳又收阴。命中数时多时少：回放过去 60 周，多数周份是个位数，全市场一起大跌的周份能到一百四十多个，大约每五六周有一周一个都没有。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱——想找超跌反弹候选的，可以看日线那几根轴有没有先转向。上市不足 6 周的新合约看不出「第 6 周不是阴线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-10（当日第 6 条）站长「新增一个TAB，逻辑是：月线恰好2连阳。」（没点资产 ⇒ 加密）。追加在末尾成为 ⑫。
            // ⚠️⚠️ 站长这次**明写了「恰好」**：倒1 阳、倒2 阳、倒3 **非**阳（含平盘）；门槛 ≥3 根已收盘月 K。
            // ⚠️⚠️ 与「月线四连阳」「月线五连阴」**结构性互斥**（倒3 阳 vs 非阳 / 倒1 阳 vs 阴）⇒ 站内互斥 live 对由四对变【六对】。
            // ⚠️ 事件型：相邻两月交集恒空；月线走缓存 ⇒ 月内成员纹丝不动（desc 里写给用户了）。
            // ⚠️ 显示名按「恰好4连阳 → 月线四连阳」的先例用汉字；key 带 Exact 照已移除的 `weeklyTwoBullExact`。
            // 📊 48 个月回放：中位 7 / max 136（＝上线当天，48 个月里最多）/ 均值 21.3 / 3 个月为 0；相邻两月交集 0、与四连阳 / 五连阴 0/48；次月约六成断、约四成成三连阳（544 : 343）
            { key: "monthlyTwoBullExact", name: "月线两连阳", tf: "月线",
              desc: "最新已收盘的那根月线，正好是连续第 2 根阳线——这个月收阳，上个月也收阳，而再往前一个月不是阳线。注意是恰好 2 根，不是至少 2 根：已经连涨 3 个月、4 个月的标的不在这张榜上，它们的最新一根是第 3 根、第 4 根而不是第 2 根。本榜和「月线四连阳」「月线五连阴」是一组，都只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按月计的上涨，是不是刚好走到第二个月。月线收一根阳线可能只是一次反弹，连着收两根说明这轮上涨已经延续到了第二个月，卡在第 2 根这个位置，找的是按月计的上涨刚刚连起来、还处在最早期的那一批。代价也要说清楚：两连阳是很弱的形态信号，回放过去四年，第二根收出来之后的下一个月，大约六成标的的连阳就断了，只有约四成走成了三连阳。站内另有一张「月线两连阳＋成交量递增」，只保留本榜里第二个月成交量比第一个月大的那一批，成员必然全部落在本榜里；回放显示放量的那一批次月走成三连阳的比例反而略低，放量本身不是更强的信号。它和「月线四连阳」「月线五连阴」永远不会出现同一个标的：四连阳要求往前第 3 个月也是阳线，而本榜要求那个月不是阳线；五连阴要求这个月收阴。这是一次性的事件清单不是持续清单——同一个标的只在第 2 根那个月出现，下个月它要么成了第 3 根、要么连阳已经断掉，两种情况都会让它离开本榜，所以相邻两个月的名单不会有重叠。命中数随大盘起伏很大：回放过去 48 个月，每月中位 7 个，普涨的月份会冲到几十个，只有 3 个月一个都没有；上线当天的 136 个恰好是这 48 个月里最多的一个月，别把它当成常态。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 3 个月的新合约看不出「再往前一个月不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-10（当日第 7 条）站长「新增一个TAB，逻辑是：月线恰好3连阴之后出现阳线。理解？」→ 回复口径、站长确认后开工。追加在末尾成为 ⑬。
            // ⚠️⚠️ 口径：倒1 阳（严格）、倒2..倒4 全阴（严格）、倒5 **非**阴（含阳与平盘）；阳线必须就是紧跟 3 连阴的那个最新已收盘月；门槛 ≥5 根。
            // ⚠️⚠️ 与「月线两连阳」「月线四连阳」（倒2 阳 vs 阴）、「月线五连阴」（倒1 阳 vs 阴）**结构性互斥** ⇒ 站内互斥 live 对由六对变【九对】。
            // ⚠️ 读法是「连跌三个月后第一次收阳」（潜在反转），不归「连续收阴」那一类 —— 别照抄五连阴那两张的三种读法措辞。
            // ⚠️ 事件型：相邻两月交集恒空；月线走缓存 ⇒ 月内成员纹丝不动（desc 里写给用户了）。
            // 📊 48 个月回放：中位 2 / max 115 / 均值 8.5 / 18 个月为 0（上线当天 112，接近最高）；相邻两月交集 0、与两连阳 / 四连阳 / 五连阴 0/48；次月又收阳 81 : 收阴 215（约 1 : 3），又收阳的 81 次全部接力进两连阳
            { key: "monthlyBullAfterThreeBear", name: "月线三连阴后首阳", tf: "月线",
              desc: "最新已收盘的那根月线收阳，而紧挨着它的前三个月都是阴线，再往前一个月又不是阴线——也就是按月计恰好连跌三个月之后，收出的第一根阳线。注意是恰好 3 根阴线：连跌 4 个月、5 个月之后才收阳的标的不在这张榜上；这根阳线也必须紧跟在三根阴线后面、并且就是最新已收盘的那个月，已经连收两个月阳线的标的也不在榜上。本榜和「月线两连阳」「月线四连阳」「月线五连阴」是一组，都只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按月计的下跌，是不是刚好在第三个月之后第一次止住、收出了阳线。它找的是连跌一个季度之后的第一次转阳，属于潜在的反转信号，但代价要说清楚：一根月阳线说明不了趋势已经反转，回放过去四年，这根阳线之后的下一个月，大约四分之三的标的又收了阴线，只有约四分之一接着收阳；而接着收阳的那一批，下个月会正好出现在「月线两连阳」那张榜上（前面三根阴线、后面两根阳线，恰好是两连阳），两张榜可以接着看。它和「月线两连阳」「月线四连阳」永远不会出现同一个标的（那两张要求上个月收阳，本榜要求上个月收阴），和「月线五连阴」也不会（那张要求这个月收阴）。这是一次性的事件清单不是持续清单——同一个标的只在收出这根阳线的那个月出现，下个月它的上一根就成了阳线，必然离开本榜，所以相邻两个月的名单不会有重叠。命中数随大盘起伏很大：回放过去 48 个月，每月中位只有 2 个，接近四成的月份一个都没有，全市场一起反弹的月份能冲到一百个以上；上线当天的 112 个就是这样的月份，接近这 48 个月里的最高值，别把它当成常态。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 5 个月的新合约看不出「再往前一个月不是阴线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-10（当日第 8 条）站长「新增一个TAB，逻辑是：月线恰好2连阳且成交量递增。」（没点资产 ⇒ 加密）。追加在末尾成为 ⑭。
            // ⚠️⚠️ 2 个条件、都在月线：① 恰好 2 连阳（与「月线两连阳」同一个判据）② 最新已收盘月 K 的成交量（k[5]）严格大于上个月。
            // ⚠️⚠️ **⊊「月线两连阳」**；互斥继承自它（与四连阳 / 五连阴 / 三连阴后首阳永不重叠）；看成交量 ⇒ 不算纯 K 线形态那一类。
            // ⚠️ name 用「成交量」照站长用词（口径是 k[5]，不是成交额 k[7]）；2 条件同周期 ⇒ ＋。
            // 📊 48 个月回放：中位 3.5 / max 87 / 均值 13.0 / 8 个月为 0（上线当天 72）；相对两连阳总保留率 60.8%；次月成三连阳：量增 35.8% < 两连阳全体 38.7% < 两连阳里量没增的 43.3%；相邻两月交集 0、互斥三对 0/48
            { key: "monthlyTwoBullExactVolRising", name: "月线两连阳＋成交量递增", tf: "月线",
              desc: "两个条件都看月线：一是最新已收盘的那根月线正好是连续第 2 根阳线——这个月收阳、上个月也收阳，而再往前一个月不是阳线（和「月线两连阳」完全同一个判据）；二是这个月的成交量比上个月大，也就是第二根阳线是放量收出来的。这里的成交量指按币计的成交量（和常见行情软件里的成交量柱一致），不是按 USDT 计的成交额，价格涨得多、成交量却缩了的月份不算递增。所以本榜的成员必然全部出现在「月线两连阳」那张榜上，是那张榜里第二个月放了量的那一批；它和「月线四连阳」「月线五连阴」「月线三连阴后首阳」也永远不会出现同一个标的。放量上涨常被当成上涨更可信的证据，但回放给出的答案并不支持这一点，这里如实说明：过去四年里，本榜标的在下一个月接着收阳、走成三连阳的比例大约三成六，反而略低于「月线两连阳」全体的三成九，也低于其中没放量那一批的四成三——至少在月线两连阳这个位置上，放量没有让连涨更容易延续，本榜更适合用来找第二个月有资金明显跟进的标的，而不是当成更强的做多信号。这是一次性的事件清单不是持续清单——同一个标的只在第 2 根阳线那个月出现，下个月它要么成了第 3 根、要么连阳已经断掉，所以相邻两个月的名单不会有重叠。命中数随大盘起伏：回放过去 48 个月，每月中位 3 到 4 个，大约六分之一的月份一个都没有，最多的一个月有 87 个；上线当天的 72 个属于偏多的月份，别把它当成常态。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的成交额是日线的 USDT 成交额，和本榜判断用的月成交量不是一回事，别拿它来核对「递增」；表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 3 个月的新合约看不出「再往前一个月不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            ...
            // ⚠️ 事件型但相邻两日交集不恒空：今天的首根明天成第二根、仍在榜上 ⇒ 最多连着两天、连着三天交集恒空。别为 SAR 加轴；18 轴照旧。
            ...
            { key: "dailySarFirstBar", name: "日线SAR多头前两根", tf: "日线",
              desc: "只看最新已收盘的那根日线：它是这一轮 Parabolic SAR 多头的头两根之一——要么是 SAR 刚刚由空翻多的第一根（前一天还在空头一侧），要么是紧接着站住的第二根（翻多之后的第二天，仍在多头一侧）。已经翻多三天及以上的标的不在这张榜上，表格里的 日SAR多头根数 在本榜只会是 1 或 2：想抢第一时间的看 1，想让信号多被确认一天、滤掉翻多当天就掉头的看 2。本榜只看 SAR 这一个条件，完全不看均线、成交额或资金流，所以「日线9/21扩张＋SAR多头前两根＋成交额递增」那张榜上的每一个标的都必然也在本榜上——那张就是本榜再叠上均线张开、成交额放大两道条件之后留下的那一小撮；本榜里多出来的，是 SAR 翻多了、但均线还没在张开或者成交额没比前一天大的那些，其中不少只是下跌途中的一次反抽，需要自己分辨，也因此表格里的 日EMA间距 可以是负数（EMA9 还在 EMA21 下方），这是判据决定的、不是数据出错。代价要说清楚：日线 SAR 是很敏感的信号，回放过去一年，翻多之后这一轮多头维持的天数中位只有 9 天左右，大约七分之一在三天之内就重新翻回空头，能撑过十天的不到一半。这是一张事件清单而不是持续清单：同一个标的最多连着两天出现在这里（翻多当天是第一根、第二天是第二根），第三天起就不在了，所以名单换得很快，每天的名单里大约一半是前一天就在榜上的第二根。命中数随行情起伏：回放过去一年，每天中位 27 个左右，全市场一起反弹之后的那一两天能到一两百个，一年里没有一天是空的。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。除了 日SAR多头根数，表格里的其余各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-11 站长「新增一个TAB，逻辑是日线4连阳。做完全局收尾。」（没点资产 ⇒ 加密）。
            //   ＝ `monthlyFourBull` / `weeklyFourBull` 的日线版，纯 K 线形态、零指标；追加在末尾（既有编号未动）。
            // ⚠️⚠️ 「恰好」不是「至少」（站长没写"恰好"，按站内全部 N 连阳/连阴榜的既有口径）：倒1..倒4 全阳 且倒5 非阳（含平盘）。
            // ⚠️ key `dailyFourBull` 复用 2026-07-22 移除的同名旧 tab、但判据不同（旧＝9/21扩张∩4连阳，本＝纯 4 连阳）；三处均无残留、复用安全。
            // ⚠️ 与月/周线四连阳无结构性关系（周期不同、互不蕴含）；一个条件 ⇒ 名字无 ＋、无 EMA ⇒「写全 EMA 周期数」不适用。纯 K 线形态由七个变【八个】。
            // 📊 90 天回放（含 ≥23 根门槛）：中位 6 / p25 3 / p75 12 / max 121 / min 0 / 3 天为 0（上线当天 2，偏少的一天）；相邻两日交集实测恒 0（判据保证）。
            { key: "dailyFourBull", name: "日线四连阳", tf: "日线",
              desc: "最新已收盘的那根日线，正好是连续第 4 根阳线——今天收阳，往前数第 2、3、4 天也都收阳，而第 5 天不是阳线。注意是恰好 4 根，不是至少 4 根：已经连涨 5 天、6 天的标的不在这张榜上，它们的最新一根是第 5 根、第 6 根而不是第 4 根。本榜是「月线四连阳」「周线四连阳」的日线版，同样只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按天计的上涨，是不是刚好走到第四天。连续四天收阳说明短线的连涨刚刚成形、中途没有一天收阴，卡在第 4 根这个位置，找的是连涨已经起步、但还没走远的那一段；想看更长周期的同类形态，可以对照「周线四连阳」「月线四连阳」——三张榜周期不同、互不蕴含，同一个标的日线四连阳并不代表它周线或月线也四连阳。这是一次性的事件清单不是持续清单——同一个标的只在第 4 根那一天出现，第二天它要么成了第 5 根、要么连阳已经断掉，两种情况都会让它离开本榜，所以相邻两天的名单不会有重叠。命中数天然不多、遇到空榜也正常：回放过去 90 天，每天命中的中位数在 6 个上下，多数日子是个位数，全市场一起普涨的那一天能冲到一百二十来个，大约每个月有一两天一个都没有；上线当天只有 2 个，是偏少的一天，别把它当成常态。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。本榜要求至少 23 根已收盘日 K 才纳入，所以上市不足约 23 天的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-11 站长「新增一个TAB，逻辑是日线2连阳。做完全局收尾。」（没点资产 ⇒ 加密）。
            //   ＝ `monthlyTwoBullExact` / 已移除的 `weeklyTwoBullExact` 的日线版，纯 K 线形态；追加在末尾（既有编号未动）。
            // ⚠️⚠️ 「恰好」不是「至少」（站长没写"恰好"，按站内既有口径）：倒1阳、倒2阳、倒3非阳（含平盘）。省掉倒3非阳就退化成 ≥2 连阳、命中翻几倍。
            // ⚠️ key `dailyTwoBullExact` 带 Exact 照月/周线两连阳先例；全新 key、站内从无、三处均无残留。**与 `dailyFourBull` 结构性互斥**（倒3 非阳 vs 阳）。
            // ⚠️ 与月/周线两连阳无结构性关系（周期不同、互不蕴含）；一个条件 ⇒ 名字无 ＋、无 EMA。纯 K 线形态由八个变【九个】。
            // 📊 90 天回放（含 ≥23 根门槛）：中位 37.5 / p25 19 / p75 71 / max 300 / min 5 / **0 天为空**（上线当天 6，偏少的一天）；相邻两日交集实测恒 0、与四连阳交集实测恒 0（判据保证）。
            { key: "dailyTwoBullExact", name: "日线两连阳", tf: "日线",
              desc: "最新已收盘的那根日线，正好是连续第 2 根阳线——今天收阳，昨天也收阳，而前天不是阳线。注意是恰好 2 根，不是至少 2 根：已经连涨 3 天、4 天的标的不在这张榜上，它们的最新一根是第 3 根、第 4 根而不是第 2 根。本榜是「月线两连阳」的日线版，同样只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按天计的上涨，是不是刚好走到第二天。日线收一根阳线常常只是一次反抽，连着收两根才说明短线的上涨延续到了第二天，卡在第 2 根这个位置，找的是按天计的连涨刚刚起步、还处在最早期的那一批——这也意味着它是站内连阳类里门槛最低、信号最弱的一张，两天的连涨随时可能断，请配合排序条上的量能、动能轴自行取舍。它和「日线四连阳」永远不会出现同一个标的：四连阳要求往前第 3 天也是阳线，而本榜要求那一天不是阳线，同一根日线不可能同时满足。想看走得更远的连涨，可以对照「日线四连阳」，或周期更长的「月线两连阳」——后者和本榜周期不同、互不蕴含，同一个标的日线两连阳并不代表它月线也两连阳。这是一次性的事件清单不是持续清单——同一个标的只在第 2 根那一天出现，第二天它要么成了第 3 根、要么连阳已经断掉，所以相邻两天的名单不会有重叠。命中数随大盘起伏很大：回放过去 90 天，每天命中的中位数在 37 个上下，冷清时也就几个，全市场一起反弹的那一天能冲到三百个，过去 90 天没有一天是空的；上线当天只有 6 个，是偏少的一天，别把它当成常态。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。本榜要求至少 23 根已收盘日 K 才纳入，所以上市不足约 23 天的新合约不入榜。范围是全部加密 USDT 永续合约。" },
        ],
    },
...
const TEASER_TAB = "dailyEmaSarFirstTwo";
...
let currentTab = "dailyEmaSarFirstTwo";
...
function switchAsset(assetK) {
    ...
    const fallback = assetK === "ashare" ? "ashareMonthlyWeeklyDaily" : "dailyEmaSarFirstTwo";
