...
const TABS_CONFIG = {
    ...
    // ⚠️ 连带作废：**`emaGap` 恒正的日线榜由三个变【两个】**
    //   （`dailyEmaSarFirstTwo` / `dailyEmaSarBearish`）；结构性互斥/包含的对数均**未受影响**。
    ...
    //   变【三对】：⑦∩⑧（周线，08-18）· ⑫∩⑧（周线，09-06，继承自 ⑦∩⑧）· ⑬∩⑭（月线，09-06）。
    ...
    //   `sarBullBars` **恒 ∈ {1, 2}**（跨实现对账）。⚠️ emaGap 恒正的日线榜由**四个变【三个】**
    //   （④⑪ 合成一个）：现为 `dailyTripleEmaCvd` / `dailyEmaSarBearish` / 本榜。
    ...
    //   里那张日线 SAR 榜〔`dailyEmaSarFirstTwo`，2026-09-09 由 ④＋⑪ 合并而成〕上方那段已改）。
    //   ⚠️ 但「本榜是站内唯一找回调的榜」这半句
    ...
    //   合并成 `dailyEmaSarFirstTwo` 后 ④∩⑪ 那一对消失 ⇒ 现为【三对】**（⑦∩⑧ · ⑫∩⑧ · ⑬∩⑭）。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            //   榜由此变三个（2026-09-03 当时：dailyTripleEmaCvd / dailyEmaSarFlip / 本榜）；
            //   ⚠️ 09-05 加 dailyEmaSarSecond 后其实是四个（这行当时没跟上）、**09-09 那两个日线榜
            //   合并成 dailyEmaSarFirstTwo 后又回到三个**（dailyTripleEmaCvd / dailyEmaSarFirstTwo / 本榜）。
