...
function axesSub(item, sf, volLabel, extra) {
    ...
    // 周线EMA间距：**两个加密周线榜的行带**（weeklyEmaBearish + dailyBreakWeeklyBearSar，
    // 都走后端 `_wk_expansion_row`），数据驱动判断。
    // ⚠️⚠️ **那条老注释「它是严格 9/21 ⇒ 恒为正、看到负值就是有人加回了 9/26 并集」已于
    // 2026-07-27 作废**（站长把 weeklyEmaBearish 改成「周线 SAR 多头即可」，EMA 条件整个
    // 去掉了；新加的 dailyBreakWeeklyBearSar 更是完全不碰 EMA）。**现在负值完全正常，
    // 别再照那条自检点排查** —— 而且负值恰恰是最有用的一档：周线方向已转、9/21 还没张开
    // ＝最早期的形态。（key 里的 "Ema"/"Bearish" 两个词也都早已名不副实。）
...
// **现在服务 ④⑤⑥⑧⑨ 五个榜**（2026-07-28 加两个突破榜、2026-07-30 加 `weeklySarSecondBar`
// 与 `weeklyEmaSarBull`；这行一度写着"只服务一个榜"/"三个榜"，都是漏改，随加榜就地补齐）。
// **别把它合并进 cryptoStrategySorts** —— 另四个加密榜（①②③⑦）的行没有
// weeklyVolume/weeklyRsi/weeklyEmaGap 字段，合并会多出永远全 null 的幽灵轴。
//   前 7 根 = 上面 cryptoStrategySorts 的日线部分**原样照搬**（"现在各种日线的升降序"）；
//   后 5 根 = 周线块（周线RSI + 周线EMA间距 + 周ADX + 周+DI + 周MACD强弱；周成交额已于
//              2026-07-28 删除，见下面第二段与 AXIS_W_VOL 那处的存档）。
...
// ⚠️ 另四个加密榜（①②③⑦）的行**没有** weeklyRsi/weeklyEmaGap 字段（后端没发），
...
const TABS_CONFIG = {
    ...
    // === 加密策略：八个榜（2026-07-25 晚站长先「移除加密所有TAB」清空 → 逐条指令加回五个 →
    ...
    // `weeklyBreakBearSar` ⇒ 六个 → 2026-07-30 新增 `dailySarSecondBar` ⇒ 七个 → 紧接着
    // 新增 `weeklySarSecondBar` ⇒ 八个 → 再新增 `weeklyEmaSarBull` ⇒ **九个**）===
    // **轴集分两族**：①②③**⑦** 用 11 轴的 cryptoStrategySorts（全日线值）；**④⑤⑥⑧⑨ 共用** 13 轴的
    // cryptoWeeklyExpansionSorts（行里同时装日线值和周线值，都走后端 `_wk_expansion_row`）。
    // 族内行 payload 逐字同构，**差别只在筛选条件**。九个榜首轴统一 = 日成交额降序。
    // ⚠️ 新榜一律**追加在末尾**（⑤⑥⑦⑧⑨ 都是这么加的）：插到中间会让 ①-⑨ 这套编号整体位移，
    // 而 CLAUDE.md、后端 docstring、本文件多处注释都按编号互相引用。
    // ① 月×周×日 三个 SAR 全多头的三级共振（3 个条件，**不含新币回退**，见后端注释）。
    monthlyWeeklyDaily: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    // ② 日线 9/21/55 三线扩张（**严格，不含 9/26**）—— **1 个条件**（2026-07-28 站长把
    // 「＋CVD递增」去掉；此前 2026-07-26 把 EMA 那条由两线收紧成三线）。
    // ⚠️ **两次改判据轴集都一根没动**（行 payload 一个字段都没改，动的都是 build 层的成员
    // 资格判断）：别以为加了 EMA55 就要加一根轴 —— 后端没给行发 ema55/emaGap，加了就是
    // 永远全 null 的幽灵轴；也别以为去掉 CVD 条件就要删「日CVD强弱」那根轴 —— 条件用的是
    // 布尔的 cvd_rising，轴用的是可排序的归一化 cvdStrength，两码事。
    ...
    // ⑤ **日线收盘价站上"最近一轮周线 SAR 空头段首根周 K 的收盘价"（1 个条件）** ——
    // 2026-07-28 站长新增。周线出参照价、日线判突破，是全站唯一一个"参照价长在另一个
    // 周期上"的榜。与 ④ 同为"**状态**"型（日线不跌回参照价下方就一直在榜上）。
    // ⚠️ **与 ④ 没有包含关系**：周线仍是 SAR 空头时本榜照样能命中（日线领先于周线翻多），
    // 那批标的一个都不在 ④ 里。别假设谁是谁的子集。
    // 行 payload 同样走后端 `_wk_expansion_row` ⇒ **与 ④⑥ 共用这 13 轴**（那个常量现有
    // 三个消费者，改它会同时动三个榜）。周线那 5 根在这里是描述值：按 周线EMA间距 /
    // 周ADX 排序，正好能把"周线还在空头、日线抢跑"和"周线已翻多、收复整轮跌幅"这两档
    // 分开 —— 这是本榜最主要的读法。
    // ⚠️ **别因为判据里有"收盘价"就加价格轴**：后端刻意没给行发 close / 参照价
    // （axesSub 是数据驱动的，多带字段会冒出排序条上没有的幽灵摘要段）。站长这次一个字
    // 都没提排序，沿用现成轴集。
    dailyBreakWeeklyBearSar: { sorts: cryptoWeeklyExpansionSorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    // ⑥ **周线收盘价站上"最近一轮周线 SAR 空头段首根周 K 的收盘价"（1 个条件）** ——
    // 2026-07-28 站长紧随 ⑤ 之后新增。**参照价与 ⑤ 完全相同**，只把突破端从日线换成周线
    // ⇒ 两端都在周线，是纯周线榜（⑤ 才是跨周期的那个）。
    // ⚠️⚠️ **与 ⑤ 互不包含**（比的是不同时点的价格：周线收盘＝上周日的，日线收盘＝昨天的）。
    // 上线当天 102 vs 80：交集 76、本榜独有 26、⑤ 独有 4，Jaccard 0.717。两个方向语义相反
    // 且都有用 —— ⑤ 独有＝日线刚站上、周线还没确认（最新鲜）；本榜独有＝周线已确认但日线
    // 又跌回参照价下方（已确认 + 回踩）。**重合度确实不低，若日后要精简这两个是首选合并
    // 候选**，但那是产品判断，未经站长明确要求别自作主张删掉任何一个。
    // ⚠️ 反直觉但实测如此：命中里 **75% 的周线 SAR 仍是空头** —— SAR 翻多要价格上穿圆点，
    // 而空头段前期圆点还高悬在段前高点附近，"收盘价收复起点"比"SAR 翻多"来得早。所以本榜
    // 与 ④（周线SAR多头）交集也只有 26：④⑤⑥ 是围绕同一根周线 SAR 的三个不同问题。
    weeklyBreakBearSar: { sorts: cryptoWeeklyExpansionSorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    // **状态**（会连着很多天/周为真、成员跨期稳定），本榜是**一次性事件**——每个标的一轮
    // 行情只在翻多后那一天命中 ⇒ **成员每天几乎全换、命中数天然很小**。
    // 看到它只有个位数、且和昨天完全不是同一批，那是正确行为不是数据挂了。
    // 行 payload 与 ①②③ 逐字同构 ⇒ **共用 cryptoStrategySorts 11 轴，一根没新加**。
    ...
    // ⚠️⚠️ 行 payload 走 `_wk_expansion_row` ⇒ **13 轴的 cryptoWeeklyExpansionSorts**
    // （与 ④⑤⑥ 共用，该常量从此有四个消费者）。轴集与 ⑦ 不同是刻意的：本榜条件在周线，
    // 周线块那 5 根轴才是它的主要读法（按周ADX / 周MACD强弱 分「大级别也在向上」vs「空头
    // 里的反弹」）。**别因为判据里有 SAR 就加 SAR 轴**（布尔值排不了序、后端没发圆点价）。
    weeklySarSecondBar: { sorts: cryptoWeeklyExpansionSorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    // ⑨ **周线 EMA9/21 扩张 且 SAR 多头（2 个条件，都在周线）** —— 2026-07-30 站长新增。
    // 就是 ④ 槽位的第二版判据（2026-07-26 指令③），但**是新榜**：④ 2026-07-27 已改成只看 SAR，
    // 站长现在要"扩张 + SAR"两条都要 ⇒ 与 ④ 并存，是 ④ 的**严格子集**（凡命中必在 ④）。
    // ⚠️ **不是取代候选**：④ 是"纯趋势方向"（命中很多、状态型），本榜再加"结构在张开"这道门
    // （命中少得多），语义是"趋势 + 结构双确认"，rail 上一眼分得开。与 ⑧ 也都 ⊊ ④，但彼此
    // 无包含关系（一个筛结构张开、一个筛刚翻多满一周）。
    // 行 payload 走后端 `_wk_expansion_row` ⇒ **13 轴的 cryptoWeeklyExpansionSorts**（与 ④⑤⑥⑧
    // 共用，该常量从此有**五个**消费者）。**轴一根没加**（站长「支持当前各种升降序」＝沿用现成
    // 那套）。⚠️ 显示名写「9/21扩张」不写「两线扩张」：后者是 (9/21 ∪ 9/26) 并集的专称，本榜是严格 9/21。
    weeklyEmaSarBull: { sorts: cryptoWeeklyExpansionSorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
...
const TAB_GROUPS = [
    ...
    {
        // 加密：**七个榜**（2026-07-25 晚站长先「移除加密所有TAB」把当天定版的三个榜全部
        // 清空、随后逐条指令加回五个 → 2026-07-26 晚移除一个周线榜剩四个 → 2026-07-28 新增
        // `dailyFourEmaPersist` 的同日又把 `dailyFourEmaAligned` 与它二选一、只留新的
        // ⇒ 仍是四个 → 同日新增 ⑤⑥ 两个突破榜 ⇒ 六个 → 2026-07-30 新增 ⑦
        // `dailySarSecondBar` ⇒ 七个 → 紧接着新增 ⑧ `weeklySarSecondBar` ⇒ **八个**；
        // 被清掉那些的显示名/desc/专属轴见 git 历史）。
        // ⚠️ **刻意不套 singleStrategyGroup 工厂**：那个工厂的 name/desc 写死的是股票系
        // 的 5 个条件（月线SAR + 周线SAR + 周线扩张 + 日线扩张 + CVD），与这八个榜都不
        // 一样。硬套会让页面上的规则说明与后端实际筛选严重不符。
        // 组 tf 取第一个 tab 的周期（月线），其余七个 tab 各自覆盖。**排法：同周期的放一起**
        // （月×周×日 → 日线两个 → 周线三个 → ⑦ 日线SAR第二根 → ⑧ 周线SAR第二根）——两个
        // 日线榜相邻是刻意的：判据只差一处（9/21/55 三线扩张 vs 四线排列且启动过），挨着才
        // 看得出对照。
        // ⚠️ **⑦⑧ 追加在末尾、没有挪去和同周期的榜作邻居**：新榜一律追加是房规（⑤⑥ 也是
        // 这么加的），插到中间会让 ①-⑧ 整套编号位移，而后端 docstring / CLAUDE.md / 本文件
        // 多处按编号互相引用。⑦⑧ 是一对（日线/周线的 SAR 第二根），追加时保持相邻即可，
        // 不必强行塞到同周期的既有榜旁边。
        // 周线那三个（周线SAR多头 / 日线突破周线SAR空头起点 / 周线突破SAR空头起点）
        // 2026-07-28 起相邻，**都以同一根周线 SAR 为轴心、行构造与 13 轴完全相同，但两两
        // 之间都没有包含关系**——挨着放正是为了看出这一点，三者问的是三个不同的问题：
        //   ④ 周线方向现在是不是向上（SAR 圆点在哪一侧）
        //   ⑤ 日线有没有先站上这轮下跌的起点（最快、最新鲜）
        //   ⑥ 周线自己有没有站上那个起点（慢半拍、更确认）
        // ⚠️ ⑤⑥ 用的是**同一个参照价**，只差拿谁去比 —— 上线当天 80 vs 102、Jaccard 0.717。
        // 重合度不低但方向相反，若日后要精简，**这两个是首选合并候选**（产品判断，别自作主张）。
        // ⚠️ 曾经还有第四个周线榜（并集＋周CVD），2026-07-26 晚站长要求移除。
        ...
        tabs: [
            ...
            // ⚠️ **轴集一根没动**（仍 cryptoStrategySorts 11 轴）：行 payload 一个字段都没改，
            // 改的只是 build 层的成员资格判断。
            ...
            { key: "dailyEmaExpansion", name: "日线9/21扩张＋量/CVD/RSI递增", tf: "日线",
              desc: "四个条件同时成立，全部看最新已收盘的那根日线：① EMA9 在 EMA21 上方，且两者的间距比前一天更大（均线正在张开，不是单纯的多头排列）；② 成交量比前一天大（口径与图表上的成交量柱一致，看的是币的数量不是 USDT 金额）；③ CVD 在 0 轴上方且比前一天更高；④ RSI 比前一天更高。结构、量能、资金、动能四路同时抬头的那一天，找的是启动当日而不是已经走了一段的行情——所以成员天天换，连着两天一样反而少见。上市不足 23 天的新合约数据不够，不入榜。范围是全部 USDT 永续合约。" },
            ...
            // 规矩（完整词义约定见下面 weeklyEmaBearish 那条）。
            { key: "dailyFourEmaPersist", name: "日线9/21/55/200扩张存续", tf: "日线",
              desc: "最新已收盘日线上 EMA9 > EMA21 > EMA55 > EMA200 四条均线依次排开，而且这段排列当初是被一次真正的「四线扩张」启动的——历史上某一天 EMA9/21、21/55、55/200 三档间距同时比前一天更大，从那天起四条均线的先后次序一直没有被打破，一直排到今天。它排除的是「四条线慢慢磨蹭成排列、从来没有一起张开过」的那一类，留下的是「有过明确启动、且趋势结构至今完好」的标的。不要求今天还在扩张，所以是一张趋势存续清单而不是当日事件清单，成员跨天稳定；行情普涨、符合条件的标的一多的时候，这道启动门槛剔得最狠。上市不足 201 天、算不出 EMA200 的新合约不入榜；另外能回看的历史约 300 根日线，启动点比这更早又一路没破的极少数标的会看不到那次扩张，按规则不入榜。范围是全部 USDT 永续合约。" },
            { key: "weeklyEmaBearish", name: "周线SAR多头", tf: "周线",
              desc: "最新已收盘周线的 Parabolic SAR 站在多头一侧（圆点落在价格下方）——只有这一个门槛，是全站扫描面最宽的榜。它看的是大级别趋势的方向本身，不问结构张开到什么程度、也不问资金，所以命中数天然很大、成员跨周稳定（SAR 多头是一种状态，上升趋势里会连续多周为真）。真正的用法在排序：在这一大批「周线方向已经向上」的标的里，按周线EMA间距降序能挑出趋势已经跑开的，按它升序则是刚翻多、结构还没张开的最早期一档（这个值可正可负）；周ADX 则区分「趋势有力度」和「刚翻多但还在震荡」。只需 3 根已收盘周 K，所以新上市合约也会进来，它们的周线轴（周RSI 要 16 根、周ADX 要 28 根、周MACD 要 35 根）会显示「—」。表格里的日线八轴不参与筛选，用来在这批标的里再分日线强弱。范围是全部 USDT 永续合约。" },
            // tf 周线：参照价长在周线上（日线只负责判有没有站上去），角标跟着参照价那一侧走。
            // ⚠️ 2026-07-28 站长逐字：「最新已收盘日线的收盘价成功突破了最近一轮周线空头SAR
            // 趋势的首根K线的收盘价。」——**「最近一轮」不要求那段空头已经结束**，最新周 K
            // 仍是空头时就是当前这一段（后端 `last_bear_sar_first_close` 的 docstring 是权威）。
            // ⚠️ **name 里的「起点」＝那段空头首根周 K 的收盘价**，不是 SAR 圆点价、也不是
            // 最高价。写「空头起点」而不是「空头SAR首根收盘价」纯粹是 rail 宽度所限（148px
            // 可用、两行封顶），准确含义交给 desc。
            // ⚠️ 本榜完全不碰 EMA，name 里那条「写全 EMA 周期数」的规矩在这里不适用。
            { key: "dailyBreakWeeklyBearSar", name: "日线突破周线SAR空头起点", tf: "周线",
              desc: "先在周线上找到最近一轮 Parabolic SAR 空头趋势，取它第一根周 K 的收盘价——那是这轮下跌开始的地方；本榜要求最新已收盘日线的收盘价站到这个价位之上（严格站上，打平不算突破）。两种情形读法不同，值得先分清：周线目前仍是空头时，参照的是正在进行的这轮下跌的起点，日线抢在周线翻多之前先站了上去，是趋势可能反转的最早一批信号；周线已经翻多时，参照的是刚结束那轮下跌的起点，站上去意味着整轮跌幅被收复干净、力度强得多。想把这两档分开就按周线EMA间距或周ADX 排序——周线那几根轴在本榜不参与筛选，只用来描述周线自己现在的状态。和下面那张「周线突破」用的是同一个参照价，区别只在拿谁去比：这里是日线（更快、更新鲜），那里是周线（更慢、更确认），两张榜互有出入。上市以来从没出现过周线 SAR 空头的标的没有参照价，不入榜；周线不足 3 根、日线不足 23 根的新合约同样不入榜。范围是全部 USDT 永续合约。" },
            // tf 周线：参照价与突破端都在周线，是纯周线榜（上一个才是跨周期的）。
            // ⚠️ 2026-07-28 站长逐字：「最新已收盘周线的收盘价突破了最近一轮周线空头SAR趋势
            // 的首根K线的收盘价。」——与上一个榜**只差突破端**，参照价是同一个。
            // ⚠️ name 只写一个「周线」：两端同周期，写成「周线突破周线SAR空头起点」既啰嗦
            // 又会让人以为有跨周期。上一个榜必须写两个周期名，正因为它是跨周期的——**这个
            // 命名差异本身就在传达"同周期 vs 跨周期"**，别为了"看起来对称"把它们改成一样。
            { key: "weeklyBreakBearSar", name: "周线突破SAR空头起点", tf: "周线",
              desc: "参照价与上面那张「日线突破」完全相同——最近一轮周线 Parabolic SAR 空头趋势第一根周 K 的收盘价，也就是这轮下跌开始的地方；区别只在拿谁去比：本榜看的是最新已收盘周线的收盘价（同样要严格站上，打平不算）。周线收盘一周才更新一次，所以它比日线那张慢半拍、也更确认：日线可能今天站上明天又跌回去，周线站上去意味着一整周都守在了上面。两张榜互有出入——上面那张独有的是「日线刚站上、周线还没确认」的最新鲜一批，本榜独有的是「周线已经确认、但日线近几天又跌回参照价下方」的回踩一批。另外有一点值得知道：本榜里大多数标的的周线 SAR 其实还是空头，这不矛盾——SAR 要翻多得等价格上穿圆点，而空头段前期圆点还高悬在下跌起点附近，所以「收盘价收复起点」通常比「SAR 翻多」来得早，本榜因此是比「周线SAR多头」更早的一档信号。上市以来从没出现过周线 SAR 空头的标的没有参照价，不入榜；周线不足 3 根的新合约同样不入榜，它们的日线各轴也可能显示「—」。范围是全部 USDT 永续合约。" },
            ...
            // ⚠️ 与上面的「周线SAR多头」（④，纯 SAR 多头）是**严格子集关系但不是双胞胎**：那张
            // 是"状态"（多头能连着很多周为真、命中很多），本榜是"事件"（只有翻多后第二周命中、
            // 命中很少）——留着两张不是冗余。轴集与 ④⑤⑥ 相同（13 轴，行走 _wk_expansion_row）。
            { key: "weeklySarSecondBar", name: "周线SAR多头第二根", tf: "周线",
              desc: "最新已收盘的那根周线，正好是这一轮 Parabolic SAR 多头趋势的第二根——上一周 SAR 刚刚由空翻多，再往前一周还是空头。它是上面「日线SAR多头第二根」的周线版，看的级别高一档：日线翻多是几天的事、容易反复，周线翻多是几周的事、一旦成立分量重得多，所以「周线翻多刚站住一周」往往是一波大级别趋势起步的信号。同样是一次性的事件清单不是持续清单——同一个标的一轮行情里只在翻多后的那一周出现，第三周就不在了，所以命中数天然很少、每周换一批。它和上面那张「周线SAR多头」的关系要分清：那张只要 SAR 还在多头一侧就一直在（可能已经涨了很多周），本榜只挑其中「刚翻多满一周」的那一小撮，是更早、更精的一档。表格里的日线八轴和周线五轴都不参与筛选，用来在这批标的里再分强弱。上市不足 4 周、还看不出「翻多的前一周是空头」的新合约不入榜；日线数据不足的，日线那几轴会显示「—」。范围是全部 USDT 永续合约。" },
            // tf 周线：两个条件都在周线。⚠️ 2026-07-30 站长逐字：「新增一个TAB，逻辑是：最新
            // 已收盘周线是EMA9/21扩张，且SAR多头。支持当前各种升降序。」（没点资产 ⇒ 加密）
            // ⚠️⚠️ 这是 ④「周线SAR多头」槽位的第二版判据的**新榜化身**（不是复活 ④）：④ 现在
            // 只看 SAR，本榜在其上再叠一道"EMA9/21 结构在张开" ⇒ 是 ④ 的严格子集但不是取代。
            // ⚠️ name 写「9/21扩张」不写「两线扩张」：后者是 (9/21 ∪ 9/26) 并集专称，本榜严格 9/21。
            { key: "weeklyEmaSarBull", name: "周线9/21扩张＋SAR多头", tf: "周线",
              desc: "最新已收盘周线要同时满足两条：一是 EMA9 在 EMA21 上方、而且两者的间距比上一周更大（均线正在张开，不是单纯的多头排列），二是这根周线的 Parabolic SAR 站在多头一侧。它比上面那张「周线SAR多头」多加了一道结构门槛——那张只问大级别趋势方向是不是向上，本榜还要求均线结构本身正在加速张开，所以命中数少得多，找的是「趋势方向和结构强度双双确认」的标的，而不是「方向朝上但可能还在磨」的一大批。用法上，SAR 保证了方向，EMA 扩张保证了力度，两者叠加天然偏强势；排序时按周ADX 或周MACD强弱 能进一步区分「大级别力度足」和「刚起步」。需要至少 22 根已收盘周 K 才算得出 EMA9/21 扩张，所以上市不足约半年的新合约不入榜。表格里的日线八轴不参与筛选，用来在这批标的里再看日线强弱。范围是全部 USDT 永续合约。" },
        ],
    },
