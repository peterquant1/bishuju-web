...
const TABS_CONFIG = {
    ...
    // ⚠️ **2026-08-16 起是站内唯一的周线榜**（同样蕴含「周线 SAR 多头」的
    // `weeklySarSecondBar` 已移除）。
    ...
    //   那条互斥仍写在后端 compute 层的 `sarSecondBar` 注释里，但已降为"live 榜 vs 保留组"；
    //   desc 里对应那段也已删。
    ...
    // 💡 行里 `emaGap` **恒为正**（9/21 扩张蕴含 EMA9>EMA21）⇒ 现在 ③④ **两个**榜都是
    //   （履历：2026-08-12 移除 `dailyEmaExpansion` 前五个、2026-08-16 移除
    //   `dailyFourEmaExpansion`/`dailyTripleEmaSarBearish` 前四个）。
    ...
    //   里 dailyEmaSarFlip 上方那段已改）——**本榜就是站内唯一找回调的榜**。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        // 张开，完全不看 SAR 也不看资金）+ **一个"找回调"榜（⑦ 周线阴K＋SAR多头，站内
        // 唯一要求阴K 的榜、也是唯一反过来问"哪些在强势里回踩"的榜）**
        // + **一个纯 K 线形态榜（⑧ 周线两连阳第二根，站内唯一不看任何指标、只数阴阳的榜）**。
        // ⚠️ **周线榜现为四个（②⑥⑦⑧），两两互不包含**：②⑥ 都蕴含周线 9/21 扩张（② 再加
        // SAR 门、⑥ 再加 EMA55 档）；②⑦ 都蕴含周线 SAR 多头（② 再加扩张、⑦ 再加阴K，
        // 60 周回放两个方向 0/60、反例 2814 + 729 人次 ⇒ 结构性独立）；⑥⑦ 更无交集条件；
        // **⑧ 完全不看 EMA/SAR，与 ②⑥ 两个方向都不包含（60 周回放反例 1782/681、1869/249）**。
        // ⇒ 「站内没有任何一对结构性**包含**的 live 榜」继续成立。
        // ⚠️⚠️ **但"没有任何一对结构性互斥"那句 2026-08-18 起不成立了**：⑦ 要求当根周 K 阴、
        // ⑧ 要求当根周 K 阳 ⇒ **交集恒为空**（60 周回放 0/60 违规），是站内重新出现的第一对。
        // **互斥 ≠ 包含，两句话别混。**
        // （⑥ 上线当天 n=1 恰好落在 ② 里、穷举会报一个"包含对"——60 周回放 25 周有反例，
        // 是巧合不是结构性的，别照单日快照下结论；那对与 ⑦⑧ 无关，别当成它们引入的。）
        // ⚠️ **②⑥ 的 `weeklyEmaGap` 恒为正、⑦⑧ 可正可负**（⑦⑧ 都完全不看 EMA）—— 四个周线
        // 榜里唯独 ⑦⑧ 不适用那条自检点，别互抄。
        ...
        tabs: [
            ...
            // ——「三线扩张」那个说法站内历来只在股票系用（完整词义约定见后端 build_rankings
            // 的「显示名的两条用词规矩」）。
            ...
            // 「周线阴K＋SAR多头」（见本组末条），**它现在是站内唯一要求阴K、也是唯一
            // "找回调"的榜**。别照那句旧话去找。
            ...
            // ——**那两条被撤回了**，以本判据为准。
            // ⚠️⚠️ **本榜是站内唯一要求阴K、也是唯一"找回调"的榜**（别的榜问"哪些在启动/
            // 在延续"，本榜问"哪些在强势里回踩"）——同日移除 dailyTripleEmaSarBearish 后
            // 那句「站内不再有阴K 榜」只成立了约两小时，见上方 dailyEmaSarFlip 那段。
    ...
    // 资产**没动 A股 一个字**。**2026-08-16 站长又改了一次判据**：五个条件（月线扩张 ×
    // 周线SAR × 日线量/CVD/RSI递增）整个换成**单条件「周线 EMA9/21/55 三线扩张」**，
    // 组 tf 随之由月线改周线。轴集仍是 singleStrategySorts 16 轴（改判据不改轴）。
    {
    ...
    { label: "A股策略", asset: "A股", tf: "周线",
      tabs: [
        { key: "ashareMonthlyWeeklyDaily", name: "周线9/21/55扩张",
          desc: "只看最新已收盘的那根周线：EMA9 在 EMA21 上方、EMA21 又在 EMA55 上方，而且 9 与 21、21 与 55 这两档间距都比上一周更大——三条周线级的均线依次排开并且同时在张开，不是单纯的多头排列。放在日线上这种形态几天就可能走完一轮，放在周线上一旦成立，对应的往往是按周计的中期趋势正在加速，短线噪音基本被抹平。本榜只看均线结构本身，不看 SAR、不看资金、也不看日线，是 A 股这边衡量「中期结构是否在加速」最纯粹的一档。它是一张状态清单不是事件清单：只要三条均线还在依次张开，同一只股票可以连着几周都在榜上；而且周线要到本周最后一个交易日收盘后才定型，同一周之内反复打开本榜，看到的名单基本不变，这是正确行为不是数据卡住了。需要至少 56 根已收盘周 K（约 13 个月）才算得出 EMA55 的扩张，所以上市一年出头以内的次新股不入榜；当日停牌的也不入榜。范围是全部沪深 A 股。请注意这是一张宽名单而不是精选：回放最近 60 周，命中数中位约 1200 只、少的时候一百来只、多的时候三千只出头（从来没有空过），行情好的周次经常超过单榜 1000 行的渲染上限、表尾会提示只显示了前 1000 名——那是正常的，用排序条和搜索框来收窄。表格里的日线各轴（RSI、成交额、量比、CVD强弱等）不参与筛选，正是用来在这批股票里再分高下；按周ADX 或周MACD强弱 排序能进一步分出「大级别已经跑起来的」和「刚开始张开的」。" },
      ] },
];
