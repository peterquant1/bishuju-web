...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //
        // 【现状分类】（**2026-09-12 1x:xx 移除 `weeklyFiveBear` / `monthlyFiveBear` 两张五连阴 ＋ 新增
        ...
        //   · **`emaGap` 恒为正的日线榜只有一个**：`dailyEmaSarFirstTwo`。另一个日线榜 `dailySarFirstBar`（09-12 移除 `dailyFourBull`
        //     之后日线端只剩这两张）不看 EMA、emaGap 可正可负 —— **别互抄**。`dailyEmaSarFirstTwo` ⊊ `dailySarFirstBar`（已计入上面的包含四对）。
        ...
        //     ⇒ 一减一加、总数没变）/ **日线榜两个**
        //     （`dailyEmaSarFirstTwo` / `dailySarFirstBar`；09-11 06:4x 一度四个，**09-12 移除 `dailyFourBull` 后两个**）。
        //     ⇒ 加密策略榜共 **14** 个（09-12 那天：06:2x 由 17 变 15、08:5x 合并后 14、09:xx 新增回到 15、
        //     **1x:xx 移除两张五连阴 ＋ 新增一张月线合并榜 ⇒ 14**）⇒ 全站 **17** 个榜。
        //
