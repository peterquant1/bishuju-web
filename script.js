...
const TABS_CONFIG = {
    ...
    //   09-09 01:15 加 `weeklyEma921Expansion` 变九对 ⇒ **09-09 15:3x 一次移除五个榜后
    //   只剩【三对】，而本榜占其中两对**：本榜 ⊊ `weeklySarUptrend`、本榜 ⊊
    //   `weeklyEma921Expansion`（第三对是 `weeklyTripleEma` ⊊ `weeklyEma921Expansion`）。
    //   ⚠️ 站内**唯一的两条传递包含链也随之消失** ⇒ 包含关系回到只有一层；但本榜同时挂在
    //   两个"父"下面，**别假设每个榜只有一条向上的边**。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     最多连着两天）· `monthlySarFirstBar`（月线 SAR 翻多首根，相邻两月交集恒空）
        ...
        //
        // 【结构性关系 —— 2026-09-09 15:3x 起】
        //   · **包含：三对**（九对 → 三对，六对的对家被同批移除）
        //       `weeklyEmaSarBull` ⊊ `weeklySarUptrend`
        //       `weeklyEmaSarBull` ⊊ `weeklyEma921Expansion`
        //       `weeklyTripleEma`  ⊊ `weeklyEma921Expansion`
        //     ⚠️ 站内**唯一的两条传递包含链已消失** ⇒ 包含回到只有一层；但
        //       `weeklyEmaSarBull` 同时挂在两个"父"下 ⇒ **别假设每个榜只有一条向上的边**。
        //     ⚠️ 第三对是**免费的跨实现对账**：后端 `emaExpansion`（三线）与 `ema921Expansion`
        //       是两份独立实现，差集非空即有人动了其中一处 —— **别把那两段合并成一份**。
        //   · **互斥：一对**（三对 → 一对）：`monthlyFourBull` ∩ `monthlyFiveBear`
        //     （倒1 一个要阳一个要阴，判据保证）。
        //   · **`weeklyEmaGap` 恒为正的周线榜三个**：`weeklyEmaSarBull` / `weeklyTripleEma` /
        //     `weeklyEma921Expansion`（都蕴含 9/21 扩张）；`weeklySarUptrend` **不适用**
        //     （它完全不看 EMA、可正可负）——**别互抄**。
