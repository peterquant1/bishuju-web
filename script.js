...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //   · 状态型（不破就一直在榜上）：`weeklySarUptrend`（周线 SAR 多头全集，命中最大）
        //     · `weeklyEma921Expansion`（周线 9/21 扩张全集）· `weeklyEmaSarBull`（前两者的交集）
        ...
        //     `weeklyFiveBear`（09-10 12:1x 起共五个）** + **`monthlyTwoBullExact`（当日第 6 条起共六个）** + **`monthlyBullAfterThreeBear`（第 7 条起共七个）**（`weeklyTwoBullExact` 2026-09-09 移除后一度只剩月线那两个）
        //
        // 【结构性关系 —— 2026-09-10 13:3x 起】
        //   · **包含：七对**（九对 → 09-09 15:3x 三对 → 09-10 新增 `weeklyEmaSarFirstTwo`
        //     一次带来三对，回到六对 → 13:3x 移除 `weeklyTripleEma` 带走一对 ⇒ 五对 → 第 8 条新增 `monthlyTwoBullExactVolRising` 带来一对 ⇒ 六对
        //     → **09-11 `dailySarFirstBar` 改成前两根带来一对 ⇒ 七对**）
        //       `weeklyEmaSarBull`      ⊊ `weeklySarUptrend`
        //       `weeklyEmaSarBull`      ⊊ `weeklyEma921Expansion`
        //       `weeklyEmaSarFirstTwo`  ⊊ `weeklyEmaSarBull`        （＝ 后者 ∩ SAR前两根）
        //       `weeklyEmaSarFirstTwo`  ⊊ `weeklyEma921Expansion`   （＝ 条件① 本身）
        //       `weeklyEmaSarFirstTwo`  ⊊ `weeklySarUptrend`        （"头两根"蕴含"多头"）
        ...
        //     ⚠️⚠️ **两条传递包含链随之回归**：`weeklyEmaSarFirstTwo` ⊊ `weeklyEmaSarBull`
        //       ⊊ `weeklySarUptrend` / `weeklyEma921Expansion` —— 09-09 15:3x 那句「唯一的
        //       两条传递包含链已消失、包含回到只有一层」**只成立了九个小时就作废**。
        //       ⇒ **别假设每个榜只有一条向上的边，也别假设包含只有一层。**
        ...
        //   · **互斥：十二对**（三对 → 一对 → 09-10 12:1x 新增三个周线连阳/连阴榜后四对 → 当日第 6 条新增 `monthlyTwoBullExact` 后六对 → 第 7 条新增 `monthlyBullAfterThreeBear` 后九对 → **第 8 条新增 `monthlyTwoBullExactVolRising` 后十二对（全部继承自两连阳）**），全是判据保证：
        ...
        //       `dailyEmaSarFirstTwo` ⊊ `dailySarFirstBar`**（已计入上面的包含七对）。
        //   · **周线榜七个**（09-10 12:1x 由五个变八个，13:3x 移除 `weeklyTripleEma` 后七个）/ **月线榜六个**（当日第 6 条加 `monthlyTwoBullExact`、第 7 条加 `monthlyBullAfterThreeBear`、第 8 条加 `monthlyTwoBullExactVolRising`）/ **日线榜两个**（第 9 条加 `dailySarFirstBar`）。
        //
