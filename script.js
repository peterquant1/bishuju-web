...
const TABS_CONFIG = {
    ...
    // 行 payload 与 ④ 逐字同构（共用后端 `_wk_expansion_row`）⇒ 共用同一套 15 轴
    // （日线 8 根 + 周线 7 根，正是指令③点名的「要有日线和周线级」）。
    ...
    // 的规矩（*MonthlyStrategy / weeklyStrategy 先例），继续有效。
    {
        ...
        tabs: [
            ...
            { key: "dailyFourEmaAligned", name: "日线四线多头排列", tf: "日线",
              desc: "最新已收盘日线上 EMA9 > EMA21 > EMA55 > EMA200 四条均线仍然从短到长依次排开——多头结构还立着的全部标的。注意这里只看「排列在不在」，不要求任何一档间距还在扩大，所以它是一张「趋势存续」清单而不是「趋势刚启动」清单：只要结构不破就一直在榜上，成员跨天比较稳定，这和另外四个榜（盯扩张、资金递增、SAR翻多这类当期发生的变化）性质不同。上市不足 201 个交易日、算不出 EMA200 的新合约不入榜。范围是全部 USDT 永续合约。" },
            ...
            // 用户没有任何线索能分辨宽窄。
            { key: "weeklyEmaBearish", name: "周线9/21扩张 × 周线SAR多头", tf: "周线",
              desc: "最新已收盘周线的 EMA9 在 EMA21 上方、两线间距还在扩大（结构在张开），同时周线 SAR 也站在多头一侧（圆点在价格下方）——两个门槛都在周线且同向，看的是大级别趋势确认后的延续，不是抢反弹。注意扩张这条要求严格 EMA9/21（不像上一个榜还认 9/26），所以周线EMA间距一定为正。表格里的日线八轴不参与筛选，用来在这批标的里再分日线强弱；周线七轴（周成交额/周RSI/周EMA间距/周ADX/DI）对应它自己的周期。范围是全部 USDT 永续合约。" },
        ],
    },
