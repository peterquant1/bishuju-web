...
const TABS_CONFIG = {
    ...
    // ⚠️ 09-11 16:3x + `weeklyTripleEmaPersist` 变【八个】、**09-12 08:5x 把周线四连阳与六连阳合并成 `weeklyFourToSixBull` 后【七个】**）。
    ...
    //   1 个条件（后端 `m["sarUptrend"] is False`，既有月线字段 ⇒ 零 schema）。轴集照旧、轴一根没加。
    // ⚠️ **站内命中最多的策略榜**（上线当天 469/524、占九成）—— 月线 SAR 是慢指标，一轮月线级下跌会让全市场
    //   绝大多数合约同处空头侧。**状态型**（SAR 不翻多就一直在）、月内纹丝不动。
    // ⚠️ 与 `monthlySarFirstBar` **互斥**（一个要 SAR 空头、一个要 SAR 多头）⇒ 站内互斥 live 对 两对 → 【三对】。
    monthlySarBearish: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },

    // ➕ **2026-09-12 1x:xx UTC 站长「新增：月线四到六连阳。」**（没点资产 ⇒ 加密）。追加在 rail 末尾。
    //   判据 1 个条件、全在月线：连阳根数 ∈ {4,5,6} ＝ 后端 `fourBull`（既有）∪ **`fiveBull` / `sixBull`
    //   （本轮新增 ⇒ MONTHLY_CACHE_SCHEMA 18→19）**；三支两两互斥 ⇒ 命中 ＝ 三批之和。判据口径逐字照
    //   08:5x 建的周线版 `weeklyFourToSixBull`、只换周期。
    // ⚠️⚠️ **上界没拆**：仍是「恰好」⇒ 7 连阳、8 连阳不在榜上（上线当天全市场 7 连阳及以上 0 个）。
    // ⚠️⚠️ **`monthlyFourBull` ⊊ 本榜**（共用同一个 `fourBull` 旗标）⇒ 站内包含 live 对 四对 → 【五对】；
    //   站长说「新增」不是「合并」⇒ 那张**没被取代**、两张并列。
    // ⚠️ 事件型但**相邻两月交集不恒空** ⇒「最多连着三个月」（第 4 根 → 第 5 根 → 第 6 根，第四个月必掉出；
    //   48 个月回放连庄分布实测 {1,2,3}）。⚠️ `monthlyFourBull`（单档）**仍然**恒空 —— 别互抄。
    // ⚠️ 与 `monthlySarBearish` / `monthlyVolRising` / `monthlyEma921Persist` 都不包含不互斥
    //   （上线当天实测交集 1 / 1 / 0）；与周线版两个方向都不包含（周期不同 ⇒ 不是同判据异周期孪生）。
    // ⚠️ 别为"连阳"加轴（布尔值排不了序）；轴集照旧、与本族其余各榜共用同一个 sorts 对象。
    // 📊 上线当天 **3**（恰好4 = 0、恰好5 = 2、恰好6 = 1；全市场连阳分档 {0:116, 1:259, 2:136, 3:13, 5:2, 6:1}）；
    //    48 个月回放 min 0 / p25 0 / 中位 **1** / p75 5 / max 68 / **14 个月为空**；分档 恰好4 中位 1 · max 68 /
    //    恰好5 中位 0 · max 19 / 恰好6 中位 0 · max 18；跨月延续中位 33.3%、Jaccard 16.7%；
    //    成员下个月收阳 43.3%（n=238）vs 全体 36.9%（n=12736）—— 有一点点差距但样本很小，desc 如实写了。
    ...
    //   ① 连阳根数 ∈ {2,3,4,5,6} ＝ 后端五支**恰好**旗标之并（`twoBullExact` / **`threeBull`** / `fourBull` /
    ...
    // ⚠️⚠️ **「2-6」两端都卡死**：1 连阳不入榜、7 连阳及以上也不入榜（恰好不是至少，同两张"四到六连阳"的既有口径）。
    // ⚠️ 与 `dailyEmaSarFirstTwo` / `dailySarFirstBar` **两个方向都不包含**（上线当天与两者交集都是 0、三边各有独有成员）；
    //   **不参与任何互斥**。⚠️ 站内没有「日线9/21扩张」全集榜（只有周线版）⇒ 别照周线那套关系图抄出一对包含来。
    // ⚠️ 事件型：**最多连着五天**（第 2 根 → 3 → 4 → 5 → 6 根，第六天必掉出；360 天回放连庄分布实测最大就是 5）。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //
        // 【现状分类】（**2026-09-12 1x:xx 又新增 `dailyTwoToSixBullRsiEma921`「日线二到六连阳＋RSI≥70＋9/21扩张」
        //   ⇒ 日线榜 2 → 3、加密策略榜 14 → 15、全站 17 → 18**；同日更早：移除 `weeklyFiveBear` / `monthlyFiveBear` 两张五连阴 ＋ 新增
        ...
        //     · **`monthlyFourToSixBull`（月线恰好 4 / 5 / 6 连阳三档合一，09-12 1x:xx 新增 —— 周线版的月线版，
        //       判据逐字同构只换周期）—— ⚠️ **它不恒空**：4→5→6 三个月都在榜上、**最多连着三个月**，第四个月必掉出
        //       （48 个月回放连庄分布实测 {1,2,3}、跨月延续中位 33%）。⚠️ `monthlyFourBull` ⊊ 它（共用后端 `fourBull` 旗标）**
        //     · **`dailyTwoToSixBullRsiEma921`（日线恰好 2~6 连阳 ∩ RSI≥70 ∩ 9/21 扩张，09-12 1x:xx 新增）
        //       ⇒ **最多连着五天**（第 2 根 → 3 → 4 → 5 → 6 根都在榜上，第六天必掉出；360 天回放连庄分布
        //       {1:928, 2:351, 3:134, 4:64, 5:20}，**>5 天 0 个**）；跨日延续中位 33%、中位每天只有 4 个、14/360 天为空。
        //       ⚠️⚠️ **回放里它略微跑输大盘**（次日收盘更高 40.8% vs 全体 45.5%）—— 连涨＋超买＋均线张开正是短线最热的时刻，
        ...
        //     · **`monthlySarBearish`（月线 SAR 空头，09-12 09:xx 新增；1 个条件、既有字段零 schema）—— 站内**命中最多**的策略榜
        //       （上线当天 469/524、占九成；48 个月回放中位 303.5、min 151、0 个月为空）⇒ 背景 / 回避名单而不是选股榜；
        //       跨月延续回放中位 **95.5%**、在榜段中位 8 个月，比上面两张还稳。⚠️ SAR 空头 ≠ 这个月收阴（月阳线照样可以在榜上）**
        ...
        //   · 纯 K 线形态、零指标（**三个，09-12 1x:xx 移除两张五连阴 ＋ 新增一张月线合并榜之后**）：
        ...
        //     → **1x:xx 站长「移除：周线五连阴。月线五连阴。」⇒ 前两对的对家双双下线 ⇒ 【一对】**），判据保证：
        //       `monthlySarBearish`    ∩ `monthlySarFirstBar`（09-12 09:xx 新增：一个要 `sarUptrend is False`、
        //         一个要 `is True` —— 同一根月线的 SAR 只能是其中一种 ⇒ 判据保证、上线当天实测 0）
        ...
        //     ⚠️ `dailyTwoToSixBullRsiEma921`（09-12 1x:xx）**不参与任何包含 / 互斥 / 孪生**：与 `dailyEmaSarFirstTwo`
        //       两个方向都不包含（两张共用 9/21 扩张这道门，但各自再叠的第二三道门互不蕴含；上线当天交集 0、两边各有独有成员）、
        //       与 `dailySarFirstBar` 同理；站内**没有**「日线9/21扩张」全集榜（只有周线版）⇒ 别照周线那套关系图抄出一对包含来。
        ...
        //   · **`emaGap` 恒为正的日线榜两个**（09-12 1x:xx 新增 `dailyTwoToSixBullRsiEma921` 后由一个变两个）：
        ...
        //     → 08:5x 合并 4/5/6 连阳后七个 → **1x:xx 移除 `weeklyFiveBear` 后六个**）/ **月线榜六个**
        ...
        //     ⇒ 一减一加、总数没变）/ **日线榜三个**
        //     （`dailyEmaSarFirstTwo` / `dailySarFirstBar` / **`dailyTwoToSixBullRsiEma921`（09-12 1x:xx 新增）**；
        //     09-11 06:4x 一度四个，09-12 移除 `dailyFourBull` 后两个，本轮回到三个）。
        //     ⇒ 加密策略榜共 **15** 个（09-12 那天：06:2x 由 17 变 15、08:5x 合并后 14、09:xx 新增回到 15、
        //     1x:xx 移除两张五连阴 ＋ 新增一张月线合并榜 ⇒ 14、**同日再新增这张日线榜 ⇒ 15**）⇒ 全站 **18** 个榜。
        //
        // ⚠️⚠️ **2026-09-12 1x:xx 起站内也不再有任何「连续收阴」的榜**（`weeklyFiveBear` / `monthlyFiveBear`
        //   一次移除）⇒ 连同下面那句，**一个要求阴 K 的榜都没有了**。`monthlySarBearish`「月线SAR空头」**不算**：
        ...
        tabs: [
            ...
            // ✏️✏️ **2026-09-12 1x:xx UTC 站长「日线9/21扩张＋SAR多头前四根＋成交额递增改：日线9/21扩张＋SAR多头前6根＋CVD递增」**
            //   ⇒ **一次动两个条件**：② 前四根 → **前六根**（后端新增日线旗标 `sarFifthBar` / `sarSixthBar`，
            //   对 `[:-4]` / `[:-5]` 再调 `calc_sar`；日线不走缓存 ⇒ **零 schema**）；③ 成交额递增 → **CVD递增**
            //   （`s in daily_cvd_rising_data`）。条件① 一个字没动；name 同步改、⚠️ **key 不动 ⇒ `FirstTwo` 第三次名不副实**
            //   （它是橱窗 + 默认落地榜，改 key 要连动前后端五处、并让访客 localStorage 里的"上次看的榜"失效）。
            // ⚠️⚠️ **「CVD递增」＝ 站内统一口径：当根 > 0 且 > 上一根**（站长 2026-07-22 确认过「CVD走强（递增）」含 >0，
            //   2026-07-28 只写「递增」时也是这么做的）—— 不是纯 `>`，desc 里对用户也写明了"还要在零轴上方"。
            // ⚠️⚠️ **② 放宽、③ 换了个量 ⇒ 是换人不是加人**（360 天回放新旧判据 Jaccard 中位仅 44.4%）。
            // ⚠️⚠️⚠️ **连带断掉一对结构性包含**：本榜 ⊊ `dailySarFirstBar` **不再成立** —— 本榜收到第 5/6 根、
            //   而那张**仍是前四根**（站长这次只点了本榜，没有 09-11 那句"涉及到的都改"的全站指令）⇒ 站内包含 live 对
            //   **五对 → 四对**；那张 desc 里"本榜上每一个标的都必然也在它上面"那句同批改掉了。
            //   ⚠️ `weeklyEmaSarFirstTwo`（周线版）也**仍是前四根**、没跟着改，它的 desc 同批改了。
            // ⚠️ 自检点随之改：行里 `sarBullBars` **恒 ∈ {1..6}**（原 {1,2,3,4}；⚠️ `dailySarFirstBar` 仍是 {1,2,3,4} —— **别互抄**）；
            //   「本榜 ⊆ turnoverRising 全集」作废、换成 **本榜 ⊆ CVD递增全集**；「最多连着四天」变 **最多连着六天**。
            // 📊 改判据当天 **2 → 14**（六档 1 / 1 / 3 / 1 / 3 / 5）。360 天回放：min 0 / p25 5 / 中位 **10** / p75 18 / max 214 /
            //    **2 天为空**（旧判据 8 天为空）；180 天与 90 天都 **0 天为空**（旧判据也是 0）⇒ **橱窗空榜风险是改善的**。
            //    连庄分布 {1:3077, 2:888, 3:218, 4:52, 5:15, 6:3}（**>6 天 0 个**，与「最多连着六天」吻合）。
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前六根＋CVD递增", tf: "日线",
              desc: "三个条件都在最新已收盘的那根日线上：一是 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开），二是日线的 SAR 恰好处在这一轮多头的前六根之内——从刚刚由空翻多的第一根，到翻多之后接着站住的第二到第六根都算，三是当天的 CVD 比前一天更高、而且已经在零轴上方（也就是按 K 线形状估出来的主动买盘占优，并且还在变强）。三个条件合起来找的是同一件事的早期形态：均线结构已经开始张开，按日计的趋势方向在最近一周之内才站到多头这一侧，而资金流向也同步转成了买方占优。有三处容易读错，先说清楚。第一，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。第二，前六根是恰好不是至少：已经翻多七天、十天的标的不在这张榜上，它们早就过了这个窗口；表格里的 日SAR多头根数 那一列在本榜上只会是 1 到 6，想抢第一时间的看 1，想让信号多被确认几天的看 5、6。第三，这里的 CVD 递增是两件事一起要求：既要比前一天高，也要已经在零轴上方——只是「跌得比昨天慢一点」不算。站内另有一张「日线SAR多头前四根」，它只看 SAR 这一个条件、而且只收到第四根，所以它和本榜之间没有包含关系：本榜里第五、第六根那一批不在那张榜上，那张榜里均线没张开或者 CVD 没转强的也不在本榜上，两边同时出现的标的反而值得多看一眼。另有一张周线版「周线9/21扩张＋SAR多头前四根」，前两个条件和本榜同构、只是放在周线上，而且不看 CVD、只收到第四根。这是一张事件清单不是状态清单：这天是第一根，第二天会变成第二根，一路到第六根都仍然留在榜上，第七天就必然掉出去，所以同一个标的最多连着出现六天；另外两个条件任何一天不满足也会中途掉出去。回放过去 360 天，连着出现超过六天的一个都没有，多数标的只出现一两天。命中数不多但很少断档：回放过去 360 天，每天中位 10 个左右，四分之一的日子不超过 5 个，全市场一起反弹的那天能到 214 个，360 天里只有 2 天一个都没有（最近 180 天、90 天都没有空过）。上线当天是 14 个，六档分别是 1 / 1 / 3 / 1 / 3 / 5 个。本榜是免费橱窗榜：没有通行证也能看到这张榜里排在第一位的那个标的（按日成交额排序的第一名）。另外要提醒一句，日线 SAR 是很敏感的信号，翻多之后维持不了几天就重新翻空是常事，本榜把窗口放到第六根，是在「抢第一时间」和「多等几天确认」之间取中，并不代表信号更可靠——越往后进场，离翻多那一刻的价格就越远。日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的 日EMA间距 在本榜上恒为正（那正是条件之一）；其余各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "monthlyFourBull", name: "月线四连阳", tf: "月线",
              desc: "最新已收盘的那根月线，正好是连续第 4 根阳线——这个月收阳，往前数第 2、3、4 个月也都收阳，而第 5 个月不是阳线。注意是恰好 4 根，不是至少 4 根：已经连涨 5 个月、6 个月的标的不在这张榜上，它们的最新一根是第 5 根、第 6 根而不是第 4 根。想把第四、第五、第六个月这三档一起看，请用「月线四到六连阳」那张——本榜的每一个标的都必然也在那张榜上，那张多出来的正是已经走到第五、第六个月的那些。本榜是全站少数几张纯看 K 线形态的榜之一，完全不看均线、SAR、资金流或任何指标——只问一件事：按月计的上涨，是不是刚好走到第四个月。月线是全站最大的周期，连续四个月收阳意味着这轮上涨已经跨过了「反弹」的量级、按月计的结构确实转了过来，而卡在第 4 根这个位置，找的是趋势已经立住、但还没走到人尽皆知的那一段。代价也要说清楚：月线级别的形态确认得慢，等到第四根阳线收出来，价格往往已经离底部很远了。这是一次性的事件清单不是持续清单——同一个标的只在第 4 根那个月出现，下个月它要么成了第 5 根、要么连阳已经断掉，两种情况都会让它离开本榜，所以相邻两个月的名单不会有重叠。命中数天然很少，遇到空榜是正常的：回放过去四年，多数月份只有个位数，全市场一起普涨的月份能到几十个，而大约每六个月就有一个月一个都没有——上线当天正好是这样的月份，全市场连阳的标的要么只走到第 3 根、要么已经超过 4 根，恰好卡在第 4 根的一个也没有。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 5 个月的新合约看不出「第 5 个月不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "dailySarFirstBar", name: "日线SAR多头前四根", tf: "日线",
              desc: "只看最新已收盘的那根日线：它是这一轮 Parabolic SAR 多头的前四根之一——从 SAR 刚刚由空翻多的第一根（前一天还在空头一侧），到翻多之后接着站住的第二、第三、第四根（仍在多头一侧）都算。已经连着站在多头一侧五天及以上的标的不在这张榜上，表格里的 日SAR多头根数 在本榜只会是 1 到 4：想抢第一时间的看 1，想让信号多被确认几天、滤掉翻多后很快掉头的看 3、4。多确认几天不是白送的，回放过去一年：从第一、二根进来的，第二天就翻回空头的约 4%；从第三、四根进来的约 6% 到 7%，之后这一轮多头还能再撑的天数中位也从 7、8 天缩到 6 天，5 天后的表现同样略差一点——越晚进场、离翻多那一刻的价格就越远。本榜只看 SAR 这一个条件，完全不看均线或资金流。站内另有一张「日线9/21扩张＋SAR多头前六根＋CVD递增」，它在 SAR 之外还要求均线正在张开、资金流转强，但窗口比本榜宽两根（收到第六根）——所以两张榜互相都不包含：那张里第五、第六根的标的不在本榜上，本榜里均线没张开或者 CVD 没转强的也不在那张上。同时出现在两边的，是 SAR 刚翻多、结构和资金流也同步转好的那一小撮；只在本榜上的，其中不少只是下跌途中的一次反抽，需要自己分辨。代价要说清楚：日线 SAR 是很敏感的信号，回放过去一年，翻多之后这一轮多头维持的天数中位只有 9 天左右，大约七分之一在三天之内就重新翻回空头，能撑过十天的不到一半。这是一张事件清单而不是持续清单：同一个标的最多连着四天出现在这里（翻多当天是第一根，之后每站住一天加一根），第五天起就不在了，所以名单换得很快，每天的名单里七成多是前一天就在榜上、只是往后挪了一根的。命中数随行情起伏：回放过去一年，每天中位 56 个左右，全市场一起反弹之后的那几天能到两三百个，一年里没有一天是空的。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。除了 日SAR多头根数，表格里的其余各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            // 🔴 2026-09-12 站长「移除两个TAB：周线两连阳＋价涨量增。日线两连阳＋价涨量增。」（纯移除、无替代）：
            ...
            // ➕ 2026-09-12 09:xx UTC 站长「另外新增一个TAB，逻辑是：最新收盘的月线SAR是空头就行。」（没点资产 ⇒ 加密）。
            //   追加在 rail 末尾成为加密策略组第 15 个。判据 1 个条件、全在月线：后端 `m["sarUptrend"] is False`
            //   （**既有字段 ⇒ 零 compute / 零 schema**）。「就行」＝ 站长明确只要这一条、不叠任何东西。
            // ⚠️⚠️ **后端必须写 `is False` 而不是 `not sarUptrend`**：不足 3 根已收盘月 K 时该字段是 **None**，
            //   写错会把新合约整批当空头（不报错）⇒ 门槛 ≥3 根已收盘月 K（上线当天 524 个里 5 个不入榜）。
            // ⚠️ **本榜是站内命中最多的策略榜**（上线当天 **469**）：月线 SAR 慢，一轮月线级下跌会让全市场九成合约同处空头侧
            //   ⇒ 它是**背景 / 回避名单**而不是选股榜，desc 里把这一点写在最前面，并提示用排序轴缩小范围。
            // ⚠️ SAR 空头**不等于这个月在跌**（月阳线照样可以在 SAR 空头里）⇒ **「站内不再有要求阴K 的榜」这句仍然成立**。
            // ⚠️ 与 `monthlySarFirstBar` 结构性互斥（判据保证）⇒ 互斥 live 对 两对 → 【三对】；与月线另四张（四连阳 / 五连阴 /
            //   成交量递增 / 9-21扩张存续）都能重叠（上线当天实测 0 / 2 / 279 / 3）—— K 线阴阳、成交量、EMA 都不蕴含 SAR 方向。
            // 📊 48 个月回放：命中 min **151** / p25 210 / 中位 **303.5** / p75 391 / max 470 / **0 个月为空**；
            //    占当月可判标的 min 46.0% / 中位 **89.9%** / max 95.4%；近 12 个月 354 → 469（月线级下跌行情，上线当天已接近历史最大值 470）。
            //    状态型实测：跨月延续中位 **95.5%**（min 79.2%）、Jaccard 88.5%、已结束在榜段中位 **8 个月** / p75 14 / max 30。
            //    **不是择时信号（反向也不是）**：成员下个月收盘更高 **35.2%** vs 当月可判全体 35.6% —— desc 如实写了。
            { key: "monthlySarBearish", name: "月线SAR空头", tf: "月线",
              desc: "最新已收盘的那根月线，它的抛物线 SAR 圆点在 K 线上方——也就是月线级别的 SAR 还站在空头这一侧。只有这一个条件：不看均线、不看 K 线的阴阳、不看成交量。有两件事最容易误读，先说清楚。第一，本榜命中数天然很大：上线当天 469 个，占全部有月线数据标的的九成。这不是判据写松了，而是月线 SAR 本身就是个很慢的指标——一轮月线级别的下跌会让全市场绝大多数合约同时落到空头侧。所以它更适合当成一张背景名单或回避名单用：先知道「这个标的在最大级别上还没转多」，再去别的榜里挑机会；把它当选股榜用会得到几百个标的，意义不大。真要在榜内挑，请直接用表格上方的排序轴缩小范围（比如日成交额只看流动性最好的那一批，或者用周涨跌幅看谁已经开始反弹）。第二，SAR 在空头侧不等于这个月在跌——一根月阳线完全可以出现在 SAR 空头里（圆点还在上方，只是价格开始往上够），本榜完全不问 K 线是阳是阴。回放过去 48 个月：命中中位数 303 个，最少的一个月也有 151 个（占当月可判标的 46%），48 个月里一个月都没有空过；占比中位 89.9%、最高 95.4%。近 12 个月从 354 一路升到 469，正是月线级下跌的行情特征，上线当天这个数已经接近 48 个月里的最大值 470，别把它当成常态上限。这是一张状态清单不是事件清单：SAR 不翻多就一直在榜上——上个月的成员这个月还在的比例中位数 95.5%，已经结束的在榜段中位长达 8 个月、四分之一超过 14 个月。也要如实说一句：它不是择时信号，反向也不是。回放里成员下个月收盘更高的比例是 35.2%，当月全体是 35.6%，两者几乎没有差别——月线 SAR 站在空头侧，并不额外预示下个月继续跌。它和「月线SAR多头首根＋收盘价反包所有空头SAR」永远不会出现同一个标的，因为同一根月线的 SAR 只能是多头或空头之一；反过来与「月线四连阳」「月线四到六连阳」「月线成交量递增」「月线9/21扩张存续」都可能重叠，那几张问的是 K 线阴阳、成交量和均线，与 SAR 的方向互不蕴含。另外月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，同一个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。门槛：算 SAR 至少要 3 根已收盘月 K，上市不足三个月的新合约不入榜（上线当天有 5 个因此不在）。表格里的日线、周线、月线各轴都不参与筛选。范围是全部加密 USDT 永续合约。" },
            ...
            // ➕ **2026-09-12 1x:xx UTC 站长「新增：月线四到六连阳。」**（没点资产 ⇒ 加密）。追加在 rail 末尾成为
            //   加密策略组第 14 个（同一条消息还移除了周线 / 月线五连阴两张 ⇒ 策略榜 15 → 14、全站 18 → 17）。
            //   判据 1 个条件、全在月线：连阳根数 ∈ {4,5,6} ＝ 后端 `fourBull`（既有、与 `monthlyFourBull` 共用）
            //   **∪ `fiveBull` / `sixBull`（本轮新增 ⇒ MONTHLY_CACHE_SCHEMA 18→19）**；三支两两互斥 ⇒ 命中 ＝ 三批之和。
            //   口径逐字照 08:5x 建的周线版 `weeklyFourToSixBull`、只换周期。
            // ⚠️⚠️ **上界没拆**（恰好不是至少）：7 连阳及以上不在榜上 —— 上线当天全市场连阳分档
            //   {0:116, 1:259, 2:136, 3:13, 5:2, 6:1}，7 连阳及以上 0 个。
            // ⚠️⚠️ **`monthlyFourBull` ⊊ 本榜**（共用同一个 `fourBull` 旗标）⇒ 站内包含 live 对 四对 → 【五对】；
            //   ⚠️ 站长说的是「新增」不是「改为 / 合并」⇒ 那张**没被取代**、两张并列（与 08:5x 周线那次不是同一种指令）。
            // ⚠️ 事件型但**相邻两月交集不恒空** ⇒「最多连着三个月」（4→5→6，第四个月必掉出；回放连庄分布实测 {1,2,3}）；
            //   `monthlyFourBull`（单档）**仍然**恒空 —— 别互抄。月线走缓存 ⇒ 月内成员纹丝不动。
            // ⚠️ 与 `monthlySarBearish` / `monthlyVolRising` / `monthlyEma921Persist` / `monthlySarFirstBar` 都不包含不互斥
            //   （上线当天实测交集 1 / 1 / 0 / 0）；与周线版两个方向都不包含 ⇒ **不是同判据异周期孪生**。
            // 📊 上线当天 **3**（AKE / LIT / XAN；恰好4 = 0、恰好5 = 2、恰好6 = 1；可判标的 522）；
            //    48 个月回放 min 0 / p25 0 / 中位 **1** / p75 5 / max 68 / **14 个月为空**；
            //    分档 恰好4 中位 1 · max 68 · 20 个月为 0 / 恰好5 中位 0 · max 19 · 29 个月为 0 / 恰好6 中位 0 · max 18 · 38 个月为 0；
            //    跨月延续中位 33.3%、Jaccard 16.7%；成员下个月收阳 43.3%（n=238）vs 全体 36.9%（n=12736）
            //    —— 样本太小、desc 里如实写了"撑不起一个结论"。
            { key: "monthlyFourToSixBull", name: "月线四到六连阳", tf: "月线",
              desc: "最新已收盘的那根月线，正好是连续第 4、第 5 或第 6 根阳线——按月计的连涨刚好走在第四、第五、第六个月这三档之中的一档。这是「周线四到六连阳」的月线版，判据逐字相同、只换了周期。有两处要先说清楚。第一，上界并没有拆掉：仍然是恰好不是至少，已经连涨 7 个月、8 个月的标的不在这张榜上，它们的最新一根是第 7 根、第 8 根；只连涨两三个月的也不在，那还没走到第四个月。第二，本榜只看 K 线形态，完全不看均线、SAR、资金流或任何指标——只问一件事：按月计的上涨，是不是正好落在第四到第六个月这一段。月线是全站最大的周期，连涨四个月意味着这轮上涨已经跨过了「反弹」的量级、按月计的结构确实转了过来；而卡在第四到第六个月这一段，找的是趋势已经立住、但还没走到人尽皆知的那一步，第七个月之后就自动离开本榜。站内另有一张「月线四连阳」，那是本榜第一档单独拿出来的一张——那张榜上的每一个标的都必然也在本榜上，本榜多出来的是已经走到第五、第六个月的那些。只盯刚满四个月的看那张，四到六个月一起看就用本榜。这不是持续清单，也不是只闪现一个月的清单——同一个标的最多连着出现三个月：这个月是第 4 根，下个月若再收阳就成第 5 根、仍在榜上，再一个月第 6 根也还在，第四个月必然离开（要么成了第 7 根、要么连阳已经断掉）。回放过去 48 个月，上个月的成员这个月还在的比例中位数约三分之一，其余的都是连阳断了。命中数天然很少、空榜是常态：回放过去 48 个月，中位只有 1 个，多数月份是个位数，全市场一起普涨的月份能到 68 个，而 48 个月里有 14 个月一个都没有——上线当天正好只有 3 个（走到第 5 根的 2 个、第 6 根的 1 个，卡在第 4 根的一个也没有）。三档的分布每个月都不一样，第六个月那一档本来就最少见，回放里近八成的月份是 0。最后一句要紧的：把它当择时信号要很小心。回放里成员下个月收盘更高的比例约 43%，全市场约 37%，看着高出几个百分点，但四年里总共只有 238 个样本，这点差距撑不起一个结论——它更适合当成「按月计的上涨已经成形」的筛选起点，再用表格里的各轴自己分强弱。另外月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，同一个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选。门槛方面：判「恰好」必须看得见连阳段之前那一根，所以第 4 根这档要至少 5 根已收盘月 K、第 5 根要 6 根、第 6 根要 7 根，上市不足五个月的新合约不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ➕ **2026-09-12 1x:xx UTC 站长「新增一个TAB，逻辑是；日线2-6连阳，且日线RSI大于等于70，且EMA9/21扩张。」**
            ...
            { key: "dailyTwoToSixBullRsiEma921", name: "日线二到六连阳＋RSI≥70＋9/21扩张", tf: "日线",
              desc: "三个条件都在最新已收盘的那根日线上：一是这根 K 线正好是连续第 2 到第 6 根阳线（也就是连涨两天到六天这一段），二是日线 RSI(14) 不低于 70，三是 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开）。三条合起来找的是同一件事：短线上涨已经连着走了几天、动能进了强势区、均线结构也在同步张开——典型的加速段画像。有三处要先说清楚。第一，连阳根数是恰好不是至少，而且两头都卡死：只涨了一天的不在这张榜上（还没走到第 2 根），已经连涨 7 天、8 天的也不在（它们的最新一根是第 7 根、第 8 根）。第二，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。第三，RSI 不低于 70 本身就是通常说的超买区间——这既是本榜要找的强势，也是它最大的风险，下面单独说。最要紧的一句，本榜必须如实说：它不是买入信号，回放里甚至略微跑输大盘。过去 360 天，本榜成员次日收盘更高的比例是 40.8%，而同期全市场是 45.5%，低了将近 5 个百分点。原因不难理解：连涨几天、RSI 超买、均线张开，正是短线最热的时刻，第二天回落的概率比平均更高。所以它更适合当成一张「谁在加速」的观察名单（看资金正在追什么、哪些标的走出了独立行情），而不是照着它追进场；想做回调的，反而可以拿它筛出「短线涨过头」的那一批，再自己等位置。这是一张事件清单不是持续清单：同一个标的最多连着出现五天——这天是第 2 根，明天若再收阳就成第 3 根、仍在榜上，一路到第 6 根都还在，第六天必然离开（要么成了第 7 根、要么连阳已经断掉）；另外两个条件任何一天不满足也会中途掉出去。回放里上一天的成员第二天还在的比例中位数约三分之一，连着出现超过五天的一个都没有。命中数很少、空榜是常态：回放过去 360 天，每天中位只有 4 个，四分之一的日子不超过 2 个，360 天里有 14 天一个都没有；全市场一起普涨的那天能到 152 个。上线当天是 4 个。五档的分布也不均匀：连涨两天、三天的最常见，第六天那一档最少见。表格里的 日线RSI 这一列在本榜上恒定不低于 70（那正是条件之一），可以直接用它在榜内再分强弱；其余各轴都不参与筛选。日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
        ],
    },
    // 🔴🔴 **2026-09-12 10:xx UTC 站长「A股直接移除所有TAB。我不玩A股了。需要恢复的时候我再告诉你。
