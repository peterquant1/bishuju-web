...
const TABS_CONFIG = {
    ...
    dailyDiCvdTurnoverVolRsiRising: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    weeklyEmaRsiBearish: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "dailyTwoToSixBullRsiEma921", name: "日线至少四连阳＋RSI≥70＋9/21扩张", tf: "日线",
              desc: "三个条件都在最新已收盘的那根日线上：一是这根 K 线是连续第 4 根或更靠后的阳线——至少连涨四天、不设上限，连涨八天、十天的也在；二是日线 RSI(14) 不低于 70；三是 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开）。三条合起来找的是同一件事：短线上涨已经连着走了好几天、动能进了强势区、均线结构也在同步张开——典型的加速段画像。有三处要先说清楚。第一，连阳是至少不是恰好：只连涨两天、三天的不在这张榜上（还没走到第 4 根），连涨到第 4 根之后，只要阳线不断就一直算；阳线看的是这根日线自己的收盘高于开盘，收盘正好等于开盘的平盘不算，会让连阳断掉。第二，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。第三，RSI 不低于 70 本身就是通常说的超买区间——这既是本榜要找的强势，也是它最大的风险，下面单独说。最要紧的一句，本榜必须如实说：它不是买入信号，回放里略微跑输大盘。过去 360 天，本榜成员次日收涨的比例是 43.5%（1181 个样本），同期全市场是 45.6%；5 天后收盘比入榜那天更高的比例是 38.2%，全市场 42.3%。原因不难理解：连涨好几天、RSI 超买、均线张开，正是短线最热的时刻，之后回落的概率比平均更高。所以它更适合当成一张「谁在加速」的观察名单（看资金正在追什么、哪些标的走出了独立行情），而不是照着它追进场；想做回调的，反而可以拿它筛出「短线涨过头」的那一批，再自己等位置。这是一张状态清单：连涨到第 4 根那天进来，之后阳线不断、RSI 还在 70 以上、均线还在张开就一直在，任何一条断掉就离开；回放里上一天的成员第二天还在的比例中位数约 46%，最长的连着在榜 8 天。命中数很少、空榜并不稀奇：回放过去 360 天，每天中位只有 2 个，最多 51 个，有 62 天（约六分之一）一个都没有，最长连着 10 天空榜；改判据当天（最新已收盘的是 2026 年 9 月 17 日那根日线）是 4 个。本榜是免费橱窗榜：没有通行证也能看到这张榜里排在第一位的那个标的（按日成交额排序的第一名）；碰上一个都没有的那天，橱窗自然也就没有内容可看。连阳根数的分布：第 4 根最常见（约四成），第 5 根近三成，第 6、7 根合计两成多，连涨 8 根以上的约一成。表格里的 日线RSI 这一列在本榜上恒定不低于 70（那正是条件之一），可以直接用它在榜内再分强弱；其余各轴都不参与筛选。日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            { key: "dailySarBearFirstDivergence", name: "日线SAR空头首根＋SAR点高于前一轮空头首根＋月线突破SAR空头第二根低点", tf: "日线/月线",
              desc: "三个条件：前两个落在最新已收盘的那根日线上，第三个看最新已收盘的那根月线。一是这根日线是这一轮 Parabolic SAR 空头的首根——SAR 圆点刚刚在这一天由价格下方翻到上方，前一天还在多头一侧；二是这根日线上的 SAR 圆点，比前一轮空头趋势首根那根日线上的 SAR 圆点更高；三是最新已收盘那根月线的收盘价，站上了最近一轮月线 SAR 空头段里第 2 根 K 线的最低价（和「月线突破SAR空头第二根低点」那张榜的条件一字不差）。拆开说第二条：SAR 由多翻空的那一根，圆点会直接跳到刚结束那一波上涨的最高价附近，所以「空头首根的圆点」基本就是这一波上涨的顶；拿它和前一轮空头首根的圆点比，比的是这一波的顶有没有高过上一波的顶——这一波创了更高的高点、日线趋势却翻了空，也就是这里说的背离。再拆开说第三条：在月线上找到最近一轮 SAR 处在空头（圆点在 K 线上方）的那一段，取这一段第 2 根月 K 的最低价当参照线，最新已收盘月线的收盘价高过这条线才算——也就是只在月线级别已经从最近一轮下跌里抬起头的标的里找日线上的这种背离；「最近一轮」不要求那一段已经结束，段里只有 1 根、或者可用历史里一次空头都没出现过的标的没有参照线，不入榜。有四处要先说清楚。第一，「前一轮空头」是从这根日线往回数、越过刚结束的那段多头之后遇到的那一段日线空头，取它的第一根；那一根必须是真正由多翻空的——日线只取最近约 500 根，最早那段空头的开头可能不在窗口里，这种没有可靠参照的标的不入榜。第二，两个圆点的比较是严格大于，一样高不算。第三，除了日线 SAR 和月线收盘价，本榜不看成交量和其他指标。第四，本榜的每一个标的必然都在「月线突破SAR空头第二根低点」上（第三条和那张一字不差），反过来差得很远：那张是月线门槛清单、成员常年几十到一两百个，本榜只是其中恰好这一天日线翻空、SAR 顶又高过上一轮的那几个。和「日线SAR多头前三根」正好相反：那张要求最新一根日线的 SAR 在多头一侧，本榜要求刚翻到空头，同一个标的不会同时出现在两张榜上；表格里的「日SAR多头根数」在本榜上一定显示「—」。这是一张事件清单：翻空当天出现，第二天要么继续空头成了第二根、要么翻回多头，都会离开本榜，同一个标的不会连着两天出现；月线那一条每月 1 号 00:00（UTC）新月线收盘后才换一次，同一个月之内能入选的范围不变。命中数很少，空榜很常见：回放过去 360 天，每天中位 1 个、最多 31 个，有 94 天一个都没有（约四分之一的天数）；只看前两条时每天中位 4 个、19 天空榜，加上月线这一条后大约只留下四分之一；改成现在这个判据时（最新已收盘的是 2026 年 9 月 13 日那根日线）从 12 个变成 3 个。次日去向：约 97% 第二天仍在空头（成了第二根），约 3% 翻回多头。也要如实说一句：加上月线这一条并没有让它更会挑。回放里本榜成员次日收涨的比例 46.2%（691 个样本），只看前两条时 46.5%，同期全市场 45.3%；5 天后收涨的比例 43.7%，只看前两条时 45.3%，全市场 42.1%——差距都很小，它不是做空或做多信号。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够、已收盘月线不足 3 根的算不出月线参照线，都不入榜；日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "weeklyBreakBearSarFirstBull", name: "周线突破SAR空头最新阳线收盘价", tf: "周线",
              desc: "只看最新已收盘的那根周线：它的收盘价，高过了最近一轮周线 SAR 空头段里最新一根阳线的收盘价。拆开说：先在周线上找到最近一轮 SAR 处在空头（圆点在 K 线上方）的那一段，在这一段里从最后一根往前找，遇到的第一根收阳的周 K（收盘高于开盘，也就是这轮下跌里离现在最近的一次反弹），把它的收盘价当参照线，再看最新已收盘那根周线的收盘价有没有高过这条线。有五处要先说清楚。第一，「最近一轮」不要求那一段已经结束：如果最新这根周线的 SAR 还在空头侧，参照的就是正在进行的这一轮下跌，这时最新已收盘的这一根自己不算参照——往前找的是它之前的那根阳线，所以这类命中说的是「这周收盘站上了这轮下跌里上一次反弹的收盘价」，是抢在 SAR 翻多之前的信号，回放里约 43% 的命中属于这一类；如果 SAR 已经翻多，参照的就是刚刚结束的那一段里最后一根阳线。第二，SAR 还在空头侧时，参照的那根阳线之后的周 K 都没有收阳，而周线的开盘价基本就是上一周的收盘价，所以这一类几乎只会在最新这一周收阳、而且收过参照线时出现——改成现在这个判据的这一周，这一类的 24 个全是这样。第三，收盘价正好等于开盘价的十字星不算阳线；那一段里（不算最新这一根）一根阳线都没有的，没有参照线，不入榜，也不会去更早的一段里找——这周刚刚由多翻空、这一轮空头只有这一根的，同样不入榜。第四，比较是严格大于，一样高不算。第五，本榜只看周线 K 线和周线 SAR，不看成交量、均线和其他指标。本榜上一版参照的是这一段里的第一根阳线（从头往后找），同一周是 153 个，和现在的 231 个重叠 94 个。和站内其他榜的关系：它和「周线SAR多头＋RSI≥50」「周线9/21扩张＋SAR多头」没有包含关系——回放里本榜成员有四成多周线 SAR 还在空头侧，反过来那两张也不看这条参照线；和「月线突破SAR空头第二根低点」是同一类思路（收盘价站上最近一轮空头段里的一条参照线），但周期、参照的那根 K 线和参照价都不同，两张之间也没有包含关系。这是一张门槛清单，但参照线动得比较勤：出现新一轮空头段、或者正在进行的这一段里又收出一根新的阳线（从下一周起改拿它当参照），参照线都会变，而收盘价每周都在动，所以成员会随收盘价在那条线上下进出——回放过去 105 周，上一周的成员这一周还在的比例中位数约 64%（上一版约 82%）。命中数：回放过去 105 周，每周中位 102 个、最少 5 个、最多 409 个，没有哪一周是空的；改成现在这个判据的这一周（最新已收盘的是 2026 年 9 月 7 日那一周）是 231 个，其中 24 个周线 SAR 还在空头侧，这一周的数量偏多，别当成常态。也要如实说一句：它不是择时信号。回放里成员下一周收涨的比例是 40.7%（12839 个样本），全市场 42.3%，上一版是 41.5%——本榜略差一点，差距很小。周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。门槛：算 SAR 至少要 3 根已收盘周 K，上市不足三周的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            { key: "dailyDiCvdTurnoverVolRsiRising", name: "日线+DI/CVD/成交额/成交量/RSI递增", tf: "日线",
              desc: "五个条件都看最新已收盘的那根日线，拿它和前一根日线比，五项同时变大才入榜：一是 +DI（DMI 指标里代表上涨力度的那条线，参数 14/14）比前一天高；二是 CVD 递增——本站 CVD 递增的统一口径是这根日线的 CVD 大于 0、而且比前一根大，CVD 还在 0 以下、只是往上回升的不算（CVD 大于 0 和表格里「日CVD强弱」为正是同一回事）；三是成交额（按 USDT 计）比前一天大；四是成交量（按合约标的的数量计，比如 BTC 合约就是这一天成交了多少个 BTC）比前一天大；五是 RSI（14）比前一天高。有四处要先说清楚。第一，「递增」只比最新这一根和前一根，不要求连续好几天都在变大；五项都是严格大于，一样大不算。第二，成交额和成交量是两件事：价格涨了、成交的币数却少了的时候，成交额可能变大、成交量却在变小，所以两条分开判断。第三，五个条件并不是都在起作用。回放过去 360 天，在另外四条都满足的样本里，CVD 这一条额外剔掉约三成，+DI 剔掉近两成，成交量剔掉约 3%，成交额只剔掉 2 个，RSI 只剔掉 3 个。原因是：RSI 比前一天高，其实就等于收盘价比前一天高；本站的 CVD 按每根 K 线的阴阳形状拆分买卖量，本质上也在读涨跌，CVD 递增的日子 RSI 同时递增的比例是 99.96%；而价格上涨又放量，成交额几乎必然跟着变大。这两条几乎不筛人的条件照指令保留。第四，表格里的「日+DI」「日RSI」「日CVD强弱」「日成交额」这几列显示的是水平（到了什么程度），本榜的条件只看方向（比前一天大没大），两者不是一回事——比如 RSI 从 20 升到 25 也满足条件。这是一张换得很快的逐日清单：每天都拿最新两根重新比一次，回放里前一天的成员第二天还在的比例中位只有 15%，在榜段中位 1 天、最长 6 天，八成多的在榜段只有 1 天。命中数：回放过去 360 天，每天中位 40 个左右，最少 5 个，全市场一起大涨的日子能到 351 个，没有哪一天是空的；上线这一天（最新已收盘的是 2026 年 9 月 14 日那根日线）是 63 个。也要如实说一句：它不是买入信号，回放里反而跑输大盘——成员第二天收涨的比例是 41.5%（20682 个样本），全市场 45.4%；5 天后收涨的比例 38.4%，全市场 42.2%。五项同时变大的这一天，价格已经涨过前一天，之后更常见的是回吐；它更适合当成「今天动能和成交都在放大」的观察名单，而不是追进去的理由。和站内其他榜的关系：它和「日线SAR多头前三根」「日线或周线9/21/55扩张存续」「日线至少四连阳＋RSI≥70＋9/21扩张」都没有包含关系（上线这一天分别重叠 11、42、3 个，最后这个是那张改判据前的口径）。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。门槛：要至少 29 根已收盘日线才算得出前一天的 +DI，上市不足 29 天的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            { key: "weeklyEmaRsiBearish", name: "周线阴K＋RSI≥60＋9/21扩张", tf: "周线",
              desc: "三个条件都看最新已收盘的那根周线：一是它是阴线——这一周的收盘价低于开盘价，收盘正好等于开盘的平盘周不算；二是周线 RSI(14) 不低于 60，正好等于 60 也算；三是 EMA9 在 EMA21 上方、而且两条均线的间距比上一周更大（结构还在张开）。三条合起来找的是：周线趋势已经很强、均线还在往外张，这一周却收了一根阴线——强势里的一个回调周。有三处要先说清楚。第一，阴线看的是这根周线自己的收盘低于开盘，不是和上一周的收盘比；表格里的「周涨跌幅」正是按这一周的开盘算的，所以在本榜上一定不高于 0（实体很小的阴线四舍五入后可能显示 0.00%）。第二，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站在 EMA21 上方。第三，一根阴线之后间距还能继续变大，意味着这一周的收盘价仍明显高于 EMA9——按周线看，价格还没跌回均线附近，这不等于回调已经结束。命中数很少、空榜很常见：回放过去 60 周，每周中位 2 个、最多 7 个，有 13 周一个都没有；上线这一周（最新已收盘的是 2026 年 9 月 7 日开盘的那根周线）是 7 个，正好是这 60 周里最多的一周，别把它当成常态。它换得很快：要连着两周在榜，下一周得再收一根阴线、RSI 和扩张还都得保住，回放里本周成员下一周还在榜上的比例中位数是 0——多数时候整批换人。次周去向：约三分之二下一周 RSI 仍不低于 60，六成多仍在扩张，两样都保住的约 55%。也要如实说一句：回放结论不稳定，它不是抄回调的信号。过去 60 周本榜成员下一周收涨的比例是 47.1%（102 个样本），同期全市场 40.6%，只看 RSI 与扩张两条、不论阴阳的是 42.1%；但拉长到 104 周，本榜只有 35.4%（209 个样本），下一周涨跌幅的中位数是 −8.4%，反而明显差于全市场的 41.9% 与 −2.3%——样本少，结果被一两段行情左右。和站内其他榜的关系：它和「周线四到六连阳」一定不会同时出现（那张要求最新一根周线收阳）；上线这一周的 7 个恰好也都在「周线9/21扩张＋SAR多头」「周线SAR多头＋RSI≥50」上，回放 104 周里也有约 97% 是这样，但这不是判据保证——本榜不看 SAR，有 6 次入选时周线 SAR 已经翻到空头一侧；和「周线首次9/21扩张」也互不包含（那张只收刚开始扩张的第一周，上线这一周两边没有重叠）。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。门槛：算扩张需要至少 22 根已收盘周 K（约五个月），上市不足半年的新合约不入榜。表格里的 周线RSI 这一列在本榜上一定不低于 60；各轴都不参与筛选，用来在这批标的里再分强弱。范围是全部加密 USDT 永续合约。" },
        ],
    },
