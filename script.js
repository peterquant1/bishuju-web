...
const TABS_CONFIG = {
    ...
    //   新增的三个周线连阳/连阴榜两两互斥，见 `weeklySixBull` 那组；**当日第 6 条再加 `monthlyTwoBullExact` 与本榜 / `monthlyFiveBear` 各一对 ⇒【六对】**）。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     `weeklyFiveBear`（09-10 12:1x 起共五个）** + **`monthlyTwoBullExact`（当日第 6 条起共六个）**（`weeklyTwoBullExact` 2026-09-09 移除后一度只剩月线那两个）
        //
        ...
        //   · **互斥：六对**（三对 → 一对 → 09-10 12:1x 新增三个周线连阳/连阴榜后四对 → **当日第 6 条新增 `monthlyTwoBullExact` 后六对**），全是判据保证：
        ...
        //   · **周线榜七个**（09-10 12:1x 由五个变八个，13:3x 移除 `weeklyTripleEma` 后七个）/ **月线榜四个**（当日第 6 条加 `monthlyTwoBullExact`）/ 日线榜一个。
        //
