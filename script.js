...
// ⚠️ 另三个加密榜的行**没有** weeklyRsi/weeklyEmaGap 字段（后端没发），
...
const TAB_GROUPS = [
    ...
    // 的规矩（*MonthlyStrategy / weeklyStrategy 先例），继续有效。
    {
        ...
        // ⇒ 仍是四个；被清掉那些的显示名/desc/专属轴见 git 历史）。
        // ⚠️ **刻意不套 singleStrategyGroup 工厂**：那个工厂的 name/desc 写死的是股票系
        // 的 5 个条件（月线SAR + 周线SAR + 周线扩张 + 日线扩张 + CVD），与这四个榜都不
        // 一样。硬套会让页面上的规则说明与后端实际筛选严重不符。
        // 组 tf 取第一个 tab 的周期（月线），其余五个 tab 各自覆盖。**排法：同周期的放一起**
        // （月×周×日 → 日线两个 → 周线三个）——两个日线榜相邻是刻意的：判据只差一处
        // （9/21/55 三线扩张 vs 四线排列且启动过），挨着才看得出对照。
        ...
        tabs: [
            ...
            // ⚠️ 本槽位已迭代四版，**key 一次都没动**（老规矩）：指令⑥ 建榜（只有 9/21 扩张
            // 一个条件）→ 指令⑫ 追加「日线 CVD 递增」→ 2026-07-26 站长把 EMA 那一条由两线
            // 收紧成三线 → **2026-07-28「日线9/21/55扩张＋CVD递增 这个TAB的逻辑改：日线
            // 9/21/55扩张。」** ⇒ **只剩 1 个条件**（EMA9>21>55 排列 + 两档间距都在扩大）。
            // 命中随之由 13 回到 23（＝后端 triple_ema_data 的全量）。
            ...
            // ⚠️ 名字里不再有 ＋——只有一个条件，没有东西要连接（同 weeklyEmaBearish 先例）。
            // ⚠️ 门槛仍是 **≥56 根已收盘日 K**（要算 EMA55 的当根和上一根），去掉 CVD 并没有
            // 放宽它，上市不足 56 天的新合约照旧不入本榜——判据使然，不是 bug。
            // ⚠️ **轴集一根没动**（仍 cryptoStrategySorts 10 轴）：行 payload 一个字段都没改，
            // 去掉的是 build 层的一行成员资格判断。**「日CVD强弱」那根轴照旧留着**——它是
            // 归一化的可排序值，跟被去掉的那个布尔条件不是一回事，反而是本榜现在唯一能再分
            // 「资金跟上了没有」的维度。
