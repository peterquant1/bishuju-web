...
const TABS_CONFIG = {
    ...
    // 变五个，**09-10 12:1x 再 + `weeklySixBull` / `weeklyFourBull` / `weeklyFiveBear` 变【八个】**）。
    ...
    //   `weeklyTripleEma` / `weeklyEma921Expansion` 那三个"恒为正"的抄。
    ...
    //   新增的三个周线连阳/连阴榜两两互斥，见 `weeklySixBull` 那组）。
    ...
    // ⚠️ 本榜是 `weeklyEmaSarBull` / `weeklyTripleEma` 的**母集**（两对结构性严格包含）
    //    —— 第三个子集 `weeklyEmaSarBearish` 已于 2026-09-09 15:3x 移除。
    // ⚠️ 其中 `weeklyTripleEma` ⊊ 本榜 是**免费的跨实现对账**：后端 `emaExpansion`（三线）
    //    与 `ema921Expansion` 是两份独立实现，差集非空即有人动了其中一处。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     · `weeklyTripleEma`（周线三线张开，完全不看 SAR 也不看资金）
        //   · 纯 K 线形态、零指标：`monthlyFourBull` / `monthlyFiveBear` + **`weeklySixBull` / `weeklyFourBull` /
        //     `weeklyFiveBear`（09-10 12:1x 起共五个）**（`weeklyTwoBullExact` 2026-09-09 移除后一度只剩月线那两个）
        //
        // 【结构性关系 —— 2026-09-10 起】
        ...
        //       `weeklyTripleEma`       ⊊ `weeklyEma921Expansion`
        ...
        //     ⚠️ `weeklyTripleEma ⊊ weeklyEma921Expansion` 那对是**免费的跨实现对账**：后端
        //       `emaExpansion`（三线）与 `ema921Expansion` 是两份独立实现，差集非空即有人动了
        //       其中一处 —— **别把那两段合并成一份**。
        //   · **互斥：四对**（三对 → 一对 → **09-10 12:1x 新增三个周线连阳/连阴榜后四对**），全是判据保证：
        ...
        //   · **`weeklyEmaGap` 恒为正的周线榜四个**：`weeklyEmaSarBull` / `weeklyTripleEma` /
        //     `weeklyEma921Expansion` / **`weeklyEmaSarFirstTwo`**（都蕴含 9/21 扩张）；
        //     `weeklySarUptrend` **不适用**（它完全不看 EMA、可正可负）——**别互抄**。
        //   · **`emaGap` 恒为正的日线榜只剩一个**：`dailyEmaSarFirstTwo`（也是唯一的日线榜）。
        //     ⚠️ 09-10 新增的那个是**周线**榜，不影响这一条。
        //   · **周线榜八个**（09-10 12:1x 由五个变八个）/ 月线榜三个 / 日线榜一个。
        //
        ...
        tabs: [
            ...
            { key: "weeklyEma921Expansion", name: "周线9/21扩张", tf: "周线",
              desc: "只看最新已收盘的那根周线一件事：EMA9 在 EMA21 上方，而且两条均线之间的间距比上一周更大——短周期均线正在加速甩开中周期均线，结构在张开。要注意「扩张」是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。这是站内周线组里条件最少的一张，也是那一组的母集：周线9/21扩张＋SAR多头、周线9/21/55扩张 这两张榜的成员必然全部出现在本榜里，它们各自是在「结构在张开」这个池子上再叠一道门——一个再要求趋势方向也站上多头，一个再要求 EMA55 那一档也排好。所以本榜适合当那两张的上游来用：先在这里看清楚哪些标的的周线结构正在张开，再决定要不要用更严的那两张往下收窄。它是一张状态清单不是事件清单：只要 EMA9 还在 EMA21 上方而且还在继续张开，同一个标的可以连着好几周都留在榜上，回放过去 60 周大约一半成员会跨周延续，每周命中数中位在 21 上下、最少 6 个、最多 67 个，还没有出现过空榜；上线当天的 67 个恰好就是这 60 周里最多的一周，所以别把它当成常态。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。需要至少 22 根已收盘周 K（约 5 个月）才算得出上一周的间距，所以上市不足半年的新合约不入榜。表格里的日线各轴不参与筛选，用来在这批标的里再看日线强弱；本榜的周线EMA间距 恒为正——这是判据决定的，按它降序能看出结构已经张开到什么程度，升序则是刚刚张开的那一批。范围是全部加密 USDT 永续合约。" },
            ...
            // ⚠️ 与 `weeklyTripleEma` 两个方向都不包含（实测交集 1、两侧独有 6 / 7）。
            // ⚠️ **行里 `weeklyEmaGap` 恒为正** ⇒ 恒正的周线榜由三个变【四个】；
