...
const TABS_CONFIG = {
    ...
    // ⚠️⚠️ **"事件"型 SAR 榜**（现共三个：本榜、④、⑨。⚠️ ④ 曾是本榜的"周线版"，
    // 2026-08-11 它改判成"首根"后已不是了 —— 本榜仍是"第二根"）：① 的 SAR 多头是
    // **状态**（会连着很多天为真、成员跨期稳定），本榜是**一次性事件**——每个标的一轮
    // 行情只在翻多后那一天命中 ⇒ **成员每天几乎全换、命中数天然很小**。
    // 看到它只有个位数、且和昨天完全不是同一批，那是正确行为不是数据挂了。
    // 行 payload 与其余各榜逐字同构 ⇒ **共用 cryptoStrategySorts 16 轴**。
    ...
    // 刚刚由空翻多），全在周线。"事件"型 SAR 榜之一（另两个是 ③ 与 ⑨）。
    ...
    // ⚠️⚠️ **与 ③「日线SAR多头第二根」严格互斥、交集恒为空**（倒2 不能既空头又多头，
    //   后端 90 天逐日实测）——"刚翻多那一天" vs "翻多站住一天"，站内唯一一对结构性
    //   互斥的 live 榜。**但两者不对称：③ 没有任何 EMA 条件、本榜有** ⇒ 两个榜拼不成
    //   "SAR 多头的前两根"，desc 里也是这么跟用户讲的。
    ...
    // 💡 行里 `emaGap` **恒为正**（9/21 扩张蕴含 EMA9>EMA21）⇒ 现在 ②⑦⑧⑨ 四个榜都是。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        // 新榜追加在末尾**（① 月×周×日 → ② 日线9/21扩张＋三递增 → ③ 日线SAR第二根 →
        // ④ 周线SAR第二根 → ⑤ 周线扩张＋SAR → ⑥ 月×周 SAR → ⑦ 日线三线＋CVD →
        // ⑧ 日线四线扩张；追加在末尾是为了不让整套编号位移）。
        ...
        // **① ⊆ ⑥ 是站内仅存的一对严格包含关系**，"更严 vs 更宽"、2026-08-01 起知情保留）。
        ...
        tabs: [
            ...
            // ⚠️⚠️ **与 ③「日线SAR多头第二根」严格互斥、交集恒为空**（判据保证，非巧合）。
            // 但**两者不对称**：③ 完全不看均线、本榜要 9/21 扩张 ⇒ **拼不成"多头的头两根"**，
            // desc 里必须把这一层讲明白，否则用户会把两张榜当成连着的两天去读。
            ...
            { key: "dailyEmaSarFlip", name: "日线9/21扩张＋SAR多头首根", tf: "日线",
              desc: "两个条件都看最新已收盘的那根日线：① Parabolic SAR 刚刚由空翻多，而且翻多的就是这一根——前一天还在空头一侧，今天是这一轮多头的第一根；② EMA9 在 EMA21 上方，并且两条均线的间距比前一天更大，也就是均线结构同一天也在张开。只看 SAR 翻多，每天都会冒出一批标的，其中不少只是下跌途中的一次反抽；再要求均线正在张开，留下的是方向刚转过来、结构也已经跟上的那一小撮，这就是它比单纯的翻多信号挑剔的地方。它和「日线SAR多头第二根」问的是相邻两天的事，两张榜永远不会出现同一个标的：这张要的是翻多当天，那张要的是翻多之后又站住了一天。但两张并不对称——那张完全不看均线，所以那些 SAR 已经翻多、9 与 21 却还没张开的标的只会出现在那张上，不能把两张拼起来当成一轮多头的头两根来读。要留意这是一张一次性的事件清单：同一个标的在一轮行情里只会在这一天出现，第二天起就不在了，所以成员几乎每天全换、命中数天然很小，遇到空榜也是正常的。范围是全部加密 USDT 永续合约。" },
        ],
    },
