...
const TABS_CONFIG = {
    ...
    // ⚠️⚠️ **与日线版 `dailyEmaSarFirstTwo` 是同判据异周期孪生、但两个方向都不包含**
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     最多连着两天）· **`weeklyEmaSarFirstTwo`（同一套判据的周线版，最多连着两周）**
        //     · `monthlySarFirstBar`（月线 SAR 翻多首根，相邻两月交集恒空）
        //     · `monthlyFourBull` / `monthlyFiveBear`（月线恰好 4 连阳 / 5 连阴）
        //     ⚠️ 前两个是**同判据异周期孪生**（判据逐字搬周期，显示名只差前两个字），
        //       但**两个方向都不包含** —— 日线刚翻多与周线刚翻多互不蕴含。
        ...
        //   没有 X 了』下一条指令就可能把 X 加回来」，已应验五次（08-16 阴K / 08-18 互斥 /
        //   08-19 包含 / 09-03 阴K再翻 / 09-06 同判据异周期孪生）。
        ...
        tabs: [
            ...
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前两根", tf: "日线",
              desc: "两个条件都看最新已收盘的那根日线：① Parabolic SAR 站在多头一侧，而且这一根是这一轮多头的头两根之一——要么是刚刚由空翻多的第一根（前一天还在空头一侧），要么是紧接着站住的第二根；② EMA9 在 EMA21 上方，并且两条均线的间距比前一天更大，也就是均线结构同一天也在张开。只看 SAR 翻多，每天都会冒出一批标的，其中不少只是下跌途中的一次反抽；再要求均线正在张开，留下的是方向刚转过来、结构也已经跟上的那一小撮。本榜把「翻多当天」和「次日站住」这两档放在了一起：想抢第一时间的看第一根，想让信号多被确认一天、滤掉翻多当天就掉头的假信号的看第二根——表格里的 日SAR多头根数 那一列就是分这两档用的，它在本榜只会是 1 或 2。要留意这是一张事件清单而不是状态清单：同一个标的最多连着两天出现在这里，第三天起就不在了，所以成员换得很快、命中数天然很小，遇到空榜也是正常的。另外表格里的 日EMA间距 一定是正数（判据要求均线正在张开），这是判据决定的、不是数据出错。范围是全部加密 USDT 永续合约。" },
