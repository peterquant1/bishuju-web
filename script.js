...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //
        // 【现状分类】（**2026-09-12 移除 `dailyFourBull` / `weeklyTwoBullExactCloseVolRising` 两张「两连阳＋价涨量增」
        ...
        //     · **`weeklyFiveBear`（周线恰好 5 连阴，09-10 12:1x 新增；单档 ⇒ 相邻两周交集**仍**恒空）**
        //       ⚠️ 合并前那三张都是"相邻两周交集恒空"，**09-12 起这句只对 `weeklyFiveBear` 成立** —— 别互抄。
        ...
        //   · 纯 K 线形态、零指标（**四个，09-12 08:5x 合并 4/5/6 连阳后少一个**）：`monthlyFourBull` / `monthlyFiveBear`
        //     + `weeklyFourToSixBull` / `weeklyFiveBear`（09-11 06:4x 一度九个；08:1x 移除月线两连阳 / 三连阴后首阳与日线两连阳后六个；
        ...
        //
        // 【结构性关系 —— **2026-09-12 08:5x 起**（包含仍四对、一个没动；互斥 八对 → 四对〔06:2x 那轮〕→ **两对**
        //   〔08:5x 合并 4/5/6 连阳：「六连阳 ∩ 四连阳」降级成一个榜内部两支的关系、另两条并成一条〕）】
        ...
        //   · **互斥：三对**（三对 → 一对 → 09-10 12:1x 新增三个周线连阳/连阴榜后四对 → 月线两连阳 / 三连阴后首阳 /
        ...
        //     → **09-12 08:5x 合并 4/5/6 连阳 ⇒ 两对**），全是判据保证：
        //       `monthlyFourBull`      ∩ `monthlyFiveBear`（倒1 一个要阳一个要阴）
        //       `weeklyFourToSixBull`  ∩ `weeklyFiveBear` （倒1 阳 vs 阴；60 周回放 0/60 有交集）
        ...
        //       与 `monthlyFourBull` 不包含不互斥（四连阳不看量、它不看前几根），与 `monthlyFiveBear` 可以同时命中（放量下跌）。
        ...
        //   · **周线榜七个**（09-11 08:5x 一度九个 → 09-12 06:2x 移除 `weeklyTwoBullExactCloseVolRising` 后八个
        //     → **08:5x 合并 4/5/6 连阳后七个**）/ **月线榜六个**（`monthlySarFirstBar` / `monthlyFourBull` / `monthlyFiveBear` /
        //     `monthlyVolRising` / `monthlyEma921Persist` / **`monthlySarBearish`（09-12 09:xx 新增）**；09-10 一度六个，
        //     09-11 08:1x 移除三个后三个，11:0x、17:5x 各新增一个 ⇒ 五个，本轮又加一个）/ **日线榜两个**
        //     （`dailyEmaSarFirstTwo` / `dailySarFirstBar`；09-11 06:4x 一度四个，**09-12 移除 `dailyFourBull` 后两个**）。
        //     ⇒ 加密策略榜共 **15** 个（09-12 那天：06:2x 由 17 变 15、08:5x 合并后 14、09:xx 新增本榜回到 15）。
        //
