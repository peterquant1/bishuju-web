...
const TABS_CONFIG = {
    ...
    //   任何一对结构性包含"自 ⑨ 上线起作废**（①②⑦ 各 ⊊ ⑨ 三对；**2026-09-06 加 ⑫
    //   `weeklyEmaSarBearish` 后由三对变【六对】**，它 ⊊⑦/⊊②/⊊⑨ ——别再照旧注释找）。
    ...
    //   live 榜由一对变两对**（另一对是 ⑦∩⑧ 周线，08-18）；**2026-09-06 同日两次再变**——
    //   先变三对（⑫`weeklyEmaSarBearish`∩⑧，继承自 ⑦∩⑧），同日又变**四对**
    //   （⑬`monthlyFourBull`∩⑭`monthlyFiveBear`，月线，倒1 阳 vs 阴，独立新事实不是继承）。⚠️ 本榜 desc 里当年那段讲互斥的
    ...
    //   **2026-09-06 再由两个变【三个】** —— ⑫`weeklyEmaSarBearish`（周线，多一道 9/21 门）。
    ...
    // === ⑫ 周线9/21扩张＋SAR多头＋阴K（2026-09-06 站长新增；没点资产 ⇒ 加密）===
    // 判据：3 个条件全在周线 —— ① ema921Expansion（严格 9/21，不并 9/26）② closedBearish（周收<周开）
    //   ③ sarUptrend。三个都是 get_weekly_rsi 的现成字段 ⇒ 后端零 compute/抓取/缓存 schema 改动。
    // ⚠️⚠️ 本榜一次性引入三对结构性严格包含：本榜 ⊊ ⑦weeklySarBearish（＋9/21扩张）、⊊ ②weeklyEmaSarBull
    //   （＋阴K）、⊊ ⑨weeklySarUptrend ⇒ 站内结构性包含 live 对由三对变【六对】；且 本榜⊊②⊊⑨ /
    //   本榜⊊⑦⊊⑨ 是站内第一条【传递包含链】（此前 ①②⑦⊊⑨ 三对是扁平地各自挂在 ⑨ 上）。
    // ⚠️ 与 ⑧weeklyTwoBullExact 结构性互斥（⑧ 要当根周 K 阳、本榜要阴）⇒ 互斥对由两对变【三对】；
    //   但这对是【继承】来的：本榜 ⊆ ⑦ 而 ⑦∩⑧＝∅ 早在案，不是新判据带来的独立发现。
    // ⚠️⚠️ 与 ⑩dailyEmaSarBearish 是同判据异周期孪生（三条件逐字搬到周线），但周期不同 ⇒ 互不包含。
    // ⚠️ 行里 weeklyEmaGap 恒为正（条件①）⇒ 周线 emaGap 恒正的榜由 ②⑥ 两个变【三个】；⑦⑧⑨ 仍不适用。
    // ⚠️⚠️ 但 changePercent（日涨跌幅轴）在本榜【可正可负】——别照 ⑩ 抄「恒为负」：⑩ 是日线阴K、
    //   本榜是周线阴K，一根周 K 收阴完全允许最后那个交易日收阳（实测 8 正 / 5 负）。
    // ⚠️ 别为阴K/SAR 加轴（布尔值排不了序）；18 轴照旧、与本族其余各榜共用同一个 sorts 对象。
    weeklyEmaSarBearish: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    //   （判据保证；48 个月回放 0/48 有交集）。站内互斥 live 对由三对变【四对】：⑦∩⑧、
    //   ④∩⑪、⑫∩⑧（继承自 ⑦∩⑧）、**⑬∩⑭（本轮，独立新事实不是继承）**。
    // ⚠️⚠️ **⑭ 是站内第一个纯粹筛「正在下跌」的榜**：⑦⑩⑫ 三个要求阴K 的榜**都另要求
    //   SAR 多头或 EMA 扩张**（问的是"强势里在回踩"），⑭ 一个多头条件都没有 ⇒ 读法与全站
    //   其余各榜相反（超跌/空头确认/回避名单三种读法，desc 里没替用户选）。**别照抄别的
    //   榜的措辞去改它的 desc。**
    // ⚠️ 两榜都是**纯 K 线形态、零指标**（站内此前只有 ⑧weeklyTwoBullExact 是这一类）。
    ...
    // ⭕ 2026-09-08 新增：月线突破 SAR 空头段第 2 根的低点。**状态型**（与上面三个月线榜相反），
    // 18 轴照旧、与本族其余各榜共用同一个 sorts 对象；别为「低点/突破」加轴（后端没发参照价字段，
    // 加了就是永远全 null 的幽灵轴）。
    monthlyBreakBearSecondLow: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    // ⭕ 2026-09-09 复活：周线 EMA9/21 扩张（**单条件** —— 站长「即可」＝只有这一个门）。
    // 18 轴照旧、与本族其余各榜共用同一个 sorts 对象；**别为「扩张」加 emaGap 轴**（它早就在那 18 根里）。
    // ⚠️ 本榜是 ②weeklyEmaSarBull / ⑥weeklyTripleEma / ⑫weeklyEmaSarBearish 三个周线榜的**母集**
    //    （三对结构性严格包含，详见 TAB_GROUPS 那一段）。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        // 的严格子集，见下方 ⑨ 那一段）**。
        // ⚠️ **周线榜当时是五个（②⑥⑦⑧⑨，2026-08-19 加 ⑨`weeklySarUptrend`）；09-06 加 ⑫、
        // **09-09 加 ⑯`weeklyEma921Expansion` 后现为【七个】**（⑯ 是 ②⑥⑫ 的母集，见本组末条）。②⑥⑦⑧
        ...
        // 周线榜里唯独 ⑦⑧⑨ 不适用那条自检点，别互抄。
        // ⚠️⚠️ **站内结构性包含的 live 对：2026-08-19 起三对**（全部因 ⑨ 而生：①②⑦ 各 ⊊ ⑨），
        // **2026-09-06 起【六对】**（⑫`weeklyEmaSarBearish` 再 ⊊⑦/⊊②/⊊⑨，并形成站内第一条
        // 传递包含链 ⑫⊊⑦⊊⑨ / ⑫⊊②⊊⑨）—— 别再照"三对"数。
        ...
        tabs: [
            ...
            // ⚠️⚠️ ⑫ 周线版的「回调榜」（2026-09-06 新增）—— 与 ⑩dailyEmaSarBearish 同判据异周期孪生。
            //   本榜 ⊊ ⑦（＋9/21扩张）、⊊ ②（＋阴K）、⊊ ⑨ ⇒ 三对结构性包含一次性诞生（站内当时
            //   共六对；**09-09 加 ⑯ 后九对**，且本榜也 ⊊ ⑯），
            //   且形成站内第一条传递包含链 本榜⊊⑦⊊⑨ / 本榜⊊②⊊⑨。与 ⑧ 互斥（阴 vs 阳，继承自 ⑦∩⑧）。
            // ⚠️ 站内「找回调」的榜现有【三张】：⑦（周线，最宽）· 本榜（周线，多一道均线门）· ⑩（日线）。
            { key: "weeklyEmaSarBearish", name: "周线9/21扩张＋SAR多头＋阴K", tf: "周线",
              desc: "三个条件都看最新已收盘的那根周线：① EMA9 在 EMA21 上方，而且两条均线的间距比上一周更大，也就是周线级别的均线结构正在张开；② Parabolic SAR 站在多头一侧，周线的方向也是向上的；③ 但这一根周线本身收阴——一周下来收盘价低于开盘价。合起来是「结构和方向都还好好的，这一周却在回撤」，找的是强势周线趋势里的回踩，而不是刚刚启动的那一批。它比「周线阴K＋SAR多头」多了一道均线门槛：那张只要方向还向上、这一周收阴就算，本榜还要求 9/21 已经张开，所以命中会少很多，留下的是结构确实跑起来了、只是这一周歇口气的那一小撮；和「周线9/21扩张＋SAR多头」相比，那张不挑阴阳，本榜专挑收阴的那一周。这几张榜是层层收窄的关系，本榜是其中最严的一档。要留意两件事：一是周线数据每周一才换一批，所以同一周内反复刷新看到成员纹丝不动是正常的，不是数据卡住了；二是这类回调名单天生换得快——结构和方向是能延续几周的状态，而「这一周收阴」是单周事件，下一周多半就不成立了，所以命中数天然很小、遇到空榜也正常。另外表格里的周线EMA间距 一定是正数（判据要求均线正在张开），这是判据决定的、不是数据出错；但日涨跌幅那一列正负都有——本榜要求的是整根周线收阴，最后那个交易日完全可以是红的。范围是全部加密 USDT 永续合约。" },
            ...
            // ⚠️⚠️ 两榜结构性互斥（倒1 阳 vs 阴，判据保证）⇒ 站内互斥 live 对由三对变四对。
            ...
            // ⭕ `monthlyBreakBearSecondLow`「月线突破SAR空头第二根低点」（2026-09-08 站长新增，加密第 15 个策略榜）
            // 站长口径逐字：「最新月线收盘价突破了月线最近一轮空头SAR的第二根K线的最低价。」
            // ⚠️⚠️ 与已移除的 `weeklyBreakBearSar` 同族但**三处都不同**：周线→**月线**、
            //   首根→**第 2 根**、收盘价→**最低价**。后端两个 helper（last_bear_sar_first_close /
            //   last_bear_sar_second_low）紧挨着、结构逐字相似，选错不报错、只能肉眼发现。
            // ⚠️⚠️ **本榜是「状态」型，不是「事件」型** —— 另外三个月线榜（四连阳/五连阴/
            //   SAR多头首根）都是「恰好第 N 根」⇒ 相邻两月成员交集恒空；本榜会连着好几个月留在榜上。
            //   **两类别混**（月内成员不变则是四者共同的：月线走缓存）。
            // ⚠️ **不要求当前 SAR 已翻多**（站长没写这条）—— 上线当天 **94/120 的月线 SAR 仍是
            //   空头**（「收盘价收复起点」比「SAR 翻多」来得早，同 `weeklyBreakBearSar` 当年实测的 75%）。
            // ⚠️⚠️ **退化一档（站长 2026-09-08 明确拍板保留）**：参照根恰好就是最新已收盘那根时，
            //   尺子是它自己的 low，而 `close >= low` 是 K 线定义 ⇒ **必然命中**（实测 18/18）。
            //   已写进 desc 告知用户。⚠️ 对照：`weeklyBreakBearSar` 取的是**收盘价**，同情形下
            //   `close == ref` 会被严格大于自动剔除 —— **取 close 还是取 low，恰好决定这个边界
            //   是自动剔除还是自动全中**。日后再建同族榜先想清楚这一条。
            { key: "monthlyBreakBearSecondLow", name: "月线突破SAR空头第二根低点", tf: "月线",
              desc: "最新收盘的那根月线，收盘价站上了「最近一轮月线 SAR 空头段里第 2 根 K 线」的最低价。拆开说：月线 SAR 翻空之后会连着走一段空头，这段里的第 2 根月 K 有一个最低价——本榜就拿这个价位当尺子，问最新月线的收盘价有没有站到它上面去。用第 2 根而不是翻空首根，是因为首根往往是跌幅很大的一根月线、它的低点常常是整段里最极端的一个，拿它当尺子太松；第 2 根是这轮下跌稍微站住之后的位置，站上它更能说明跌势被真正收复了一截。「最近一轮」不要求那段空头已经结束：如果月线 SAR 现在还是空头，用的就是正在走的这一段，此时收盘价站上尺子是抢在 SAR 翻多之前的领先信号——上线当天 120 个命中里有 94 个正是这种情况，也就是说本榜绝大多数标的的月线 SAR 还没翻多；如果 SAR 已经翻多，用的就是刚结束那一段，站上尺子意味着那轮下跌的开头位置已经被收回来了。本榜不要求 SAR 必须已经翻多，只问收盘价站上去了没有。有一档要注意：如果最新收盘的月线本身就是那个「第 2 根」，尺子就成了它自己的最低价——而收盘价高于当根最低价这件事对绝大多数 K 线都成立，所以这一档几乎必然入榜。上线当天有 18 个是这种情况，多数是上市才三四个月的新合约；它们不算错，但含义比其余 102 个弱得多，看到榜上有一批很新的合约就是这个原因。和月线的另外三张榜不同，本榜是「状态」不是「事件」——四连阳、五连阴、SAR 多头首根那三张，同一个标的只在某一个月出现、下个月必然离开；本榜只要收盘价还在尺子上方、而且那一轮空头仍是「最近一轮」，它就会连着好几个月留在榜上。没有尺子可用的标的不入榜，三种情况：上市不足 3 个月算不出 SAR；上市以来月线 SAR 一次都没翻过空；以及那一段空头只有 1 根月 K、根本没有第 2 根。上线当天这三种合计 21 个。另外月线数据每月 1 号（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ⭕ `weeklyEma921Expansion`「周线9/21扩张」（2026-09-09 站长复活，加密第 16 个策略榜）
            ...
            { key: "weeklyEma921Expansion", name: "周线9/21扩张", tf: "周线",
              desc: "只看最新已收盘的那根周线一件事：EMA9 在 EMA21 上方，而且两条均线之间的间距比上一周更大——短周期均线正在加速甩开中周期均线，结构在张开。要注意「扩张」是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。这是站内周线组里条件最少的一张，也是那一组的母集：周线9/21扩张＋SAR多头、周线9/21/55扩张、周线9/21扩张＋SAR多头＋阴K 这三张榜的成员必然全部出现在本榜里，它们各自是在「结构在张开」这个池子上再叠一道门——分别是趋势方向、EMA55 那一档、以及回调阴线。所以本榜适合当那三张的上游来用：先在这里看清楚哪些标的的周线结构正在张开，再决定要不要用更严的那几张往下收窄。它是一张状态清单不是事件清单：只要 EMA9 还在 EMA21 上方而且还在继续张开，同一个标的可以连着好几周都留在榜上，回放过去 60 周大约一半成员会跨周延续，每周命中数中位在 21 上下、最少 6 个、最多 67 个，还没有出现过空榜；上线当天的 67 个恰好就是这 60 周里最多的一周，所以别把它当成常态。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。需要至少 22 根已收盘周 K（约 5 个月）才算得出上一周的间距，所以上市不足半年的新合约不入榜。表格里的日线各轴不参与筛选，用来在这批标的里再看日线强弱；本榜的周线EMA间距 恒为正——这是判据决定的，按它降序能看出结构已经张开到什么程度，升序则是刚刚张开的那一批。范围是全部加密 USDT 永续合约。" },
        ],
    },
