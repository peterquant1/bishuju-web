...
const TABS_CONFIG = {
    ...
    // 本榜 · `weeklyTripleEma` · `weeklySarUptrend` · `weeklyEma921Expansion`。
    ...
    //   结构性互斥 live 对只剩【这一对】** —— 本榜与 `monthlyFiveBear`，别的都没了。
    ...
    //   ⚠️ **2026-09-09 15:3x 那三个阴K 榜被一次性全部移除** ⇒ 它现在是站内唯一涉及"连续
    //   收阴"的榜；读法与其余各榜相反（超跌/空头确认/回避名单三种，desc 里没替用户选）。
    //   **别照抄别的榜的措辞去改它的 desc。**
    // ⚠️ 两榜都是**纯 K 线形态、零指标**（同类此前还有 `weeklyTwoBullExact`，
    //   2026-09-09 15:3x 移除 ⇒ **这一类现在只剩本组这两个**）。
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     · `monthlySarFirstBar`（月线 SAR 翻多首根，相邻两月交集恒空）
        ...
        //   · 纯 K 线形态、零指标：`monthlyFourBull` / `monthlyFiveBear` 两个
        //     （`weeklyTwoBullExact` 2026-09-09 移除后这一类只剩它们）
        //
        // 【结构性关系 —— 2026-09-10 起】
        ...
        //       其中一处 —— **别把那两段合并成一份**。
        //   · **互斥：一对**（三对 → 一对）：`monthlyFourBull` ∩ `monthlyFiveBear`
        //     （倒1 一个要阳一个要阴，判据保证）。
