...
function axesSub(item, sf, volLabel, extra) {
    ...
    // 周成交额用自己的 fmtWeeklyVolVal（别套 fmtVolVal，那个写死读日成交额）。月成交额 / 月线RSI 两段随轴删了（2026-09-22），
    //   行字段照发 ⇒ 复活那两根轴要把段一起加回来（A股 休眠轴集里还挂着它俩，复活 A股 时同理）。
...
// 月线RSI / 月成交额：2026-09-22 站长从排序条删掉（加密四份轴集都不挂）；常量留给休眠的 A股 轴集与复活。
...
const TABS_CONFIG = {
    ...
    dailyEmaBreakWeeklyBearSarFirstBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    monthlyOrWeeklyOrDailyFourBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    dailyEma926SarDivergencePersist: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    dailyCvdEmaChangeRising: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    weeklyEmaSarBearishCvd: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    weeklySarDivergencePersist: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    monthlySarBearTwoBull: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "weeklyEmaSarBull", name: "周线9/21扩张＋SAR多头", tf: "周线",
              desc: "两个条件都看最新已收盘的那根周线：一是 EMA9 在 EMA21 上方、而且两条均线的间距比上一周更大（结构正在张开）；二是这根周线的 Parabolic SAR 站在多头一侧（圆点在价格下方，不管已经多头了几周）。扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；EMA 的算法和 TradingView 自带的 EMA 一致。和「周线首次9/21扩张」不同，本榜不要求这一周才刚开始扩张——已经张开好几周、这一周还在继续张开的都算；SAR 也不要求刚翻多。有三处要先说清楚。第一，两条都是拿最新这一周和上一周比（或看这一周的状态）：间距这一周不再变大、或者 SAR 翻到空头，就离开本榜；回放里这一周的成员下一周还在的比例中位约 67%。第二，9/21 扩张的标的绝大多数本来就站在 SAR 多头一侧（回放 104 周里约 84%），所以第二条筛掉的不多，主要是挡掉 SAR 已经翻空、均线还没来得及收拢的那一小批。第三，算 EMA9/21 扩张至少要 22 根已收盘周 K，所以上市不足约五个月的新合约不入榜。和站内其他榜的关系：「周线突破SAR空头最新阳线收盘价＋SAR多头」也要求周线 SAR 多头，本榜的标的绝大多数也在那张上（上线这一周 82 个里有 80 个），但不是必然——那张要在最近一轮空头段里找到一根阳线当参照，找不到就不入榜，本榜不看这一条；「周线首次9/21扩张」「周线SAR多头首根」各只管其中一半、另一半不看，和本榜都互不包含。本榜里这一周收阴、而且 CVD 为正的那一小批，另有一张「周线阴K＋9/21扩张＋SAR多头＋CVD>0」单独列出来（上线这一周 82 个里的 6 个）。命中数：回放过去 104 周（2024 年 9 月 23 日开盘的那一周到 2026 年 9 月 14 日开盘的那一周），每周中位 14 个、最少 1 个、最多 208 个，没有一周是空的，中位约占全市场的 3%。上线这一周（最新已收盘的是 2026 年 9 月 14 日开盘的那根周线）是 82 个，远高于中位——这一周普涨，站在 SAR 多头一侧的标的是这两年最多的一周，别当常态。本榜是免费橱窗榜：没有通行证也能看到这张榜里排在第一位的那个标的（按日成交额排序的第一名）——成员一周才换一次，但排序看的是每天的成交额，所以这个第一名在周内也可能换人。也要如实说一句：它不是买入信号。回放里成员下一周收阳的比例是 42.3%（2239 个样本），全市场 41.8%，基本持平；按周比，成员下一周涨跌幅的中位数跑赢全市场的周数约一半（51%）；4 周后收盘比入榜那一周更高的比例 31.0%，全市场 35.6%，4 周后涨跌幅的中位数 −13.93%（全市场 −8.36%）——周线均线正在张开的标的，往后一个月反而回落得更多。只看 9/21 扩张一条是 41.9%、30.9% 和 −13.84%，几乎一样。它更适合当成「周线趋势结构正在张开」的背景名单，配合日线各轴再挑，而不是追进去的理由。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。周线每周一 00:00（UTC）收盘后才换一批，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
            { key: "monthlySarBearish", name: "月线突破SAR空头第二根低点", tf: "月线",
              desc: "最新已收盘那根月线的收盘价，站上了最近一轮月线 SAR 空头段里第 2 根 K 线的最低价。拆开说：先在月线上找到最近一轮 SAR 处在空头（圆点在 K 线上方）的那一段，取这一段第 2 根月 K 的最低价当参照线，再看最新已收盘那根月线的收盘价有没有高过这条线。只有这一个条件。有四处要先说清楚。第一，「最近一轮」不要求那一段已经结束：如果最新这根月线的 SAR 还在空头侧，参照的就是正在进行的这一轮下跌——收盘价站上这轮下跌第 2 根的低点，是抢在 SAR 翻多之前的领先信号，改判据当天 120 个命中里有 94 个正是这一类；如果 SAR 已经翻多，参照的就是刚刚结束的那一段。第二，本榜完全不问当前 SAR 是多头还是空头（这一点和它上一版判据正相反，上一版只问 SAR 方向）。第三，有一个退化的情形：当那个参照根恰好就是最新已收盘的这根月线时，「收盘价高于自己的最低价」按 K 线定义必然成立，这类标的会自动命中（改判据当天 120 个里有 18 个属于此类）——这是刻意保留的，不是漏做。第四，参照价取的是那一段第 2 根的最低价，不是收盘价、也不是第 1 根。命中数：改判据当天 120 个（上一版是 469 个、占全市场九成，那是一张背景名单，这一版不是）。回放过去 48 个月：中位 96 个、最少的一个月 14 个、最多 184 个，48 个月里一个月都没有空过；占当月可判标的中位约27.6%，也就是四分之一上下。这是一张门槛清单：参照线只在 SAR 换段时才会变，而收盘价每个月都在动，所以成员会随收盘价在那条线上下进出——上个月的成员这个月还在的比例中位数 84%，已经结束的在榜段中位只有 2 个月，最长的一段39 个月。也要如实说一句：它不是择时信号。回放里成员下个月收盘更高的比例是 37.0%（3994 个样本），当月可判全体是37.4%，两者几乎没有差别——它标记的是「价格已经从这轮月线级下跌里抬起头」这个事实，而不是预测下个月。它和「月线SAR多头首根＋收盘价反包所有空头SAR」那张榜大多数时候会重叠（那张要求收盘价越过整段空头 SAR 的圆点，位置比本榜这条线更高，站上更高的线自然也站上了这条），但不是必然：如果那一轮空头段只有 1 根月 K，本榜就没有「第 2 根」可参照、那个标的不会出现在本榜上——改判据当天就有这样一个。另有一张「月线突破SAR空头第二根低点＋成交量递增或缩量阳K」是本榜的子集——在本榜这一条之上再要求这个月放量、或者缩量收阳，它的每一个标的都必然在本榜上，回放里每个月大约占本榜的六成多。「月线突破SAR空头第二根低点＋至少二连阳」则更窄——在本榜这一条之上再要求这个月和上个月都收阳，它的每一个标的也必然在本榜上，回放里每个月中位约占本榜的一成半。日线那张「日线SAR空头首根＋SAR点高于前一轮空头首根＋月线突破SAR空头第二根低点」的每一个标的同样必然在本榜上——它的月线那一条和本榜一字不差，只是再要求这一天日线 SAR 刚刚由多翻空、而且圆点高过前一轮空头首根的圆点，所以每天通常只有零到几个。「月线突破SAR空头第二根低点＋日线9/21扩张＋SAR多头」也一样：月线那一条和本榜一字不差，再要求最新一根日线 EMA9/21 正在扩张、日线 SAR 在多头一侧，它的每一个标的也必然在本榜上。门槛：算 SAR 至少要 3 根已收盘月 K；另外那一轮空头段必须有第 2 根，段里只有 1 根、或者可用历史里一次空头都没出现过的标的不入榜（改判据当天有 16 个因此不在）。另外月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，同一个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。范围是全部加密 USDT 永续合约。" },
            { key: "dailyEmaCvdSarTwoBreakBearDot", name: "日线9/21扩张＋SAR多头前两根＋突破或上影线触及最近一轮空头最后SAR点＋CVD递增＋RSI≥65", tf: "日线",
              desc: "五个条件都看最新已收盘的那根日线：一是 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开）；二是它正好是这一轮 Parabolic SAR 多头的第 1 根或第 2 根——SAR 圆点刚翻到价格下方不久；三是它的最高价碰到或越过了刚结束那一轮 SAR 空头段里最后一根 K 线的圆点——收盘站上它当然算，收盘没站上、但上影线摸到了也算；四是 CVD 比前一天走强；五是日线 RSI 不低于 65。拆开说第三条：SAR 空头期间圆点在价格上方、一路往下压，那一段最后一根的圆点是整段里最低的一个，也是价格翻多之前最后一道压制线；本榜只要求这根日线盘中冲到过这条线，收盘价在不在它上方都可以。有五处要先说清楚。第一，「前两根」是两个「恰好」旗标的并集——翻多当天是第 1 根、第二天是第 2 根，到第 3 根就掉出，所以同一个标的最多连着两天出现。第二，「摸到」含正好相等：最高价等于那个圆点也算；参照圆点取不到（不足 3 根、窗口内没出现过空头）的标的不入榜。第三，CVD 用的是站内一贯口径：当根为正、而且比前一天大，不是单纯看方向。第四，RSI 是 TradingView 默认的 RSI(14)，门槛含 65 本身，就是表格里「日线RSI」那一列的数，本榜每一行都不低于 65。第五，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方。和站内其他榜的关系：和「日线SAR空头首根＋SAR点高于前一轮空头首根＋月线突破SAR空头第二根低点」一定不会同时出现（那张要求最新一根日线刚翻到空头）；和「日线9/21扩张＋SAR多头首根＋最近一轮空头首根SAR点高于前一轮」都要 9/21 扩张、也都收翻多首根，但那张看的是两轮空头首根圆点谁更高，本榜看的是价格有没有冲到上一轮的最后一个圆点、还多了 CVD 和 RSI 两条，两张互不包含（改判据这一天本榜 18 个里有 2 个同时在那张上）。和「日线CVD递增＋9/21扩张＋涨幅递增」共用 9/21 扩张与 CVD 递增两条，那张另要今天涨得比昨天多、不看 SAR 和 RSI，两张也互不包含（那张上线这一天本榜 25 个里有 22 个同时在那张上）。命中数：回放过去 360 天（2025 年 9 月 26 日到 2026 年 9 月 20 日），每天中位 1 个、最多 91 个，有 77 天一个都没有（约五分之一的天数）；改判据这一天（最新已收盘的是 2026 年 9 月 20 日那根日线）是 18 个，其中翻多首根 3 个、第 2 根 15 个。五条里最卡人的是 RSI：只看前两条每天中位 6 个，加上「摸到最后一个圆点」和 CVD 剩 4 个，再加 RSI 不低于 65 只剩 1 个。和上一版比（上一版要求收盘价严格站上那个圆点、没有 RSI 条件）：上一版每天中位 2 到 3 个、47 天空榜；放宽成「上影线摸到也算」本身会多放进来一些，但 RSI 这一条砍掉的更多。也要如实说一句：它不是买入信号，这次改判据也没有让它更会挑。回放里成员次日收涨的比例是 40.2%（1107 个样本），上一版 41.8%，全市场 45.8%；5 天后收盘比入榜那天更高的比例 37.9%，上一版 37.2%，全市场 42.8%，5 天后涨跌幅的中位数 −4.31%（上一版 −3.86%，全市场 −1.44%）。拆开看两处改动：只放宽第三条、不加 RSI，是 42.5% 和 39.2%，比上一版略好；只加 RSI 不低于 65、第三条不放宽，是 40.2% 和 36.8%，比上一版差——刚翻多就已经冲得很热的标的，短线反而更容易回落。它更适合当成「刚翻多、已经冲到上一轮压制线、资金同步转强、动能偏热」的观察名单，而不是追进去的理由。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "dailyEmaBreakWeeklyBearSarFirstBull", name: "日线9/21扩张＋突破周线SAR空头首根阳线收盘价", tf: "日线/周线",
              desc: "两个条件：一是最新已收盘的那根日线 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开）；二是这根日线的收盘价，高过了最近一轮周线 SAR 空头段里首根周线阳 K 的收盘价。拆开说第二条：在周线上找到最近一轮 Parabolic SAR 处在空头（圆点在价格上方）的那一段，从这一段的第一根周 K 往后找，遇到的第一根收阳的周 K（收盘高于开盘）就是参照根，拿它的收盘价当参照线，再看最新已收盘那根日线的收盘价有没有高过这条线——用日线收盘去比周线级别的参照线，不用等这一周收盘就能看到突破。有五处要先说清楚。第一，「最近一轮」不要求那一段已经结束：周线 SAR 还在空头侧时，参照的就是正在进行的这一轮下跌，是抢在周线 SAR 翻多之前的信号；已经翻多的，参照的就是刚结束的那一段——上线这一天（最新已收盘的是 2026 年 9 月 20 日那根日线）129 个里前一类 47 个、后一类 82 个。第二，翻空的那一周如果自己就收阳，它就是参照根（上线这一天有 10 个）；收盘正好等于开盘的十字星不算阳线；那一段里一根阳线都没有的、或者可用历史里周线一次空头都没出现过的，没有参照线，不入榜，也不会去更早的一段里找。第三，比较是严格大于，一样高不算；参照线只在周线 SAR 换段、或者那一段第一次收阳时才会变，每周一 00:00（UTC）周线收盘后最多变一次，其余时候每天变的只是日线那两条。第四，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方。第五，本榜不看日线和周线的 SAR 方向，也不看成交量、RSI。和站内其他榜的关系：和「周线突破SAR空头最新阳线收盘价＋SAR多头」是同一类思路，但那张拿周线收盘去比那一段里最新一根阳线的收盘价、还要求周线 SAR 已经翻多，本榜拿日线收盘去比首根阳线的收盘价、不看 SAR 方向，两张没有包含关系（上线这一天本榜 129 个里有 80 个也在那张上）；和另外几张日线 9/21 扩张的榜也都没有包含关系。命中数：回放过去 360 天，每天中位 21 到 22 个、最少 1 个、最多 138 个，没有一天是空的，大约是当天全部 9/21 扩张标的的四成多；上线这一天是 129 个，接近这 360 天里的最高，因为这一天全市场有 314 个标的日线 9/21 在扩张（平时中位 47 个），别当常态。前一天的成员第二天还在的比例中位约 67%。也要如实说一句：它不是买入信号。回放里成员次日收涨的比例是 44.5%（9625 个样本），全市场 45.9%；5 天后收盘比入榜那天更高的比例 39.8%，全市场 42.9%——都低于大盘；只看 9/21 扩张、不管参照线是 44.1% 和 40.0%，加上「站上周线参照线」这一条几乎没有差别。它更适合当成「日线结构正在张开、价格已经越过周线级别下跌里第一次反弹的收盘价」的观察名单，而不是追进去的理由。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。门槛：日线要至少 23 根已收盘日 K、周线要至少 3 根已收盘周 K，所以上市不足 23 天的新合约不入榜；日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "dailyEma926SarDivergencePersist", name: "日线9/26金叉存续＋最近一轮空头首根SAR点高于前一轮", tf: "日线",
              desc: "两个条件都看最新已收盘的那根日线，而且都是「还在不在」的状态，不是「刚刚发生」的事件：一是 EMA9 在 EMA26 上方——9/26 金叉之后还没有死叉；二是最近一轮 Parabolic SAR 空头段首根日线上的 SAR 圆点，比再前一轮空头首根的圆点更高。拆开说第二条：SAR 由多翻空的那一根，圆点会直接跳到刚结束那一波上涨的最高价附近，所以「空头首根的圆点」基本就是那一波上涨的顶；拿最近一轮和再前一轮比，比的是最近一个波段高点有没有高过前一个波段高点。这个比较在那一轮空头开始的那天就定下来了，之后不管 SAR 翻回多头还是继续空头都不变，要等下一轮空头出现才重新比一次——这就是这里说的「存续」。两条合起来找的是：日线 EMA9 还在 EMA26 上方、最近一次见顶又比上一次高的标的，也就是日线级别的上升结构还没被破坏的那一批。有五处要先说清楚。第一，「最近一轮空头」包括正在进行的这一轮：最新这根日线的 SAR 还在空头侧（价格正在回调）时，比的就是这一轮回调开始那天的圆点，所以本榜不看当前 SAR 是多头还是空头——表格里「日SAR多头根数」显示「—」的就是正在回调的，上线这一天 282 个里有 83 个，回放里平均约占一半。第二，两个「空头首根」都必须是真正由多翻空的那一根——日线只取最近约 500 根，最早那段空头的开头可能不在窗口里，这种没有可靠参照的标的不入榜。第三，两处比较都是严格大于，一样高不算。第四，第一条只看最新这一根日线上两条均线的上下位置，不要求在可用历史里看得到那次金叉——新币上市后 EMA9 一直在 EMA26 上方的也算；EMA 的算法和 TradingView 自带的 EMA 一致。第五，本榜不看 EMA9/21 是否扩张，也不看成交量、RSI 和 CVD。和站内其他榜的关系：「日线9/21扩张＋SAR多头首根＋最近一轮空头首根SAR点高于前一轮」比的也是这一对圆点——那张只在 SAR 重新翻多的那一天比，本榜从这一轮空头开始的那天起就一直记着；那张的标的绝大多数也在本榜上，但不是必然：9/21 扩张推不出 EMA9 在 EMA26 上方，回放 360 天里那张累计 1196 个成员（按天计）中有 49 个当天不在本榜。「日线SAR空头首根＋SAR点高于前一轮空头首根＋月线突破SAR空头第二根低点」的前两条，就是本榜第二条在那一轮空头开始那天成立，所以那张的标的只要日线 EMA9 也在 EMA26 上方，就一定在本榜上；但那张不要求这一条、本榜不看月线，两张互不包含。「周线最近一轮空头首根SAR点高于前一轮」比的是同一对圆点、但看的是周线（本榜看日线），而且不要求 EMA9 在 EMA26 上方，两张互不包含：上线这一天本榜 301 个里有 76 个同时在那一张上。这是一张状态清单：两条都不破就一直在——回放里前一天的成员第二天还在的比例中位约 94%，已经结束的在榜段中位 9 天、最长 106 天；离开本榜的，约四分之三是 EMA9 跌回 EMA26 下方，约五分之一是新一轮空头开始、首根圆点没能高过上一轮（最近一个高点没创新高），其余是两条同一天破。命中数：回放过去 360 天（2025 年 9 月 26 日到 2026 年 9 月 20 日），每天中位 60 个、最少 20 个、最多 325 个，没有一天是空的，中位约占全市场的 12%；只看第一条每天中位 97 个，只看第二条中位 191 个。上线这一天（最新已收盘的是 2026 年 9 月 20 日那根日线）是 282 个，占全市场 523 个的一半多，接近这 360 天里的最高（最多的几天在 2026 年 5 月中和 9 月上旬），是这几天普涨后的高位，别当常态。也要如实说一句：它不是择时信号。回放里成员次日收涨的比例 47.0%（31726 个样本），全市场 45.8%；5 天后收盘比入榜那天更高的比例 45.5%，全市场 42.8%，5 天后涨跌幅中位数 −1.01%（全市场 −1.44%）；只看第一条是 45.9% 和 44.3%，只看第二条是 46.4% 和 43.0%。合在一起算略好于大盘，但按天比，成员次日涨跌幅的中位数跑赢当天全市场的只有 46% 的天数，而且成员天天大多是同一批、样本高度重叠，这点差距撑不起结论。它更适合当成「日线上升结构还没坏」的背景名单，配合别的榜或表格里的各轴再筛。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。上市不足 26 天的新合约算不出 EMA26，不入榜。范围是全部加密 USDT 永续合约。" },
            { key: "dailyCvdEmaChangeRising", name: "日线CVD递增＋9/21扩张＋涨幅递增", tf: "日线",
              desc: "三个条件都看最新已收盘的那根日线：一是 CVD 比前一天走强；二是 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开）；三是这根日线的涨幅比前一天大——今天收涨，而且涨得比昨天多。涨幅和表格里「日涨跌幅」那一列是同一个算法：这根 K 线收盘相对开盘涨了多少，比的是今天和昨天这两根。有五处要先说清楚。第一，CVD 用的是站内一贯口径：当根为正、而且比前一天大，不是单纯看方向。第二，第一条已经保证今天收阳——CVD 是按 K 线形态拆出买量和卖量再平滑的，当根为正又比前一天大，只可能发生在收阳的那天——所以第三条里「今天收涨」在本榜永远成立，第三条真正筛掉的只有「今天涨得没昨天多」的那些。第三，昨天可以是跌的：昨天跌 3%、今天涨 2% 也算涨幅递增；如果要求两天都涨、而且一天比一天多，回放里每天中位只剩 11 个，之后的表现也没有更好。第四，几处比较都是严格大于，一样大不算。第五，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；EMA 的算法和 TradingView 自带的 EMA 一致。三条都是拿最新两根日线比，所以这是一张天天大换血的清单：回放里今天的成员第二天还在的比例中位只有 12%。和站内其他榜的关系：「日线9/21扩张＋SAR多头前两根＋突破或上影线触及最近一轮空头最后SAR点＋CVD递增＋RSI≥65」和本榜共用 9/21 扩张与 CVD 递增两条，其余条件各不相同，两张互不包含（上线这一天那张 25 个里有 22 个也在本榜上）。命中数：回放过去 360 天（2025 年 9 月 27 日到 2026 年 9 月 21 日），每天中位 21 个、最少 1 个、最多 307 个，没有一天是空的，中位约占全市场的 4%；只看前两条（CVD 递增＋9/21 扩张）每天中位 28 个，第三条每天留下其中的八成左右。上线这一天（最新已收盘的是 2026 年 9 月 21 日那根日线）是 307 个，占全市场 527 个的一半多，也是这 360 天里最多的一天——普涨日会一次进来大半个市场，别当常态。也要如实说一句：它不是买入信号，加上「涨幅递增」也没有让它更会挑。回放里成员次日收涨的比例是 44.0%（12961 个样本），全市场 45.7%；5 天后收盘比入榜那天更高的比例 39.6%，全市场 42.3%，5 天后涨跌幅的中位数 −2.50%（全市场 −1.55%）；只看前两条是 43.9%、39.7% 和 −2.46%，几乎一样；按天比，成员次日涨跌幅的中位数跑赢当天全市场的只有 36% 的天数。它更适合当成「资金、均线、涨幅同时在加速」的观察名单，而不是追进去的理由。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            { key: "weeklyEmaSarBearishCvd", name: "周线阴K＋9/21扩张＋SAR多头＋CVD>0", tf: "周线",
              desc: "四个条件都看最新已收盘的那根周线：一是它收阴（收盘低于开盘）；二是 EMA9 在 EMA21 上方、而且两条均线的间距比上一周更大（结构正在张开）；三是这根周线的 Parabolic SAR 站在多头一侧；四是这根周线的 CVD 为正。合起来找的是「周线结构还在张开、SAR 还在多头，但这一周收阴」的回踩型标的。有五处要先说清楚。第一，条件二三和「周线9/21扩张＋SAR多头」那张一字不差，所以本榜的标的必然也在那张上——本榜就是在那张里再挑出「这一周收阴、而且 CVD 为正」的一小批（回放里那张每周中位 14 个，本榜剩 3 个）。第二，阴 K 是严格的收盘低于开盘，收平不算。第三，CVD 只看正负、不要求比上一周大：站内别处说「CVD 递增」时还含「比上一根大」，本榜没有这一条。第四，这一条几乎不筛人——回放里「结构张开＋SAR 多头＋收阴」的标的有 97% 的 CVD 本来就是正的；CVD 是按周 K 形态拆买卖量再平滑出来的，不是真实成交归边，要 15 根已收盘周 K 才算得出来。第五，算 EMA9/21 扩张要 22 根已收盘周 K，所以上市不足约五个月的新合约不入榜。命中数：回放过去 104 周（2024 年 9 月 23 日开盘的那一周到 2026 年 9 月 14 日开盘的那一周），每周中位 3 个、最多 159 个，有 17 周一个都没有（约六分之一的周）——空榜是正常的，不是数据坏了。上线这一周（最新已收盘的是 2026 年 9 月 14 日开盘的那根周线）是 6 个。也要如实说一句：它不是买入信号，而且回放里明显跑输大盘。成员下一周收阳的比例是 36.1%（640 个样本），全市场 41.8%；下一周涨跌幅的中位数 −5.94%，全市场 −2.42%；4 周后收盘比入榜那一周更高的比例 28.9%，全市场 35.6%，4 周后涨跌幅的中位数 −14.11%（全市场 −8.36%）。拿它和父榜比更直接：同样是均线在张开、SAR 在多头，父榜整体是 42.3% 和 31.0%，而这一周收阴的这一批之后明显更差——「趋势里回踩一周就是买点」这个说法，在回放里不成立。它更适合当成观察名单：看这批回踩的标的下一周是接着跌，还是重新站回去。另外它换血很快：这一周的成员下一周还在的比例中位只有 3%（同一个标的连着两周收阴本来就少见）。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。周线每周一 00:00（UTC）收盘后才换一批，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
            { key: "weeklySarDivergencePersist", name: "周线最近一轮空头首根SAR点高于前一轮", tf: "周线",
              desc: "只看周线的 Parabolic SAR 圆点，一个条件：最近一轮 SAR 空头段首根周线上的圆点，比再前一轮空头首根的圆点更高。拆开说：SAR 由多翻空的那一根，圆点会直接跳到刚结束那一波上涨的最高价附近，所以「空头首根的圆点」基本就是那一波上涨的顶；拿最近一轮和再前一轮比，比的就是周线级别最近一个波段高点有没有高过上一个波段高点——高过了才入榜。这个比较在那一轮空头开始的那一周就定下来，之后不管 SAR 翻回多头还是继续空头都不会变，要等下一轮空头出现才重新比一次，所以它是「还在不在」的状态、不是「刚刚发生」的事件。有五处要先说清楚。第一，「最近一轮空头」包括正在进行的这一轮：最新这根周线的 SAR 还在空头一侧（周线正在回调）时，比的就是这一轮回调开始那一周的圆点，所以本榜不看当前周线 SAR 是多头还是空头——上线这一周 139 个里，62 个的周线 SAR 在多头、77 个正在空头里。反过来说，新进成员几乎都是在 SAR 刚翻空的那一周进来的（回放里 242 个新进成员无一例外）。第二，两个「空头首根」都必须是真正由多翻空的那一根：上市初期那种从第一根周 K 直接起算、圆点就是第一根最高价的「播种」空头不算数，这种没有可靠参照的标的不入榜——上线这一周全市场 527 个里，有 34 个连一轮有效的空头都找不到、81 个只有一轮（没有可比的前一轮）。周线取 499 根，对现有合约就是全部历史，和 TradingView 上看到的一样。第三，比较是严格大于，一样高不算。第四，本榜只看这一对圆点：不看均线、RSI、成交量、CVD，也不看收盘价站在哪里。第五，算 SAR 至少要 3 根已收盘周 K，上市不足三周的新合约不入榜。命中数：回放过去 104 周（2024 年 9 月 23 日开盘的那一周到 2026 年 9 月 14 日开盘的那一周），每周中位 112 个、最少 36 个、最多 139 个，没有一周是空的，中位约占全市场的四分之一（最低的一周 9%、最高的一周 62%）——这是一张范围很宽的背景榜，不是少数标的的精选。上线这一周（最新已收盘的是 2026 年 9 月 14 日开盘的那根周线）是 139 个，占全市场 527 个的 26%，也是这 104 周里最多的一周。换血很慢：比较只在新一轮周线空头开始时才重做一次，回放里这一周的成员下一周还在的比例中位是 100%（103 个可比周里有 53 周一个都不掉，掉得最多的一周 57 个；每周新进中位 1 个、最多 47 个），已结束的在榜段中位 22 周、最长 92 周。也要如实说一句：它不是买入信号，回放里和大盘基本持平。成员下一周收阳的比例 42.3%（10083 个样本），全市场 42.4%；下一周涨跌幅的中位数两边都是 −2.19%；4 周后收盘比入榜那一周更高的比例 37.4%，全市场 36.6%，4 周后涨跌幅的中位数 −7.41%（全市场 −7.60%）——4 周这一档略好于大盘，但成员每周大多是同一批、样本高度重叠，这点差距撑不起结论（按周比，成员次周涨跌幅的中位数跑赢当周全市场的周数占 60%）。它更适合当成「周线级别的高点还在往上抬」的背景名单，配合日线各轴或别的榜再筛。和站内其他榜的关系：「日线9/26金叉存续＋最近一轮空头首根SAR点高于前一轮」比的是同一对圆点，但看的是日线、而且另要求 EMA9 在 EMA26 上方，和本榜互不包含（上线这一天那张 301 个里有 76 个同时在本榜上）；其余几张周线榜看的是均线扩张、SAR 方向或者收盘价有没有站上某条参照线，判据和本榜不相干，也都互不包含（上线这一周本榜 139 个里，23 个同时在「周线9/21扩张＋SAR多头」上、54 个在「周线突破SAR空头最新阳线收盘价＋SAR多头」上、4 个在「周线SAR多头首根」上）。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。周线每周一 00:00（UTC）收盘后才换一批，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
            { key: "monthlySarBearTwoBull", name: "月线SAR空头＋两连阳", tf: "月线",
              desc: "两个条件都看最新已收盘的那根月线：一是这根月线的 Parabolic SAR 站在空头一侧（圆点在价格上方，不管这一轮空头已经走了几个月）；二是它正好是连涨的第 2 根——这根和上一根都收阳，再往前那一根不是阳线。合起来找的是：月线级别还在 SAR 下跌趋势里、但刚刚连着收了两根月阳线的标的，也就是大周期还没转向、价格已经反抽了两个月的那一批。有五处要先说清楚。第一，「两连阳」是恰好两根：第三个月再收阳，它就成了三连阳、离开本榜；而且不管下个月收阴还是收阳，原来那根「再往前不是阳线」的位置都换成了阳线，所以同一个标的不会连着两个月出现在本榜上——回放 48 个月里相邻两月的交集是 0。第二，SAR 空头不看第几根：这个月刚翻空的、和已经空头一年的都算；本榜也不管收盘价站在哪条参照线的上方。第三，阳线是严格的收盘高于开盘，收盘正好等于开盘的十字星不算阳线，而且会打断连阳。第四，本榜只看月线 SAR 的方向和月 K 的阴阳，不看均线、RSI、成交量和 CVD。第五，门槛是至少 3 根已收盘月 K，上市不足三个月的新合约不入榜。命中数：回放过去 48 个月，每月中位只有 4 个、最多 129 个，有 7 个月一个都没有（约七分之一的月份）——空榜是正常的，不是数据坏了；中位约占当月可判标的的 1.6%。两条里真正卡人的是「恰好两连阳」：单看 SAR 空头每月中位 160 个，单看恰好两连阳每月中位只有 7 个。上线这个月（最新已收盘的是 2026 年 8 月那根月线）是 129 个，正好是这 48 个月里最多的一个月——这个月全市场 519 个可判标的里，469 个月线 SAR 在空头侧、136 个恰好走到连涨第 2 根，两头都偏多，别当常态。也要如实说一句：它不是买入信号。回放里成员下个月收盘更高的比例是 43.3%（617 个样本），当月可判全体是 37.4%；下个月涨跌幅的中位数 −4.34%，全体 −7.24%；3 个月后收盘更高 35.7%，全体 31.2%，中位 −15.51%（全体 −17.38%）——三档都比大盘略好，但按月比，成员下个月涨跌幅的中位数跑赢当月全体的月份只有 48%（不到一半），而且每月中位只有 4 个标的、样本一共六百多个，这点差距撑不起结论。它更适合当成「大周期还在下跌、但已经反抽两个月」的观察名单。和站内其他榜的关系：和「月线SAR多头首根＋收盘价反包所有空头SAR」一定不会同时出现（那张要求月线 SAR 刚刚翻多，本榜要求还在空头）；三张「月线突破SAR空头第二根低点」系的榜都不看 SAR 方向、看的是收盘价有没有站上那条参照线，和本榜没有包含关系（上线这个月本榜 129 个里有 26 个同时在那三张上）——其中「＋至少二连阳」那张要的是至少两连阳，本榜要的是恰好两连阳，所以本榜里站上参照线的那一部分必然也在它上面，反过来不成立。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，整个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
        ],
    },
...
const TEASER_TAB = "weeklyEmaSarBull";
...
// TF_SHORT：tf → rail 徽标缩写，查不到时徽标静默消失（跨周期榜要在这里补，两个字及以上自动套 .tf-chip--wide）。tf 纯展示用。
const TF_SHORT = { "日线": "日", "周线": "周", "月线": "月", "日线/周线": "日周", "日线/月线": "日月", "日线/周线/月线": "日周月" };
...
const lastTabByAsset = { crypto: "weeklyEmaSarBull" };
...
function switchAsset(assetK) {
    ...
    const fallback = assetK === "ashare" ? "ashareMonthlyWeeklyDaily" : "weeklyEmaSarBull";
...
const RESO_CARDS = [
    ...
    { tab: "monthlyChange", tf: "月", sorts: monthlyChangeSorts, volKey: "monthlyVolume", rankKey: "monthlyVolumeRank",
      top: 10, min: 2, wide: false, period: "本月", refresh: "月线每月 1 号 00:00 UTC 换一批" },
];
