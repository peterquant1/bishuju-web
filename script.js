...
// （2026-09-04 +日SAR多头根数）→ **19（2026-09-11 +周成交额、+月成交额）**。
...
// 加密**全部策略榜共用**的轴集，**20 根**（2026-07-30 站长「能否对齐轴数？以最多的
// 为准」＋ 2026-08-12「所有升降序也新增一个涨幅升降序」＋ 2026-09-04「日线SAR翻多后的
// 新鲜度」＋ 2026-09-11「也要有周成交额…月成交额和TOP几也要有」之后的现状）：日线 12 根（日成交额·
// 日涨跌幅·日线RSI·日CVD强弱·日订单流·日量比·日EMA间距·日ADX·日+DI·日MACD强弱·
// 日波动幅度·日SAR多头根数）+ 周线 6 根（周成交额·周线RSI·周线EMA间距·周ADX·周+DI·周MACD强弱）
// + 月线 2 根（月成交额·月线RSI）。与后端唯一的行构造 `_strategy_row` 严格一一对应。
...
const TABS_CONFIG = {
    ...
    //   上线前实测公开顶层/paidMeta/付费镜像三处均无残留，复用安全。别为"连阳"加轴（布尔值排不了序）；轴集照旧、共用同一个 sorts。
    // ✏️ **2026-09-11 08:5x UTC 站长「日线四连阳改为日线2连阳且价涨量增。」** ⇒ 判据改成 3 条（都在日线）：恰好 2 连阳（倒3 非阳）
    //   ∧ 价涨（倒1 收盘 > 倒2 收盘，close-to-close）∧ 量增（倒1 成交量 k[5] > 倒2）；name 改「日线两连阳＋价涨量增」；
    //   ⚠️⚠️ **key 不动 ⇒ `FourBull` 已完全名不副实**（改判据 key 不动的老规矩），读到它别去找四连阳。上面三行按建榜时读。
    //   ⇒ 不再算「纯 K 线形态」（看成交量）；与下面新增的周线版是**同判据异周期孪生**（两个方向都不包含）。
    ...
    // ➕ 2026-09-11 08:5x UTC 同一条消息「另外新增一个TAB，逻辑是：周线2连阳且价涨量增。」（没点资产 ⇒ 加密）＝ 上面
    //   `dailyFourBull` 改判据后的**周线版**（同判据异周期孪生）：恰好 2 连阳 ∧ 价涨（close-to-close）∧ 量增（k[5]），三条都在周线。
    //   后端新增周线字段 `closeRising` / `volRising` ⇒ WEEKLY_CACHE_SCHEMA 21→22；条件① 用既有字段 `twoBullExact`。
    // ⚠️⚠️ **不是 09-09 移除的 `weeklyTwoBullExact`「周线两连阳第二根」的复活**（那个只有形态一条）⇒ 全新 key（三处均无残留）。
    // ⚠️⚠️ 与 `weeklySixBull` / `weeklyFourBull` / `weeklyFiveBear` **两两结构性互斥**（倒3 非阳 vs 阳 / 倒1 阳 vs 阴）⇒ 互斥四对 → 七对。
    // ⚠️ 事件型：相邻两周交集恒空；周线走缓存 ⇒ 周内成员纹丝不动。别为"连阳 / 价涨 / 量增"加轴；轴集照旧、共用同一个 sorts。
    weeklyTwoBullExactCloseVolRising: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
    ...
    // ⚠️ 不看价格方向（放量上涨、放量下跌都在榜上）；与各榜无包含 / 互斥 / 孪生；不计入纯 K 线形态（不看阴阳）。
    // ⚠️ 既不是事件型也不是状态型：下个月约三分之一的成员还在；月线走缓存 ⇒ 月内成员纹丝不动。命中很宽（上线当天 303）。
    ...
    // ✏️⚠️ **2026-09-11 13:2x 站长改判据「月线成交量递增且必须是月线阳K」**（key 不动、名改「月线成交量递增＋阳K」）：
    //   ② 该月 K `close > open`（后端比缓存里未 round 的 raw 值）⇒ 上面「不看价格方向 / 与各榜无互斥」**作废**：
    //   与 `monthlyFiveBear` 结构性互斥（倒1 阳 vs 阴）；当天 303 → 253；回放中位 37.5、1 个月为空；次月留存中位约 14%（上面「约三分之一」作废）。
    //   sorts 一根没动。
    ...
    weeklyTripleEmaPersist: { sorts: cryptoStrategySorts, subFormat: (v, sf) => axesSub(v, sf, "日成交额") },
...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //
        // 【现状分类】（2026-09-11 17:5x 新增 `monthlyEma921Persist` 之后；16:3x 新增 `weeklyTripleEmaPersist`；13:2x `monthlyVolRising` 追加「月线阳K」改名「月线成交量递增＋阳K」；11:0x 新增它；更早 08:5x `dailyFourBull` 改判据 ＋ 新增 `weeklyTwoBullExactCloseVolRising`）
        //   · 事件型（成员换得快）：`dailyEmaSarFirstTwo`（日线 SAR 翻多**前四根**＋9/21 扩张
        //     ＋成交额递增〔09-10 追加〕，**最多连着四天**）· **`weeklyEmaSarFirstTwo`（周线 SAR
        //     翻多**前四根**＋9/21 扩张，**最多连着四周**）**
        //     ✏️ 09-11 14:xx 站长「涉及到SAR多头前2根的都改为SAR多头前4根」⇒ 这两张与下面 `dailySarFirstBar` 由前两根同批改前四根（key 都没动）。
        //     · `monthlySarFirstBar`（月线 SAR 翻多首根**＋收盘价反包所有空头SAR**〔09-10 12:1x 追加〕，相邻两月交集恒空）
        //     · `monthlyFourBull` / `monthlyFiveBear`（月线恰好 4 连阳 / 5 连阴）
        ...
        //     · **`dailyFourBull`（09-11 06:0x 建时是日线恰好 4 连阳；✏️ 08:5x 改成「日线恰好 2 连阳＋价涨量增」、key 没改；相邻两日交集恒空）**
        //     · **`weeklyTwoBullExactCloseVolRising`（周线恰好 2 连阳＋价涨量增，09-11 08:5x 新增；相邻两周交集恒空）**
        //       ⚠️ 这两张是**同判据异周期孪生**（逐字同三条、只换周期；两个方向都不包含）⇒ 下面那句「现无孪生」作废。
        ...
        //     · **`weeklyTripleEmaPersist`（周线三线扩张存续：仍 EMA9>21>55 且这段排列扩张过，09-11 16:3x 新增；跨周延续回放中位 100%）**
        //     · **`monthlyEma921Persist`（月线 9/21 扩张存续：仍 EMA9>21 且这段排列扩张过，09-11 17:5x 新增；两线版 ≈「月线 EMA9 在 EMA21 上方」、只差窗口边界；跨月延续回放中位 91%）**
        //   · **逐月比较型（既不是事件型也不是状态型）**：`monthlyVolRising`（月线成交量 > 上个月 **＋ 该月收阳**〔13:2x 追加〕，09-11 11:0x 新增）——
        //     每月拿新一根和上一根比 ⇒ 相邻两月交集**不**恒空（改判据前回放中位 36%、13:2x 起中位约 14% 的成员下个月还在），也不会「不破就一直在」。
        //     ⚠️ 11:0x 写的「现役榜里唯一**完全不看价格**的一张」**13:2x 起作废**（追加了阳K）—— 全称句只成立了约两个半小时，现役已没有完全不看价格的榜。
        //   · 纯 K 线形态、零指标（**五个**）：`monthlyFourBull` / `monthlyFiveBear` + `weeklySixBull` / `weeklyFourBull` /
        //     `weeklyFiveBear`（09-11 06:4x 一度九个；08:1x 移除月线两连阳 / 三连阴后首阳与日线两连阳后六个；
        //     **08:5x `dailyFourBull` 改成两连阳＋价涨量增 ⇒ 看成交量、移出这一类 ⇒ 五个**；同时新增的周线版同理不计入；
        //     11:0x 新增的 `monthlyVolRising` 看成交量〔13:2x 起还要收阳〕，也不计入）
        //
        // 【结构性关系 —— 2026-09-11 13:2x 起（包含不变；互斥 08:5x +3 对、13:2x 再 +1 对）】
        ...
        //   · **互斥：八对**（三对 → 一对 → 09-10 12:1x 新增三个周线连阳/连阴榜后四对 → 月线两连阳 / 三连阴后首阳 /
        ...
        //     → **09-11 13:2x `monthlyVolRising` 追加「月线阳K」⇒ 与 `monthlyFiveBear` 互斥 ⇒ 八对**），全是判据保证：
        //       `monthlyFourBull` ∩ `monthlyFiveBear`（倒1 一个要阳一个要阴）
        //       `monthlyVolRising` ∩ `monthlyFiveBear`（倒1 阳 vs 阴；13:2x 起，48 个月回放交集 0、改判据前当天交集 2）
        //       `weeklySixBull`   ∩ `weeklyFourBull` （倒5 一个要阳一个要非阳）
        //       `weeklySixBull`   ∩ `weeklyFiveBear` · `weeklyFourBull` ∩ `weeklyFiveBear`（倒1 阳 vs 阴）
        //       `weeklyTwoBullExactCloseVolRising` ∩ `weeklySixBull` · ∩ `weeklyFourBull`（倒3 一个要非阳、一个要阳）
        //       `weeklyTwoBullExactCloseVolRising` ∩ `weeklyFiveBear`（倒1 阳 vs 阴）
        //     ⚠️ `dailyFourBull` 改判据后**不参与任何包含 / 互斥对**（日线端已无别的连阳榜）。
        //     ⚠️ `monthlyVolRising`（09-11 11:0x）建榜时**不参与任何包含 / 互斥 / 孪生**；✏️ **13:2x 起与 `monthlyFiveBear` 互斥**（已计入上面八对），
        //       仍不参与包含 / 孪生：与 `monthlyFourBull` 不包含不互斥（四连阳不看量、它不看前几根）；
        //       「两连阳＋价涨量增」两张里的「量增」同名同义，但叠着别的条件、周期也不同。
        //     ⚠️ `weeklyTripleEmaPersist`（09-11 16:3x）**不参与任何包含 / 互斥 / 孪生**：9/21 扩张那三张要求这一周还在张开、它不要求；它要 EMA55 那档、那三张不看。
        //     ⚠️ `monthlyEma921Persist`（09-11 17:5x）**也不参与任何包含 / 互斥 / 孪生**：月线另四张不看 EMA（48 个月回放与四张各有交集、两个方向差集都非空）；周线那几张 9/21 / 三线的榜周期不同。
        //   · **`weeklyEmaGap` 恒为正的周线榜四个**（09-10 13:3x 移除 `weeklyTripleEma` 后三个，09-11 16:3x 新增 `weeklyTripleEmaPersist` 回到四个）：
        //     `weeklyEmaSarBull` / `weeklyEma921Expansion` / **`weeklyEmaSarFirstTwo`**（都蕴含 9/21 扩张）/ **`weeklyTripleEmaPersist`**（三线排列蕴含 EMA9 在 EMA21 上方）；
        //     `weeklySarUptrend` **不适用**（它完全不看 EMA、可正可负）——**别互抄**。`monthlyEma921Persist` 也不适用（它的 EMA 在月线上，行里没有月线 EMA 间距列、周线EMA间距可正可负）。
        //   · **`emaGap` 恒为正的日线榜只有一个**：`dailyEmaSarFirstTwo`。另两个日线榜 `dailySarFirstBar` / `dailyFourBull`
        //     都不看 EMA、emaGap 可正可负 —— **别互抄**。`dailyEmaSarFirstTwo` ⊊ `dailySarFirstBar`（已计入上面的包含四对）。
        //   · **周线榜九个**（09-11 08:5x 新增 `weeklyTwoBullExactCloseVolRising` 前七个、16:3x 新增 `weeklyTripleEmaPersist` 前八个）/ **月线榜五个**（`monthlySarFirstBar` / `monthlyFourBull` / `monthlyFiveBear` /
        //     `monthlyVolRising` / `monthlyEma921Persist`；09-10 一度六个，09-11 08:1x 移除三个后三个，11:0x、17:5x 各新增一个）/ **日线榜三个**（`dailyEmaSarFirstTwo` / `dailySarFirstBar` / `dailyFourBull`；09-11 06:4x 一度四个）。
        //
        ...
        // **各榜轴集已于 2026-07-30 对齐成同一套**（2026-09-04 加「日SAR多头根数」后为 18 轴、2026-09-11 加周 / 月成交额后为 20 轴）。
        ...
        tabs: [
            ...
            // ✏️ **2026-09-11 14:xx UTC 站长「日线9/21扩张＋SAR多头前两根＋成交额递增改为：日线9/21扩张＋SAR多头前4根＋成交额递增。
            //   另外涉及到SAR多头前2根的都改为SAR多头前4根」** ⇒ 条件② 由前两根放宽成**前四根**（恰好第 1~4 根；后端
            //   `daily_sar_flip_data` ∪ `sarSecondBar` ∪ 新旗标 `sarThirdBar` ∪ `sarFourthBar`，日线零 schema）；name「…前四根…」
            //   （站长写「4」，站内命名一律汉字数字）；⚠️ **key 不动 ⇒ `FirstTwo` 已名不副实**（它是橱窗 + 默认落地榜，改 key 连动五处）。
            //   ⇒ 上面各段「头两根」「1 或 2」「最多连着两天」按「前四根」「1~4」「最多连着四天、第五天必掉出」读。
            //   纯放宽：当天 3 → 5；360 天回放中位 7 / 8/360 天为空（前两根版 19/360）、90 / 180 天 0 天为空；
            //   第三 / 四根那批 5 日后收涨 36.9% vs 第一 / 二根 38.7%（desc 如实写了「多确认几天不是白送的」）。
            //   与 `dailySarFirstBar` 同批改 ⇒ 本榜 ⊊ 它照旧。
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前四根＋成交额递增", tf: "日线",
              desc: "三个条件都看最新已收盘的那根日线：① Parabolic SAR 站在多头一侧，而且这一根是这一轮多头的前四根之一——从刚刚由空翻多的第一根（前一天还在空头一侧），到翻多之后接着站住的第二、第三、第四根都算，已经连着站在多头一侧五天及以上的不在这张榜上；② EMA9 在 EMA21 上方，并且两条均线的间距比前一天更大，也就是均线结构同一天也在张开；③ 这一天的成交额比前一天大。只看 SAR 翻多，每天都会冒出一批标的，其中不少只是下跌途中的一次反抽；再要求均线正在张开，留下的是方向刚转过来、结构也已经跟上的那一小撮；第三条再把其中价格翻上来了、成交额却比前一天还小的筛掉。站内另有一张「日线SAR多头前四根」，只看 SAR 多头的前四根这一个条件、不看均线和成交额；本榜的每一个标的都必然在那张榜上，本榜就是它再叠上均线张开、成交额放大两道条件之后留下的那一小撮。成交额按 USDT 计，就是表格里 日成交额 那一列的数，比的只是这一天和前一天，不是和更早的均量比，也不是按币的个数算的成交量。本榜把翻多当天到翻多第四天这四档放在了一起：想抢第一时间的看第一根，想让信号多被确认几天、滤掉翻多后很快掉头的假信号的看后面几根——表格里的 日SAR多头根数 那一列就是分这几档用的，它在本榜只会是 1 到 4。多确认几天不是白送的：回放过去一年，第三、四根才进来的那批，5 天后收涨的比例比第一、二根略低（约 37% 对 39%），差距不大，但越晚进场、离翻多那一刻的价格就越远。要留意这是一张事件清单而不是状态清单：同一个标的最多连着四天出现在这里，第五天起就不在了，所以成员换得很快、命中数天然不大：回放过去一年，每天命中的中位数在 7 个上下，全市场一起反弹之后的那几天能到一百多个；最近半年每天都有，更早的行情冷清期里有 8 天一个都没有——遇到空榜是正常的。另外表格里的 日EMA间距 一定是正数（判据要求均线正在张开），这是判据决定的、不是数据出错。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "weeklyEmaSarFirstTwo", name: "周线9/21扩张＋SAR多头前四根", tf: "周线",
              desc: "两个条件都在最新已收盘的那根周线上：一是 EMA9 在 EMA21 上方、而且两条均线的间距比上一周更大（结构正在张开），二是周线的 SAR 恰好处在这一轮多头的前四根之内——从刚刚由空翻多的第一根，到翻多之后接着站住的第二、第三、第四根都算。两个条件合起来找的是同一件事的早期形态：均线结构已经开始张开，而按周计的趋势方向也在最近一个月之内才站到多头这一侧。有两处容易读错，先说清楚。第一，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。第二，前四根是恰好不是至少：已经翻多五周、八周的标的不在这张榜上，它们早就过了这个窗口。日线那张「日线9/21扩张＋SAR多头前四根＋成交额递增」的前两个条件和本榜一样、只是放在日线上，另外还多要求当天成交额比前一天大（本榜不看成交额）。两张榜之间没有包含关系，同时出现在两边的标的很少见——日线刚翻多和周线刚翻多是两回事，真撞在一起时反而值得多看一眼。本榜是站内最严的一张周线榜：它的成员必然同时出现在「周线9/21扩张＋SAR多头」「周线9/21扩张」这两张榜里，因为那两张各自只要求本榜条件的一部分。所以反过来用也成立——先看那两张里的哪些标的，最近四周之内才刚刚把趋势和结构凑齐。另有一张「周线SAR多头＋RSI≥50」，它和本榜都要求周线 SAR 多头，但它还要求周线 RSI 不低于 50、本榜不看 RSI，所以本榜的成员不一定在那张榜上。这是一张事件清单不是状态清单：这周的第一根，下周会变成第二根，再往后是第三、第四根，都仍然留在榜上，第五周就必然掉出去，所以同一个标的最多连着出现四周。表格里没有周线 SAR 的根数这一列（日SAR多头根数 量的是日线，见下），想分清是刚翻多还是已经第四周，得自己去看周线图。命中数不多，遇到空榜也正常：回放过去 60 周，每周中位 5 个左右，有 1 周一个都没有；改成前四根的这一周是 26 个（第一根 7 个、第三根 16 个、第四根 3 个），恰好是这 60 周里最多的一周，别把它当成常态。多确认几周不是白送的：回放里第三、四根才进来的那批，下一周仍保持 SAR 多头的比例约 94%，比第一、二根（约 96%）略低。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。门槛方面：算上一周的间距需要至少 22 根已收盘周 K（约 5 个月），所以上市不足半年的新合约不入榜。表格里的各轴都不参与筛选，用来在这批标的里再分强弱：本榜的周线EMA间距 恒为正，这是判据决定的，按它升序能挑出结构刚刚张开的那一批。要提醒一句，日SAR多头根数 那根轴量的是日线不是周线，在本榜上它可以是任意值、也可能显示「—」，别把它当成本榜的筛选条件来读。日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            ...
            { key: "weeklyFiveBear", name: "周线五连阴", tf: "周线",
              desc: "最新已收盘的那根周线，正好是连续第 5 根阴线——这一周收阴，往前数第 2 到第 5 周也都收阴，而第 6 周不是阴线。同样是恰好 5 根不是至少 5 根：已经连跌 6 周、8 周的标的不在这张榜上。本榜是「月线五连阴」的周线版，方向与站内大多数榜相反：它一个多头条件都没有，连续五周下跌就是连续五周下跌。所以它有三种截然不同的读法，取决于你自己的判断：可以当成超跌名单，找跌了一个多月、可能酝酿反弹的标的；也可以当成空头趋势确认，按周计的下跌结构完好；还可以当成回避名单，直接排除掉这些标的。本站不替你选其中哪一种——榜只负责把「按周计已经连跌五周」这件事挑出来。卡在第 5 根的用意是：跌一两周是波动，连跌五周说明这是趋势而不是噪声。这同样是一次性的事件清单——同一个标的只在第 5 根那一周出现，下一周它要么成了第 6 根、要么已经收阳，所以相邻两周的名单不会有重叠；它和「周线四连阳」「周线六连阳」「周线两连阳＋价涨量增」永远不会出现同一个标的，因为那几张都要求这一周收阳，而同一根周线不可能既收阳又收阴。命中数时多时少：回放过去 60 周，多数周份是个位数，全市场一起大跌的周份能到一百四十多个，大约每五六周有一周一个都没有。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱——想找超跌反弹候选的，可以看日线那几根轴有没有先转向。上市不足 6 周的新合约看不出「第 6 周不是阴线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // 🔴 2026-09-11 08:1x 站长「移除：月线两连阳，月线三连阴后首阳，月线两连阳＋成交量递增，日线两连阳」（纯移除、无替代）：
            ...
            { key: "dailySarFirstBar", name: "日线SAR多头前四根", tf: "日线",
              desc: "只看最新已收盘的那根日线：它是这一轮 Parabolic SAR 多头的前四根之一——从 SAR 刚刚由空翻多的第一根（前一天还在空头一侧），到翻多之后接着站住的第二、第三、第四根（仍在多头一侧）都算。已经连着站在多头一侧五天及以上的标的不在这张榜上，表格里的 日SAR多头根数 在本榜只会是 1 到 4：想抢第一时间的看 1，想让信号多被确认几天、滤掉翻多后很快掉头的看 3、4。多确认几天不是白送的，回放过去一年：从第一、二根进来的，第二天就翻回空头的约 4%；从第三、四根进来的约 6% 到 7%，之后这一轮多头还能再撑的天数中位也从 7、8 天缩到 6 天，5 天后的表现同样略差一点——越晚进场、离翻多那一刻的价格就越远。本榜只看 SAR 这一个条件，完全不看均线、成交额或资金流，所以「日线9/21扩张＋SAR多头前四根＋成交额递增」那张榜上的每一个标的都必然也在本榜上——那张就是本榜再叠上均线张开、成交额放大两道条件之后留下的那一小撮；本榜里多出来的，是 SAR 翻多了、但均线还没在张开或者成交额没比前一天大的那些，其中不少只是下跌途中的一次反抽，需要自己分辨，也因此表格里的 日EMA间距 可以是负数（EMA9 还在 EMA21 下方），这是判据决定的、不是数据出错。代价要说清楚：日线 SAR 是很敏感的信号，回放过去一年，翻多之后这一轮多头维持的天数中位只有 9 天左右，大约七分之一在三天之内就重新翻回空头，能撑过十天的不到一半。这是一张事件清单而不是持续清单：同一个标的最多连着四天出现在这里（翻多当天是第一根，之后每站住一天加一根），第五天起就不在了，所以名单换得很快，每天的名单里七成多是前一天就在榜上、只是往后挪了一根的。命中数随行情起伏：回放过去一年，每天中位 56 个左右，全市场一起反弹之后的那几天能到两三百个，一年里没有一天是空的。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。除了 日SAR多头根数，表格里的其余各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
            ...
            // ✏️ **2026-09-11 08:5x UTC 站长「日线四连阳改为日线2连阳且价涨量增。」** ⇒ 判据改 3 条（都在日线）：恰好 2 连阳（倒1阳、倒2阳、倒3非阳）
            //   ∧ 价涨（倒1 收盘 > 倒2 收盘，close-to-close）∧ 量增（倒1 成交量 k[5] > 倒2）；name 改「日线两连阳＋价涨量增」；⚠️⚠️ **key 不动 ⇒ `FourBull` 已完全名不副实**。
            //   上面几行是建榜时（四连阳）的注释。口径三处是替站长定的：恰好（不是至少）、价涨取 close-to-close（不是 close > open）、量增取 k[5]（不是 k[7] 成交额）。
            //   ⇒ 不再算纯 K 线形态（看成交量）；与下面新增的周线版是同判据异周期孪生；不参与任何包含 / 互斥对。rail 位置不动。
            // 📊 改判据后 360 天回放：min 0 / p25 8 / 中位 17 / p75 34 / max 259 / 均值 31.6 / 4 天为空（90 天中位 22.5、0 天为空）；改判据当天 5 个。
            //    「价涨」单独只挡 0.20% 人次；「量增」挡掉恰好两连阳里的 43.0%；成员次日收阳 42.3% vs 量没放大的两连阳 44.3%（放量没带来增益，desc 如实写了）。
            { key: "dailyFourBull", name: "日线两连阳＋价涨量增", tf: "日线",
              desc: "最新已收盘的那根日线，正好是连续第 2 根阳线，而且这第二根比第一根收得更高、成交量也更大——三个条件一起看：一是今天收阳、昨天也收阳，而前天不是阳线（恰好两连阳，已经连涨三天及以上的不在这张榜上）；二是今天的收盘价高于昨天的收盘价（价涨）；三是今天的成交量大于昨天的成交量（量增）。成交量按标的数量计（比如 BTC 合约就是成交了多少个 BTC），不是按 USDT 计的成交额，所以表格里 日成交额 那一列变大并不等于成交量变大——价格涨得多的时候，两者可能一个增一个减。本榜找的是短线连涨刚刚起步、而且第二天有量配合的那一刻：两连阳本身很常见，价涨量增说的是买盘在第二天没有缩手。有两处先说清楚。第一，价涨这一条在加密合约上几乎总是和收阳一起成立——合约 24 小时不停盘，今天的开盘价基本就是昨天的收盘价，收阳也就意味着收得比昨天高，回放过去一年它单独只挡掉了千分之二的样本；真正起筛选作用的是量增，两连阳里有四成多因为第二天量没放大被挡在外面。第二，量增并没有让后面更容易接着涨：回放过去一年，本榜成员第二天继续收阳的比例约 42%，比两连阳里量没放大的那部分（约 44%）还略低一点。所以把它当成放量确认过的起涨点来看就好，别当成更强的做多信号。本榜和「周线两连阳＋价涨量增」是同一套条件，一个看日线、一个看周线，两张榜之间没有包含关系。这是一次性的事件清单不是持续清单——同一个标的只在第 2 根阳线那一天出现，第二天它要么成了第 3 根、要么连阳已经断掉，两种情况都会让它离开本榜，所以相邻两天的名单不会有重叠。命中数随行情起伏：回放过去一年，每天中位 17 个左右，全市场一起普涨后的那一天能到两百多个，一年里有四天一个都没有。日线每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。本榜要求至少 23 根已收盘日 K 才纳入，所以上市不足约 23 天的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-11 08:5x UTC 同一条消息「另外新增一个TAB，逻辑是：周线2连阳且价涨量增。」（没点资产 ⇒ 加密）＝ 上面那张改判据后的周线版。
            //   3 条都在周线：恰好 2 连阳（既有字段 `twoBullExact`）∧ 价涨 ∧ 量增（新增字段 `closeRising` / `volRising` ⇒ WEEKLY_CACHE_SCHEMA 21→22）。追加在末尾（⑭）。
            // ⚠️⚠️ 不是 09-09 移除的 `weeklyTwoBullExact`「周线两连阳第二根」的复活（那个只有形态一条）⇒ 全新 key、三处均无残留。
            // ⚠️⚠️ 与「周线六连阳」「周线四连阳」「周线五连阴」两两结构性互斥（倒3 非阳 vs 阳 / 倒1 阳 vs 阴）⇒ 站内互斥四对 → 七对；desc 里写给用户了。
            // ⚠️ 事件型：相邻两周交集恒空；周线走缓存 ⇒ 周内成员纹丝不动。显示名按汉字数字先例；「价涨量增」照站长用词；同周期多条件 ⇒ ＋。
            // 📊 60 周回放：min 1 / p25 5 / 中位 13 / p75 30 / max 182 / 均值 25.4 / 0 周为空；上线当天 5 个（当周纯恰好两连阳 27 个，量增挡掉 22 个）。
            //    「价涨」单独只挡 0.13% 人次；「量增」挡掉 41.3%；成员下一周收阳 35.0% vs 量没放大的两连阳 40.1%（放量没带来增益，desc 如实写了）。
            { key: "weeklyTwoBullExactCloseVolRising", name: "周线两连阳＋价涨量增", tf: "周线",
              desc: "最新已收盘的那根周线，正好是连续第 2 根阳线，而且这第二根比第一根收得更高、成交量也更大——三个条件都在周线上：一是这一周收阳、上一周也收阳，而再往前一周不是阳线（恰好两连阳，已经连涨三周及以上的不在这张榜上）；二是这一周的收盘价高于上一周的收盘价（价涨）；三是这一周的成交量大于上一周（量增，按标的数量计，不是按 USDT 计的成交额）。本榜是「日线两连阳＋价涨量增」的周线版，条件完全相同、只是换成按周看：连涨两周、第二周放量，说明这一轮上涨已经走出一周以上，而且买盘在第二周没有缩手。和日线那张一样，有两处先说清楚：价涨在加密合约上几乎总与收阳同时成立（这一周的开盘价基本就是上一周的收盘价），回放过去 60 周它单独只挡掉了约千分之一的样本，真正起筛选作用的是量增——两连阳里约四成因为第二周量没放大被挡在外面；而量增并没有让下一周更容易接着涨：回放过去 60 周，本榜成员下一周继续收阳的只有约 35%，比量没放大的那部分两连阳（约 40%）还低。所以把它当成放量确认过的周线起涨点来看就好，别当成更强的做多信号。本榜和「周线四连阳」「周线六连阳」「周线五连阴」永远不会出现同一个标的——两连阳要求往前第 3 周不是阳线，而四连阳、六连阳要求那一周也收阳，五连阴则要求这一周收阴；和日线那张之间没有包含关系。这是一次性的事件清单不是持续清单——同一个标的只在第 2 根阳线那一周出现，下一周它要么成了第 3 根、要么连阳已经断掉，所以相邻两周的名单不会有重叠。命中数随行情起伏：回放过去 60 周，每周中位 13 个左右，全市场一起普涨的周份能到一百八十多个，60 周里没有一周是空的。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 3 周的新合约看不出「再往前一周不是阳线」，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            ...
            // ✏️⚠️ **2026-09-11 13:2x 站长「月线成交量递增的逻辑改为：月线成交量递增且必须是月线阳K。」** ⇒ 2 个条件（都在最新已收盘那根月 K）：
            //   ① 成交量 k[5] > 上一根（没动）② 该月 K 收阳 `close > open`（严格、平盘不算；后端比未 round 的 raw 值）。**key 不动**，名改「月线成交量递增＋阳K」
            //   （同周期多条件 ⇒ ＋，同「周线9/21扩张＋SAR多头＋阴K」的写法）。上面「不看价格方向 / 与各榜无互斥 / 约三分之一还在 / 各回放数字」**全部作废**：
            //   当天 303 → 253（挡掉放量收阴 50、平盘 0）＝ 48 个月最大值；回放 min 0（2022-12）/ 中位 37.5 / max 253 / **1 个月为空**、6 个月个位数；
            //   放量人次里收阳的只有 48.3%（2025-10 放量 305、收阳 38）；次月收阳 36.8%（改前 34.8%、全体 37.2%）、次月量续增 31.9%（全体 45.0%）；
            //   相邻两月留存中位 13.6%。**与 `monthlyFiveBear` 结构性互斥**（desc 写给用户了）；仍不计入纯 K 线形态（看成交量）。
            { key: "monthlyVolRising", name: "月线成交量递增＋阳K", tf: "月线",
              desc: "最新已收盘的那根月线要同时满足两条：一是成交量比上一个月大，二是这根月线本身收阳（收盘价高于开盘价，收平不算）。成交量按标的数量计（比如 BTC 合约就是这个月成交了多少个 BTC），不是按 USDT 计的成交额——价格跌了一半的月份，成交的币数可能翻倍、USDT 成交额却没怎么变。「递增」只比最新这一个月和上一个月，不要求连续好几个月都在放大；「阳」只看这一个月自己收盘有没有高于开盘，不管上个月。所以它找的是「放量上涨」的月份，把放量下跌的那部分挡在外面：回放过去 48 个月，放量的标的里当月收阳的只有一半左右，全市场普跌的月份挡得最狠——比如 2025 年 10 月，放量的有 305 个，收阳的只剩 38 个。命中数起伏很大：回放过去 48 个月，每月中位 38 个左右，少的月份只有个位数，2022 年 12 月一个都没有，空榜不是数据出错；改成这个逻辑的当天正赶上八月全市场普遍放量上涨，上榜 253 个，是这 48 个月里最多的一次，别当成常态。它仍然不是做多信号：回放里上榜标的下个月收阳的约 37%，只比「光看放量」时的约 35% 略高，和全部标的的平均（约 37%）差不多；下个月成交量继续放大的只有约 32%，比全部标的的平均（约 45%）低不少——放量上涨之后更常见的是缩量。所以更适合把它当成一张「这个月谁在放量上涨」的名单，再结合其他条件自己判断，而不是直接当买入清单。本榜和「月线五连阴」永远不会出现同一个标的：一个要求这个月收阳，一个要求这个月收阴。这张榜既不是一次性的事件清单、也不是持续清单——每个月都拿新的一根重新判断一次，下个月还留在榜上的通常只有一两成。月线数据每月 1 号 00:00（UTC）新月线收盘后才刷新一次，同一个月之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱；表格里有「月成交额」这一列（这个月成交了多少 USDT，按它排序时左边还标着全市场排第几），但没有「月成交量增幅」这一列，排不出「放大了多少」。已收盘月线不足两根的新合约没有上一个月可比，不入榜；刚上市第二个月的合约，拿来比的上一个月是不满一整月的上市首月，结果要打个折看。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-11 16:3x UTC 站长「新增一个TAB，逻辑是：周线是EMA9/21/55三线扩张的存续期间。」（没点资产 ⇒ 加密）。追加在末尾（⑯）。
            ...
            { key: "weeklyTripleEmaPersist", name: "周线9/21/55扩张存续", tf: "周线",
              desc: "最新已收盘的那根周线，处在 EMA9/21/55 三线扩张之后的存续期里——两条合起来看：一是这一周 EMA9 在 EMA21 上方、EMA21 又在 EMA55 上方，三线多头排列还立着；二是从这段排列形成以来，至少出现过一周三线扩张，也就是 EMA9 与 EMA21、EMA21 与 EMA55 这两档间距同时比上一周更大。换句话说：均线结构曾经真正张开过，而且到现在都没有被打破。它不要求这一周还在继续张开——扩张过之后进入横盘、间距慢慢收窄，只要三条线的上下顺序没乱，就仍然留在榜上；一旦 EMA9 跌回 EMA21 下方、或者 EMA21 跌回 EMA55 下方，这段存续期就结束了，要等下一轮排列重新成型、并再扩张一次才会回来。所以这是一张状态清单不是事件清单：回放过去 60 周，上一周的成员这一周还留在榜上的比例中位是百分之百，一段存续期中位能维持 13 周左右、四分之一在 18 周以上；每周命中数中位 14 个、最少 2 个、最多 29 个，还没有出现过空榜。有两处先说清楚。第一，扩张量的是加速度不是涨幅：均线间距要比上一周更大才算，一个每周匀速上涨的标的，间距逐周几乎不变，可能迟迟算不上扩张。第二，大多数周份里，本榜和只看三线多头排列的名单几乎一样（回放里中位保留九成六），差别主要落在两类标的上：一类是这段排列一路磨蹭形成、从来没有真正张开过的；另一类是上市一年出头的新合约——EMA55 要攒够 55 根周线才算得出来，如果它第一次算得出来时排列就已经立着，之前有没有扩张过就看不到了，这类标的要等这段排列里下一次两档间距同时变大才会进来（TradingView 上同一个指标看到的也是这样）。它和站内那几张看周线 9/21 扩张的榜没有包含关系：那几张要求这一周 EMA9 与 EMA21 的间距还在变大，本榜不要求；本榜要求 EMA55 那一档，那几张不看。它也不是择时信号：回放里本榜成员下一周收盘比这一周高的比例约 42%，和全部标的的平均（约 40%）差不多——更适合当成「周线趋势结构完好」的候选池，再用表格里的其他轴自己挑。另外周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱；本榜的周线EMA间距 恒为正，这是判据决定的。上市不足 56 周的新合约算不出 EMA55 的前后两周，不入榜；日线数据不足 23 天的，日线那几轴会显示「—」。范围是全部加密 USDT 永续合约。" },
            // ➕ 2026-09-11 17:5x UTC 站长「新增一个TAB，逻辑是：月线EMA9/21扩张存续期间。」（没点资产 ⇒ 加密）。追加在末尾（⑰）。
