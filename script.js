...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "dailySarBearFirstDivergence", name: "日线SAR空头首根＋SAR点高于前一轮空头首根", tf: "日线",
              desc: "两个条件，都落在最新已收盘的那根日线上：一是它是这一轮 Parabolic SAR 空头的首根——SAR 圆点刚刚在这一天由价格下方翻到上方，前一天还在多头一侧；二是这根日线上的 SAR 圆点，比前一轮空头趋势首根那根日线上的 SAR 圆点更高。拆开说第二条：SAR 由多翻空的那一根，圆点会直接跳到刚结束那一波上涨的最高价附近，所以「空头首根的圆点」基本就是这一波上涨的顶；拿它和前一轮空头首根的圆点比，比的是这一波的顶有没有高过上一波的顶——这一波创了更高的高点、趋势却翻了空，也就是这里说的背离。有三处要先说清楚。第一，「前一轮空头」是从这根往回数、越过刚结束的那段多头之后遇到的那一段空头，取它的第一根；那一根必须是真正由多翻空的——日线只取最近约 500 根，最早那段空头的开头可能不在窗口里，这种没有可靠参照的标的不入榜。第二，比较是严格大于，两个圆点一样高不算。第三，本榜只看 SAR，不看收盘价、成交量和其他指标。和「日线SAR多头前四根」正好相反：那张要求最新一根日线的 SAR 在多头一侧，本榜要求刚翻到空头，同一个标的不会同时出现在两张榜上；表格里的「日SAR多头根数」在本榜上一定显示「—」。这是一张事件清单：翻空当天出现，第二天要么继续空头成了第二根、要么翻回多头，都会离开本榜，同一个标的不会连着两天出现。命中数不多，遇到空榜也正常：回放过去 360 天，每天中位 4 个、最多 72 个，有 19 天一个都没有；全部 SAR 空头首根里约四成满足「圆点高于前一轮」；上线当天是 12 个。次日去向：约 97% 第二天仍在空头（成了第二根），翻回多头的不到 3%。它不是做空或做多信号：回放里本榜成员次日收涨的比例 46.5%，同期全市场 45.3%；5 天后收涨的比例 45.3%，全市场 42.1%——差距都很小。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜；日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。范围是全部加密 USDT 永续合约。" },
        ],
    },
