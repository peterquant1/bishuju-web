...
// 标签同样带「日」前缀：加密日线族四个榜的行里装的全是日线值（榜的筛选条件却横跨月/周/日），
...
// **现在只服务一个榜，但别把它合并进 cryptoStrategySorts** —— 另四个加密榜的行没有
...
// ⚠️ 另四个加密榜的行**没有** weeklyRsi/weeklyEmaGap 字段（后端没发），
...
const TABS_CONFIG = {
    ...
    // `dailyFourEmaPersist`「日线四线扩张存续」，回到五个）===
    // **轴集分两族**：①②③④ 用 11 轴的 cryptoStrategySorts（全日线值）；⑤ 独用 13 轴的
    ...
    // ③ 日线四线多头排列**存续**（2026-07-26 指令②，1 个条件）。"**状态**"型榜——只要排列
    // 还立着就一直在，成员跨天稳定。⚠️ 旧注释说它是"全站唯一的状态型榜"，2026-07-27/28 两次
    // 改判据后已不成立：现在只有 ② 是"**事件**"型（扩张＝间距在变大，成员天天换），①③④
    ...
    // ④ 日线四线扩张**存续期**（2026-07-28 站长新增，1 个条件）。同为"状态"型。
    // ⚠️ 与 ③ **只差一句话**：③ 只要"四条线现在排好了"，④ 还要求"这段排列当初是被一次
    // 真扩张启动的"（区间内出现过一根三档间距同时张大的 bar）⇒ **④ 是 ③ 的严格子集**。
    // 两个榜在 rail 里紧挨着就是为了这个对照，别把它们的 desc 互相照抄。
    // ⚠️ 行 payload 与 ③ **共享后端同一个 entry 对象**（逐字同构）⇒ 共用 cryptoStrategySorts。
    // **别因为名字里有"扩张"就加 emaGap / ema55 / ema200 轴** —— 后端没给行发这些字段，
...
const TAB_GROUPS = [
    ...
    // 的规矩（*MonthlyStrategy / weeklyStrategy 先例），继续有效。
    {
        ...
        // `dailyFourEmaPersist` 回到五个；被清掉那些的显示名/desc/专属轴见 git 历史）。
        ...
        // （月×周×日 → 日线三个 → 周线一个）——三个日线榜相邻是刻意的，它们两两只差一处
        // 判据（9/21/55 扩张 vs 四线排列 vs 四线排列+启动过），挨着才看得出对照；其中后两个
        // 还是严格的包含关系。周线原本也是相邻的两个（并集＋周CVD vs 严格 9/21＋周SAR），
        // 2026-07-26 晚站长要求移除前者后只剩一个。
        ...
        tabs: [
            ...
            { key: "dailyFourEmaAligned", name: "日线四线多头排列", tf: "日线",
              desc: "最新已收盘日线上 EMA9 > EMA21 > EMA55 > EMA200 四条均线仍然从短到长依次排开——多头结构还立着的全部标的。注意这里只看「排列在不在」，不要求任何一档间距还在扩大，也不问这个排列当初是怎么形成的，所以它是全站扫描面最宽的日线结构榜：只要结构不破就一直在榜上，成员跨天比较稳定。想只要「当初是被一次真扩张启动、之后一直没破」的那一批，看下面那个「日线9/21/55/200扩张存续」——它是本榜的严格子集。上市不足 201 天、算不出 EMA200 的新合约不入榜。范围是全部 USDT 永续合约。" },
            // tf 日线。⚠️ 2026-07-28 站长「最新已收盘日线处于四线扩张存续期间，也就是说日线
            // 出现过首次扩张后，四条均线依然还是ema9>21>55>200」。
            // ⚠️⚠️ **紧挨着上一个榜是刻意的，判据只差一句话**：上一个只要"现在排好了"，
            // 本榜还要求"这段排列当初被一次真扩张启动过"⇒ **本榜 ⊆ 上一个榜**（后端有实测
            // 自检点）。两条 desc 都互相点了名，改一条时另一条也要看一眼。
            // ⚠️ **name 把周期数写全**（9/21/55/200），遵守加密这边"不用「N 线扩张」说法"的
            // 规矩（完整词义约定见下面 weeklyEmaBearish 那条）。上一个榜叫「四线多头排列」
            // 不违规——那是「排列」不是「扩张」，「N 线扩张」这个受管制的说法才不能用。
            { key: "dailyFourEmaPersist", name: "日线9/21/55/200扩张存续", tf: "日线",
              desc: "在上面那张「四线多头排列」里再筛一道：这段多头排列当初必须是被一次真正的「四线扩张」启动的——历史上某一天 EMA9/21、21/55、55/200 三档间距同时比前一天更大，从那天起四条均线的先后次序一直没有被打破，一直排到最新已收盘这根日线。换句话说，它排除的是「四条线慢慢磨蹭成排列、从来没有一起张开过」的那一类，留下的是「有过明确启动、且趋势结构至今完好」的标的。它不要求今天还在扩张，所以是一张趋势存续清单而不是当日事件清单，成员跨天稳定。上市不足 201 天、算不出 EMA200 的新合约不入榜；另外能回看的历史有限（约 300 根日线），启动点比这更早又一路没破的极少数标的会看不到那次扩张，按规则不入榜。范围是全部 USDT 永续合约。" },
