...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "weeklyAdxDiRising", name: "周线+DI>−DI＋ADX递增或+DI递增", tf: "周线",
              desc: "两个条件都看最新已收盘的周线：一是这一周的 +DI 在 −DI 上方——这是硬过滤，不满足的一律不入榜；二是在此基础上，这一周的 ADX 比上一周高，或者这一周的 +DI 比上一周高，两者满足其一即可。ADX、+DI、−DI 都来自 TradingView 自带的 DMI 指标（默认参数：DI 长度 14、ADX 平滑 14），数值和 TradingView 周线图上看到的一致：+DI、−DI 分别量多方和空方的方向压力，+DI 在上说明多方占优；ADX 只量趋势有多强、不分方向——涨得越来越猛它会升，跌得越来越猛它也会升。第一条就是用来定方向的：上一版没有它，ADX 在升的标的里有很多是下跌正在加速，回放里成员有六成多的时候是 −DI 在上；加上它之后，剩下的都是「多方占优，而且趋势在变强、或者多方压力在变大」的标的。有四处要先说清楚。第一，三处比较都是严格大于，一样高不算；「递增」只比最新这两根周线，不要求连着好几周都在升。第二，第二条是「或」：上线这一周（最新已收盘的是 2026 年 9 月 14 日那一周）251 个里，只有 ADX 在升的 123 个、只有 +DI 在升的 45 个、两个都在升的 83 个。第三，表格里有「周ADX」「周+DI」两列，但只是最新这一周的值，没有周线的 −DI、也没有上一周的值（「日ADX」「日+DI」是日线的值），第一条和两个「比上一周高」没法在表格里直接核对，要点开图表看。第四，三条都要至少 29 根已收盘周 K（ADX 要 28 根才算得出来，再多一根用来比较），所以上市不足约七个月的新合约不入榜；TradingView 上 +DI、−DI 线从第 15 根周线就开始画，上市 16 到 28 周的新合约在那边可能已经满足条件、在这里不入榜——上线这一周有 4 个这样的标的。命中数：回放过去 104 周，每周中位 55 到 56 个、最少 3 个、最多 248 个，没有一周是空的，中位约占可判标的的 18%，大约是上一版的三分之一多；上线这一周是 251 个，比回放里最多的一周还多，是行情偏多时的罕见高位（多方占优的标的本来就多），别当常态。上一周的成员这一周还在的比例中位约 69%。也要如实说一句：加上第一条并没有让它更会挑。回放里成员下一周收涨的比例是 41.8%（7426 个样本），全市场 42.5%，上一版 41.9%；4 周后收盘比入榜那周更高的比例 33.8%，全市场 36.7%，上一版 37.7%——拉长到 4 周反而更差；只看 +DI 在 −DI 上方、不管 ADX 和 +DI 升不升，是 42.9% 和 35.0%。回放这 104 周全市场 4 周涨跌幅的中位数是 −7.6%，大部分时间是山寨币的下跌行情，换一段行情结论可能不同，但就这 104 周而言它不是择时信号。它更适合当成「周线级别多方占优、趋势还在变强」的观察名单，配合别的榜或表格里的各轴再筛。和站内其他周线榜没有判据上的包含关系：上线这一周「周线SAR多头首根」33 个里 23 个、「周线首次9/21扩张」34 个里 29 个也在本榜上。周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线各轴都不参与筛选，用来在这批标的里再分强弱。范围是全部加密 USDT 永续合约。" },
