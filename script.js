...
const TABS_CONFIG = {
    ...
    // === 加密策略：八个榜（数轮增删后 2026-07-30 一度达九个 → 同日站长移除三个周线族 SAR
    // 榜 ⇒ 六个 → 2026-08-01 复活 monthlyWeeklySar ⇒ 七个 → 2026-08-07 站长新增
    // dailyTripleEmaCvd ⇒ **八个**。被移除各榜判据存档见后端 build_rankings docstring 末尾）===
...
const TAB_GROUPS = [
    ...
    {
        // 加密：**八个榜**（2026-07-30 一度达九个 → 同日站长移除三个周线族 SAR 榜 ⇒ 六个
        // → 2026-08-01 复活 monthlyWeeklySar ⇒ 七个 → 2026-08-07 站长「最新已收盘日线是
        // EMA9/21/55扩张，且CVD递增」新增 dailyTripleEmaCvd ⇒ **八个**；被移除各榜的
        // 显示名/desc/判据存档见 git 历史与后端 build_rankings docstring 末尾）。
