...
function axesSub(item, sf, volLabel, extra) {
    ...
    // 周线EMA间距：**加密七个策略榜 + 股票系三个策略榜的行都带**（2026-07-30 轴数对齐后
...
// 加密**七个策略榜全部共用**的轴集，**17 根**（2026-07-30 站长「能否对齐轴数？以最多的
 ...
 *  月线榜装月线 RSI）。这与另七个策略榜相反（那些榜通用 key 一律是日线值、周线值另带
...
const TABS_CONFIG = {
    ...
    // ⚠️ 建榜时它与日线版 `dailySarSecondBar`「只差周期」——那个榜 2026-08-11 已被移除，
    // 且本榜同日改判成"首根"（那个是"第二根"）⇒ **那句"同构"早已双重作废**，别照旧读。
    // ⚠️⚠️ 与 `dailyEmaSarFlip` 都叫"首根"但两处不同：① 周期不同（周线 vs 日线）⇒ 无包含
    // 关系；② **本榜是纯 SAR 首根、完全不看 EMA**，它另要日线 9/21 扩张。别拿本榜当"周线版
    // 的 `dailyEmaSarFlip`"，也别给本榜偷加 EMA 门槛。
    // ⚠️ 本榜条件蕴含「周线 SAR 多头」——自检点（改判据后仍成立）。
    // ⚠️ 行 payload 走后端唯一的 `_strategy_row` ⇒ **17 轴的 cryptoStrategySorts**（七个榜
    ...
    // ⚠️ 本榜条件蕴含「周线 SAR 多头」，与 `weeklySarSecondBar` 都蕴含它、但彼此无包含关系
    // （一个筛结构张开、一个筛刚翻多的那一周〔2026-08-11 那个榜改判据后是首根、不再是"满一周"〕）。
    // ⚠️⚠️ **它曾是 `weeklyEma921Expansion`（2026-08-12 移除）的严格子集** —— 那对包含关系
    //   随对家消失；连同同日移除的 `monthlyWeeklySar` 带走的另一对 ⇒ **站内现已没有任何
    //   一对结构性包含的 live 榜**。别再照旧注释找"那两对"。
    // 行 payload 走后端唯一的 `_strategy_row` ⇒ **17 轴的 cryptoStrategySorts**（七个榜共用；
    ...
    // 按记了九次的先例做加密）
    // ⚠️ 这**恰好是已移除的 `dailyEmaExpansion` 槽位第三版的判据**（2026-07-26～07-28 早），
    // 但建榜时**是新榜不是复活**：那个 key 当时仍 live 且已换成第五版的另一套逻辑，
    // 「同判据 ⇒ 复用旧 key」只适用于 key 已退役的情形（weeklyEmaBearish / monthlyWeeklySar
    // 两个先例）。**它 2026-08-12 被站长移除后，本榜与其余各榜都无包含关系。**
    // ⚠️ 那条「命中数撞上一样先看交集、别下结论」的教训仍然有效（站内为此栽过两次：
    // 「18 = 18」和「17 = 17」都是巧合），只是当年的对家已经不在了。
    ...
    // ⚠️ 与 `dailyTripleEmaCvd` 无包含关系（都 ⊆ 三线扩张，一个再加 EMA200+gap3、一个再加 CVD 递增）。
    // ⚠️ **别因为名字里有四条 EMA 就加 ema55/ema200/emaGap 轴** —— 后端没发这些字段，
    // 加了就是永远全 null 的幽灵轴（axesSub 数据驱动，这个坑站内踩过多次）。
    // 💡 行里 `emaGap` **恒为正**（四线扩张蕴含 9/21 扩张），同 `dailyTripleEmaCvd` 那个免费自检点。
    ...
    //   （2026-08-12 移除 `dailyEmaExpansion` 之前是五个）。
    ...
    //   且 倒2 空头 ⇒ 这个月刚刚由空翻多。
    // ⚠️ 与「周线SAR多头首根」是**真正的同判据异周期对**（都是纯 SAR 首根、都不看 EMA——
    //   不像 ⑥ 还叠一道 9/21 扩张门），周期不同 ⇒ 无包含关系。事件型 SAR 榜现共**三个**
    //   （周/日/月各一）。
    // ⚠️ **月内成员恒定**：判据全在月线、月线走缓存 ⇒ 每月 1 号 00:00 UTC 才换一批，
    //   月内看到成员纹丝不动是正确行为（同 ② 周线榜"周内恒定"的月线版）。
    ...
    // === A股：唯一的单策略榜（2026-07-24 站长定版，2026-07-29 改判据）===
...
const TAB_GROUPS = [
    ...
    {
        // 加密策略榜：**八个**（数量以下面 tabs 数组的条目为准，别在注释里维护一个会发霉
        ...
        // 新榜追加在末尾**（① 月×周×日 → ② 周线SAR首根 → ③ 周线扩张＋SAR →
        // ④ 日线三线＋CVD → ⑤ 日线四线扩张 → ⑥ 日线9/21扩张＋SAR首根 →
        // ⑦ 日线三线＋SAR＋阴K → ⑧ 月线SAR首根；追加在末尾是为了不让整套编号位移，
        // 所以 ⑧ 虽是月线榜也不挪去 ① 旁边）。
        ...
        // ⚠️ 现存**三个**"事件"型 SAR 榜（② 周线首根 / ⑥ 日线首根＋EMA / ⑧ 月线首根，
        // **周期两两不同 ⇒ 均无包含关系**；②⑧ 是真孪生——都是纯 SAR 首根、只差周期，
        // ⑥ 另叠 9/21 扩张门）+ 一个"趋势＋结构双确认"榜（③ 周线9/21扩张＋SAR多头）
        // + 一个纯 SAR 三级共振"状态"型榜（①）。
        // ⚠️⚠️ **站内现已没有任何一对结构性包含的 live 榜**（2026-08-12 起）：此前的两对
        // （① ⊊ `monthlyWeeklySar`、`weeklyEmaSarBull` ⊊ `weeklyEma921Expansion`）**两个
        // 对家当天都被站长移除了**。别再照旧注释去找"那一对/那两对"。
        ...
        tabs: [
            ...
            { key: "dailyTripleEmaCvd", name: "日线9/21/55扩张＋CVD递增", tf: "日线",
              desc: "两个条件都看最新已收盘的那根日线：① EMA9 在 EMA21 上方、EMA21 又在 EMA55 上方，而且 9 与 21、21 与 55 这两档间距都比前一天更大——三条均线依次排开并且同时在张开，不是单纯的多头排列；② CVD 在 0 轴上方且比前一天更高，也就是资金这一侧同步在跟。比起只看两条均线，多要求的那条 EMA55 把「短周期刚翻上来、中周期还没转过来」的那批挡在外面，留下的是中短周期结构已经一致向上、且还在加速张开的标的。它找的是结构确认而不是某一天的启动信号，所以成员跨天比那些盯单日事件的榜稳一些：只要三条均线还在依次张开、资金也没掉头，同一个标的可以连着好几天都在。上市不足 56 天、还算不出 EMA55 的新合约不入榜。范围是全部加密 USDT 永续合约。" },
            ...
            // ⚠️ 与 ②「周线SAR多头首根」判据同构、只差周期（都是纯 SAR 首根、不看 EMA）
            // ⇒ 显示名刻意对称；周期不同 ⇒ 无包含关系。
    ...
    // A股 的判据 2026-07-29 就与美股/ETF 分家了（改成「月线扩张 × 周线SAR × 日线量/CVD/
    // RSI递增」，当天从工厂里拆出来手写），所以移除那两个资产**没动 A股 一个字**。
    // 轴集仍是 singleStrategySorts 16 轴（A股 的三个「递增」是布尔值、不设轴）。
    {
...
//    不会撑破磁贴——所以这里可以放榜名，不用怕最长的那个「日线9/21/55扩张＋SAR多头＋阴K」。
