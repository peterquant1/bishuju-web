...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        tabs: [
            ...
            { key: "weeklyBreakBearSarFirstBull", name: "周线突破SAR空头首根阳线收盘价", tf: "周线",
              desc: "只看最新已收盘的那根周线：它的收盘价，高过了最近一轮周线 SAR 空头段里第一根阳线的收盘价。拆开说：先在周线上找到最近一轮 SAR 处在空头（圆点在 K 线上方）的那一段，在这一段里从头往后找第一根收阳的周 K（收盘高于开盘，也就是这轮下跌里的第一次反弹），把它的收盘价当参照线，再看最新已收盘那根周线的收盘价有没有高过这条线。有五处要先说清楚。第一，「最近一轮」不要求那一段已经结束：如果最新这根周线的 SAR 还在空头侧，参照的就是正在进行的这一轮下跌——收盘站上这轮下跌里第一根反弹阳线的收盘价，是抢在 SAR 翻多之前的信号，回放里约 45% 的命中属于这一类；如果 SAR 已经翻多，参照的就是刚刚结束的那一段。第二，段里第一根就收阳（SAR 刚翻空的那一周收了阳线）也算首根阳线；收盘价正好等于开盘价的十字星不算阳线。第三，那一段里一根阳线都没有的（一路收阴的下跌）没有参照线，不入榜，也不会去更早的一段里找。第四，比较是严格大于；参照的那根阳线恰好就是最新已收盘这一根时，收盘价不可能高过它自己，这类标的不会出现在本榜上。第五，本榜只看周线 K 线和周线 SAR，不看成交量、均线和其他指标。和站内其他榜的关系：它和「周线SAR多头＋RSI≥50」「周线9/21扩张＋SAR多头」没有包含关系——回放里本榜成员有四成多周线 SAR 还在空头侧，反过来那两张也不看这条参照线；和「月线突破SAR空头第二根低点」是同一类思路（收盘价站上最近一轮空头段里的一条参照线），但周期、参照的那根 K 线和参照价都不同，两张之间也没有包含关系。这是一张门槛清单：参照线只在出现新一轮空头段、或者新一段里出现第一根阳线时才会变，而收盘价每周都在动，所以成员会随收盘价在那条线上下进出——回放过去 104 周，上一周的成员这一周还在的比例中位数约 82%。命中数：回放过去 104 周，每周中位 55 个、最少 7 个、最多 250 个，没有哪一周是空的；上线这一周（最新已收盘的是 2026 年 9 月 7 日那一周）是 153 个，属于偏多的一周，别当成常态。也要如实说一句：它不是择时信号。回放里成员下一周收涨的比例是 41.5%（7726 个样本），有参照线但没突破的是 42.0%，全市场 42.3%——本榜略差一点，差距很小。周线数据每周一 00:00（UTC）新周线收盘后才刷新一次，同一周之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的日线、周线、月线各轴都不参与筛选，用来在这批标的里再分强弱。门槛：算 SAR 至少要 3 根已收盘周 K，上市不足三周的新合约不入榜。范围是全部加密 USDT 永续合约。" },
        ],
    },
