...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //     （共用同一个遍历源 `ema921_expansion_data`），但**两个方向都不包含**（一个再要 SAR 前七根、
        ...
        tabs: [
            ...
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前七根", tf: "日线",
              desc: "两个条件都在最新已收盘的那根日线上：一是 EMA9 在 EMA21 上方、而且两条均线的间距比前一天更大（结构正在张开），二是日线的 SAR 恰好处在这一轮多头的前七根之内——从刚刚由空翻多的第一根，到翻多之后接着站住的第二到第七根都算。两个条件合起来找的是同一件事的早期形态：均线结构已经开始张开，而按日计的趋势方向也在最近一周多之内才站到多头这一侧。本榜不看资金流、不看成交量、也不看 K 线的阴阳。有两处容易读错，先说清楚。第一，扩张是两半合起来才算：光是间距在变大还不够，EMA9 必须已经站到 EMA21 上方；一个标的的间距从 −6.2% 收窄到 −5.9%，数字确实变大了，但那时 EMA9 还在下面，不算扩张。第二，前七根是恰好不是至少：已经翻多八天、十天的标的不在这张榜上，它们早就过了这个窗口；表格里的 日SAR多头根数 那一列在本榜上只会是 1 到 7，想抢第一时间的看 1，想让信号多被确认几天的看 6、7。站内另有一张「日线SAR多头前四根」，它只看 SAR 这一个条件、不要求均线张开，而且只收到第四根，所以两张榜互相都不包含：本榜里第五到第七根那一批不在那张榜上，那张榜里均线没张开的也不在本榜上，两边同时出现的标的反而值得多看一眼。另有一张周线版「周线9/21扩张＋SAR多头前四根」，两个条件和本榜逐字同构、只是放在周线上，而且窗口更窄（只收到第四根）。这是一张事件清单不是状态清单：这天是第一根，第二天会变成第二根，一路到第七根都仍然留在榜上，第八天就必然掉出去，所以同一个标的最多连着出现七天；均线一旦不再张开也会中途掉出去。回放过去 360 天，连着出现超过七天的一个都没有。命中数很少断档：回放过去 360 天，每天中位 20 个左右，最少的一天也有 3 个，全市场一起反弹的那天能到 245 个，360 天里没有一天是空的（最近 180 天、90 天最少的一天也有 5 个）。上线当天是 22 个，七档分别是 1 / 3 / 3 / 1 / 6 / 6 / 2 个。本榜是免费橱窗榜：没有通行证也能看到这张榜里排在第一位的那个标的（按日成交额排序的第一名）。要如实说一句：它不是买入信号。回放过去 360 天，本榜成员第二天收盘更高的比例是 42.5%，而全市场同期是 45.3%——SAR 刚翻多、均线刚张开的那几天正是短线最热的时刻，第二天回落的概率反而比平均略高一点。把它当成「谁刚刚转向」的观察名单更合适，真要动手还得自己看图、自己定仓位。另外日线 SAR 是很敏感的信号，翻多之后维持不了几天就重新翻空是常事；本榜把窗口放到第七根，是在「抢第一时间」和「多等几天确认」之间取中，并不代表信号更可靠——越往后进场，离翻多那一刻的价格就越远。日线数据每天 00:00（UTC）收盘后才换一批，同一天之内反复打开本榜，看到的标的完全一样，这是正确行为不是数据卡住了。表格里的各轴都不参与筛选，用来在这批标的里再分强弱。上市不足 23 天的新合约日线数据不够，不入榜。范围是全部加密 USDT 永续合约。" },
