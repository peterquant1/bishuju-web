...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //   · **周线榜七个**（09-10 12:1x 由五个变八个，13:3x 移除 `weeklyTripleEma` 后七个）/ **月线榜六个**（当日第 6 条加 `monthlyTwoBullExact`、第 7 条加 `monthlyBullAfterThreeBear`、第 8 条加 `monthlyTwoBullExactVolRising`）/ 日线榜一个。
        //
        // ⚠️⚠️ **站内已不再有任何要求「阴K」或找「回调」的榜**（三个在 2026-09-09 15:3x 被
        //   一次移除）。**但别把这句当永久前提去简化代码** —— 在案通用教训：「『站内已经
        //   没有 X 了』下一条指令就可能把 X 加回来」，已应验六次（08-16 阴K / 08-18 互斥 /
        ...
        tabs: [
            ...
            { key: "dailyEmaSarFirstTwo", name: "日线9/21扩张＋SAR多头前两根＋成交额递增", tf: "日线",
              desc: "三个条件都看最新已收盘的那根日线：① Parabolic SAR 站在多头一侧，而且这一根是这一轮多头的头两根之一——要么是刚刚由空翻多的第一根（前一天还在空头一侧），要么是紧接着站住的第二根；② EMA9 在 EMA21 上方，并且两条均线的间距比前一天更大，也就是均线结构同一天也在张开；③ 这一天的成交额比前一天大。只看 SAR 翻多，每天都会冒出一批标的，其中不少只是下跌途中的一次反抽；再要求均线正在张开，留下的是方向刚转过来、结构也已经跟上的那一小撮；第三条再把其中价格翻上来了、成交额却比前一天还小的筛掉。成交额按 USDT 计，就是表格里 日成交额 那一列的数，比的只是这一天和前一天，不是和更早的均量比，也不是按币的个数算的成交量。本榜把「翻多当天」和「次日站住」这两档放在了一起：想抢第一时间的看第一根，想让信号多被确认一天、滤掉翻多当天就掉头的假信号的看第二根——表格里的 日SAR多头根数 那一列就是分这两档用的，它在本榜只会是 1 或 2。要留意这是一张事件清单而不是状态清单：同一个标的最多连着两天出现在这里，第三天起就不在了，所以成员换得很快、命中数天然很小；加上成交额这一条之后更少，回放过去 90 天，每天命中的中位数在 6 个上下，其中有 3 天一个都没有——遇到空榜是正常的。另外表格里的 日EMA间距 一定是正数（判据要求均线正在张开），这是判据决定的、不是数据出错。范围是全部加密 USDT 永续合约。" },
