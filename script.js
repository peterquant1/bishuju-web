...
function axesSub(item, sf, volLabel, extra) {
    ...
    // 周线EMA间距：**两个加密周线榜的行带**（weeklySarSecondBar + weeklyEmaSarBull，
    // 都走后端 `_wk_expansion_row`），数据驱动判断。
    // ⚠️ 值可正可负，别再套那条早已作废的「恒为正」自检点（旧 weeklyEmaBearish 时代的）：
    // weeklySarSecondBar 不要求 EMA 扩张 ⇒ 负值完全正常（周线方向已转、9/21 还没张开＝最早期
    // 形态）；而 weeklyEmaSarBull 的入榜条件之一就是 9/21 扩张 ⇒ 在那个榜上此值恒为正。
...
// === 周成交额 / 周线EMA间距（2026-07-25 晚指令⑪加，现为加密周线族两个榜专用）===
// ⚠️ 这两根轴是为 `weeklyExpansionDailyCvd` 建的，那个榜 2026-07-26 晚被移除；轴**照常
// 保留**——现存的两个周线榜（weeklySarSecondBar + weeklyEmaSarBull）走同一个后端行构造
// `_wk_expansion_row`，两根轴一直有值。
...
// 加密周线族的 **13 轴**（原为 `weeklyEmaBearish` 指令①③ 而建；指令⑥砍 DI 前是 15 轴）。
// 站长原话「支持现在各种日线的升降序。也要有周成交额，周RSI等数据。」
// ⚠️ 消费者历经多次增删（`weeklyExpansionDailyCvd` / `weeklyEmaBearish` / 两个突破榜均先后
// 被移除），**现在服务 ⑤⑥ 两个榜**（`weeklySarSecondBar` + `weeklyEmaSarBull`）。这个数字
// 每次加/删榜都要跟着改（一度写过"一个"/"三个"/"五个"，都漏改过）。
// **别把它合并进 cryptoStrategySorts** —— 日线族四个榜（①②③④）的行没有
// weeklyRsi/weeklyEmaGap 字段，合并会多出永远全 null 的幽灵轴。
...
// ⚠️ 日线族四个榜（①②③④）的行**没有** weeklyRsi/weeklyEmaGap 字段（后端没发），
...
const TABS_CONFIG = {
    ...
    // === 加密策略：六个榜（数轮增删后 2026-07-30 一度达九个 → 同日站长「移除:周线突破SAR
    // 空头起点/日线突破周线SAR空头起点/周线SAR多头，共三个TAB」⇒ **六个**。移除的三个都是
    // 13 轴周线族，判据存档见后端 build_rankings docstring 末尾）===
    ...
    // ⚠️ 新榜一律**追加在末尾**；移除中间的榜时要把后面的**重新编号**（本次移除了旧④⑤⑥、
    ...
    // （与 ⑥ 共用，该常量现有两个消费者）。轴集与 ④ 不同是刻意的：本榜条件在周线，
    ...
    // 共用，该常量现有两个消费者）。**轴一根没加**（站长「支持当前各种升降序」＝沿用现成
...
const TAB_GROUPS = [
    ...
    {
        // 加密：**六个榜**（2026-07-30 一度达九个 → 同日站长「移除:周线突破SAR空头起点/日线
        // 突破周线SAR空头起点/周线SAR多头，共三个TAB」⇒ **六个**；被移除的榜的显示名/desc/
        // 判据存档见 git 历史与后端 build_rankings docstring 末尾）。
        // ⚠️ **刻意不套 singleStrategyGroup 工厂**：那个工厂的 name/desc 写死的是股票系
        // 的 5 个条件（月线SAR + 周线SAR + 周线扩张 + 日线扩张 + CVD），与这六个榜都不
        // 一样。硬套会让页面上的规则说明与后端实际筛选严重不符。
        // 组 tf 取第一个 tab 的周期（月线），其余五个 tab 各自覆盖。**排法：同周期的放一起**
        // （① 月×周×日 → ②③ 日线两个 → ④ 日线SAR第二根 → ⑤ 周线SAR第二根 → ⑥ 周线扩张＋SAR）。
        // ⚠️ **移除中间的榜要把后面的重新编号**：本次移除了旧④⑤⑥（三个周线 SAR 榜），把旧
        // ⑦⑧⑨ 重编成 ④⑤⑥。① ~ ⑥ 是 rail 位置编号，后端 docstring / CLAUDE.md / 本文件多处
        // 按它互相引用，别留缺口。
        // ⚠️ 现存两个"事件"型 SAR 榜（④ 日线第二根 = 11 轴日线族 / ⑤ 周线第二根 = 13 轴周线族）
        // + 一个"趋势＋结构双确认"榜（⑥ 周线9/21扩张＋SAR多头 = 13 轴周线族）。⑤⑥ 都蕴含
        // 「周线 SAR 多头」但彼此无包含关系（一个筛刚翻多满一周、一个筛结构在张开）。
        ...
        tabs: [
            ...
            // 一撮。轴集与 ⑥ `weeklyEmaSarBull` 相同（13 轴，行走 _wk_expansion_row）。
