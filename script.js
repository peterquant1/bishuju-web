...
const TAB_GROUPS = [
    ...
    },
    {
        ...
        //
        // 【现状分类】（2026-09-11 16:3x 新增 `weeklyTripleEmaPersist` 之后；13:2x `monthlyVolRising` 追加「月线阳K」改名「月线成交量递增＋阳K」；11:0x 新增它；更早 08:5x `dailyFourBull` 改判据 ＋ 新增 `weeklyTwoBullExactCloseVolRising`）
        ...
        //   · **周线榜九个**（09-11 08:5x 新增 `weeklyTwoBullExactCloseVolRising` 前七个、16:3x 新增 `weeklyTripleEmaPersist` 前八个）/ **月线榜四个**（`monthlySarFirstBar` / `monthlyFourBull` / `monthlyFiveBear` /
        //     `monthlyVolRising`；09-10 一度六个，09-11 08:1x 移除三个后三个，11:0x 新增一个）/ **日线榜三个**（`dailyEmaSarFirstTwo` / `dailySarFirstBar` / `dailyFourBull`；09-11 06:4x 一度四个）。
        //
